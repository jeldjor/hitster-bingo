...
/* =========================
   V135 - DJ Pads Only
   - Alle rad/carrousel/lichtjes animaties uit de actieve flow
   - Host en speler zien dezelfde DJ-pad animatie
   - 12 seconden, stopt exact op gekozen kleur
   ========================= */
(function(){
  const q=id=>document.getElementById(id);
  const E=s=>(typeof esc==='function'?esc(String(s??'')):String(s??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m])));
  ...
  function categoryFor(color){try{return q(color.input)?.value||'Geen categorie'}catch(_e){return 'Geen categorie'}}
  function easeOut(t){return 1-Math.pow(1-t,2.65)}
  function cancel(){if(raf){cancelAnimationFrame(raf);raf=null}}

  function killOld(){
    try{window.__bbIntroV131?.hideIntro?.()}catch(_e){}
    try{window.__bbWheelV92?.hideWheelFull?.()}catch(_e){}
    try{window.__bbWheelV110?.hideWheelV110?.()}catch(_e){}
    document.querySelectorAll('#bbIntroOverlayV131,#bbWheelFullOverlayV92,#bbWheelV123Overlay,#bbWheelV125Overlay,.bbIntroCardV131,.bbCarouselWindowV131,.bbCarouselTrackV131,.bbLightRingV131,.wheelWrap,.wheel,.dashWheel,.showWheel').forEach(el=>{
      if(el.id==='bbDjOverlayV135') return;
      el.style.setProperty('display','none','important');
      el.style.setProperty('visibility','hidden','important');
    });
  }
  function overlay(){
    let o=q('bbDjOverlayV135');
    if(!o){o=document.createElement('div');o.id='bbDjOverlayV135';o.className='bbDjOverlayV135 hidden';document.body.appendChild(o)}
    return o;
  }
  function hideDj(){cancel();activeKey='';const o=q('bbDjOverlayV135');if(o){o.classList.add('hidden');o.innerHTML=''}}
  function html(roundId){
    return `<div class="bbDjStageV135" data-round="${E(roundId||'')}">
      <div class="bbDjTitleV135">Categorie wordt gekozen...</div>
      <div class="bbDjPadsV135">
        ${KEYS.map(k=>`<div class="bbDjPadV135" data-key="${E(k)}" style="--pad:${HEX[k]}">${EMOJI[k]}</div>`).join('')}
      </div>
      <div class="bbDjResultV135"><div class="bbDjSubV135">DJ Pads draaien...</div></div>
    </div>`;
  }
  function resultHtml(color,cat){
    return `<div class="bbDjStageV135"><div class="bbDjResultV135" style="--chosen:${color.hex}">
      <div class="bbDjColorV135">${E(color.emoji)} ${E(color.name)}</div>
      <div class="bbDjCatV135">${E(cat||'')}</div>
    </div></div>`;
  }
  function animate(targetKey){
    const pads=[...document.querySelectorAll('#bbDjOverlayV135 .bbDjPadV135')];
    if(!pads.length)return;
    const targetIdx=Math.max(0,KEYS.indexOf(targetKey));
    // Exact eindigen op targetIdx: 70 is deelbaar door 5, + targetIdx = juiste kleur.
    const total=70+targetIdx;
    let last=-1;
    const set=i=>{
      const idx=((i%5)+5)%5;
      if(idx===last)return;
      last=idx;
      pads.forEach(p=>p.classList.remove('active','final'));
      pads[idx]?.classList.add('active');
    };
    const start=performance.now();
    function frame(now){
      const t=Math.min(1,(now-start)/INTRO_MS);
      const step=Math.min(total,Math.floor(easeOut(t)*total));
      set(step);
      if(t<1)raf=requestAnimationFrame(frame);
      else{cancel();pads.forEach(p=>p.classList.remove('active','final'));pads[targetIdx]?.classList.add('active','final');}
    }
    set(0);raf=requestAnimationFrame(frame);
  }
  function showDj(targetKey,roundId){
    const key=(roundId||'')+':'+targetKey;
    killOld();
    const o=overlay();
    if(activeKey===key&&!o.classList.contains('hidden'))return;
    cancel();activeKey=key;
    o.innerHTML=html(roundId);
    o.classList.remove('hidden');
    animate(targetKey);
  }
  function showDjResult(targetKey,cat){
    cancel();activeKey='result:'+targetKey;
    const color=colorObj(targetKey);const o=overlay();
    o.innerHTML=resultHtml(color,cat);o.classList.remove('hidden');
    try{navigator.vibrate&&navigator.vibrate(50)}catch(_e){}
  }

  // Oude animatie-API's definitief neutraal maken.
  window.__bbIntroV131={showIntro:showDj,showResult:showDjResult,hideIntro:hideDj};
  window.__bbWheelV92={showWheelFull:()=>{},showWheelResult:()=>{},hideWheelFull:()=>{}};
  window.__bbWheelV110={showWheelV110:()=>{},showResultV110:()=>{},hideWheelV110:()=>{},wheelHTMLV110:()=>''};
  window.__bbWheelV119={showWheel:()=>{},showResult:()=>{},hideWheel:()=>{},wheelHTML:()=>'',resultHTML:()=>''};
  window.__bbWheelFixedV121={showIndex:()=>{},showKey:()=>{},showResult:()=>{},hide:()=>{},htmlWheel:()=>'',htmlResult:()=>''};
  if(typeof pickerHTML==='function') pickerHTML=function(){return '<div class="bbDjHostStatusV135">Categorie wordt gekozen...</div>'};

  // Nieuwe host flow: alleen DJ Pads.
  startRoundVisual=function(room){
    hideDj();killOld();
    if(q('hostAnswerArea'))q('hostAnswerArea').innerHTML='';
    if(q('playBtn')){q('playBtn').disabled=true;q('playBtn').textContent='🎵 Nummer start zo...'}
    if(q('showAnswerBtn'))q('showAnswerBtn').disabled=true;
    if(q('hostPickerArea'))q('hostPickerArea').innerHTML='<div class="bbDjHostStatusV135">Categorie wordt gekozen...</div>';
    const pool=(typeof COLORS!=='undefined'&&COLORS.length)?COLORS.map(c=>colorObj(c.key)):KEYS.map(colorObj);
    const color=pickOne(pool),cat=categoryFor(color);
    currentRoundId='r_'+Date.now();
    db.ref('rooms/'+room+'/currentRound').set({
      id:currentRoundId,status:'picking',pickerMode:'djPadsV135',introMode:'djPads',pendingColorKey:color.key,pendingColorName:color.name,pendingColorEmoji:color.emoji,pendingCategory:cat,pickerStartedAt:firebase.database.ServerValue.TIMESTAMP,seconds:Number(q('duration')?.value)||20
    }).then(()=>showDj(color.key,currentRoundId));
    setTimeout(()=>{
      try{if(typeof flash==='function')flash()}catch(_e){}
      showDjResult(color.key,cat);
      if(q('hostPickerArea'))q('hostPickerArea').innerHTML=`<div class="bbIntroHostResultV131">${E(color.emoji)} ${E(color.name)}<br><span>${E(cat)}</span></div>`;
      const ends=Date.now()+3000;
      db.ref('rooms/'+room+'/currentRound').set({
        id:currentRoundId,status:'precount',pickerMode:'djPadsV135',introMode:'djPads',colorKey:color.key,colorName:color.name,colorEmoji:color.emoji,category:cat,seconds:Number(q('duration')?.value)||20,precountStartedAt:firebase.database.ServerValue.TIMESTAMP,precountEndsAt:ends
      }).then(()=>{
        q('hostScorePanel')?.classList.remove('hidden');
        if(q('hostStatus'))q('hostStatus').textContent='Categorie gekozen. 3-2-1... daarna start het nummer.';
  ...
  // Spelers: bij picking altijd DJ Pads tonen; nooit oude animaties.
  const prevRender=typeof renderCompactDashboard==='function'?renderCompactDashboard:null;
  renderCompactDashboard=function(room,r){
    if(r&&r.status==='picking'){
      const root=q('screenDashboard');
      if(root){document.body.classList.add('bbStageMode');root.className='compactDashboard bbStageDashboard stageIntroV135';root.innerHTML='<section class="bbDjPlayerStatusV135">Categorie wordt gekozen...</section>'}
      showDj(r.pendingColorKey||r.colorKey||'yellow',r.id||'');
      return;
    }
  ...
  let bound='';
  function roomCodeNow(){return(currentRoomCode||new URLSearchParams(location.search).get('room')||localStorage.hb_player_room||localStorage.hb_host_room||'').toUpperCase()}
  function bind(){
    try{if(!db)return;const room=roomCodeNow();if(!room||room===bound)return;if(bound)db.ref('rooms/'+bound+'/currentRound').off('value.bbDjV135');bound=room;db.ref('rooms/'+room+'/currentRound').on('value.bbDjV135',s=>{const r=s.val()||{};killOld();if(r.status==='picking'){showDj(r.pendingColorKey||r.colorKey||'yellow',r.id||'');if(q('hostPickerArea'))q('hostPickerArea').innerHTML='<div class="bbDjHostStatusV135">Categorie wordt gekozen...</div>'}else hideDj();});}catch(_e){}
  }
  document.addEventListener('DOMContentLoaded',()=>setTimeout(bind,300));
  setInterval(()=>{bind();killOld()},800);
})();
