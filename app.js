...
/* =========================
   V12 HOST + SPELER COMPACT MODE
   - Startpopup keuze: Alleen host / Host + speler
   - Host + speler blijft in compact dashboard
   - Rechtsonder = host acties
   - Scorebord toont pas antwoorden na eigen antwoord
   - Na timer goed/fout beoordelen inclusief jezelf
   ========================= */

let hbHostPlayerModeV12 = localStorage.getItem("hb_host_player_mode_v12") === "1";
let hbHostPlayerIdV12 = localStorage.getItem("hb_host_player_id_v12") || "";
let hbHostPlayerNameV12 = localStorage.getItem("hb_host_player_name_v12") || "";
let hbHostPlayerRoomV12 = "";

function hbRoomV12(){
  return currentRoomCode || localStorage.getItem("hb_host_room") || localStorage.getItem("hb_last_stable_room") || "";
}

function hbEnsureStartChoiceV12(){
  const overlay = document.getElementById("hbHostStartOverlay") || document.getElementById("hbStartOverlay");
  const modal = overlay?.querySelector(".hbHostStartModal,.hbStartModal");
  if(!modal || document.getElementById("hostModeStartChoiceV12")) return;

  const box = document.createElement("div");
  box.id = "hostModeStartChoiceV12";
  box.className = "hostModeStartChoice";
  box.innerHTML = `
    <button id="hostOnlyStartV12" class="secondary">🎮 Alleen host</button>
    <button id="hostPlayerStartV12">👤 Host + speler</button>
  `;
  modal.appendChild(box);

  document.getElementById("hostOnlyStartV12").addEventListener("click", () => {
    hbHostPlayerModeV12 = false;
    localStorage.setItem("hb_host_player_mode_v12","0");
    alert("Alleen host gekozen. Je kunt spelen zoals normaal.");
  });

  document.getElementById("hostPlayerStartV12").addEventListener("click", hbStartHostPlayerModeV12);
}

function hbStartHostPlayerModeV12(){
  const room = hbRoomV12();
  if(!room){
    alert("Maak of hervat eerst een kamer.");
    return;
  }

  let name = hbHostPlayerNameV12 || localStorage.getItem("hb_player_name") || "";
  name = (prompt("Naam waarmee jij meespeelt:", name || "Host") || "").trim();
  if(!name) return;

  hbHostPlayerModeV12 = true;
  hbHostPlayerRoomV12 = room;
  hbHostPlayerNameV12 = name;
  localStorage.setItem("hb_host_player_mode_v12","1");
  localStorage.setItem("hb_host_player_name_v12",name);

  if(!hbHostPlayerIdV12){
    hbHostPlayerIdV12 = "host_" + Math.random().toString(36).slice(2,10) + Date.now().toString(36).slice(-4);
    localStorage.setItem("hb_host_player_id_v12", hbHostPlayerIdV12);
  }

  const ref = db.ref("rooms/" + room + "/players/" + hbHostPlayerIdV12);
  ref.once("value").then(s => {
    const ex = s.val() || {};
    return ref.update({
      name,
    ...
    currentRoomCode = room;
    currentPlayerId = hbHostPlayerIdV12;
    currentPlayerName = name;
    localStorage.setItem("hb_player_room", room);
    localStorage.setItem("hb_player_id", hbHostPlayerIdV12);
    localStorage.setItem("hb_player_name", name);

    const overlay = document.getElementById("hbHostStartOverlay") || document.getElementById("hbStartOverlay");
    if(overlay) overlay.classList.add("hidden");

    hbEnterHostPlayerDashboardV12();
  });
}

function hbEnterHostPlayerDashboardV12(){
  document.body.classList.add("playerMode","hostPlayerMode");

  const hostApp = document.getElementById("hostApp");
  const playerApp = document.getElementById("playerApp");
  if(hostApp) hostApp.classList.add("hidden");
  ...
  const roomCode = document.getElementById("playerRoomCode");
  if(roomCode) roomCode.textContent = currentRoomCode;

  if(typeof listenPlayer === "function") listenPlayer();
  if(typeof listenHost === "function") listenHost(currentRoomCode);
  if(typeof listenBingo === "function") listenBingo(currentRoomCode);

  hbListenHostPlayerRoomV12();
}

function hbListenHostPlayerRoomV12(){
  const room = hbRoomV12();
  if(!room || window.__hbHostPlayerListenV12 === room) return;
  window.__hbHostPlayerListenV12 = room;

  db.ref("rooms/" + room).on("value", snap => {
    const data = snap.val() || {};
    hbRenderHostActionPanelV12(data);
  });
}

function hbOwnAnsweredV12(room,r){
  return !!(r && r.id && room.answers && room.answers[r.id] && room.answers[r.id][hbHostPlayerIdV12]);
}

function hbAllPlayersJudgedV12(room,r){
  const ps = room.players || {};
  const correct = room.correct && r?.id ? (room.correct[r.id] || {}) : {};
  return Object.keys(ps).length > 0 && Object.keys(ps).every(pid => typeof correct[pid] !== "undefined");
}

function hbRenderHostActionPanelV12(room){
  if(!hbHostPlayerModeV12) return;

  const area = document.getElementById("dashPickerArea");
  if(!area) return;

  const r = room.currentRound || {};
  const ownAnswered = hbOwnAnsweredV12(room,r);

  if(!r.id){
    area.innerHTML = `
      <div class="hostPlayerActionBox">
        <div class="hostPlayerActionTitle">🎮 Host actie</div>
        <button onclick="hbHostStartRoundV12()">🎲 Start ronde</button>
        <div class="hostPlayerActionSub">Iedereen ready? Start de kleur/animatie.</div>
      </div>`;
    return;
  }

  if(r.status === "picking"){
    // Laat bestaande animatie zichtbaar blijven als speler-render hem net heeft gezet
    if(!area.innerHTML || area.innerHTML.includes("Host actie")){
      area.innerHTML = `<div class="hostPlayerActionBox"><div class="hostPlayerActionTitle">Animatie draait...</div></div>`;
    }
    return;
  }
  ...
  if(r.status === "ready"){
    area.innerHTML = `
      <div class="hostPlayerActionBox">
        <div class="hostPlayerActionTitle">${esc(r.colorEmoji || "")} ${esc(r.colorName || "")}</div>
        <div class="hostPlayerActionSub">${esc(r.category || "")}</div>
        <button onclick="hbHostPlaySongV12()">🎵 Start liedje</button>
      </div>`;
    return;
  }
  ...
  if(r.status === "answering"){
    area.innerHTML = `
      <div class="hostPlayerActionBox">
        <div class="hostPlayerActionTitle">⏱️ Liedje speelt</div>
        <div class="hostPlayerActionSub">${ownAnswered ? "Je antwoord is ingestuurd." : "Vul eerst jouw antwoord in."}</div>
        ${ownAnswered ? `<div class="hostPlayerMiniAnswer">Daarna zie je het scorebord.</div>` : `<div class="hostPlayerGateMsg">Geen antwoorden bekijken vóór jouw eigen antwoord.</div>`}
      </div>`;
    return;
  }
  ...
  if(r.status === "locked"){
    ...
    area.innerHTML = `
      <div class="hostPlayerActionBox">
        <div class="hostPlayerActionTitle">⏱️ Tijd voorbij</div>
        ${ans ? `<div class="hostPlayerMiniAnswer">🎵 ${esc(ans.track||"-")}<br>👤 ${esc(ans.artist||"-")}</div>` : `<button onclick="hbHostShowAnswerV12()">✅ Toon antwoord</button>`}
        <div class="hostPlayerActionSub">Keur nu alle antwoorden in het scorebord.</div>
      </div>`;
    return;
  }
  ...
  if(r.status === "judged"){
    area.innerHTML = `
      <div class="hostPlayerActionBox">
        <div class="hostPlayerActionTitle">✅ Ronde klaar</div>
        <button onclick="hbHostStartRoundV12()">🎲 Volgende ronde</button>
        <div class="hostPlayerActionSub">Goede spelers kiezen hun vakje.</div>
      </div>`;
    return;
  }
}

window.hbHostStartRoundV12 = function(){
  if(typeof startRound === "function") startRound();
};

window.hbHostPlaySongV12 = function(){
  if(typeof playHidden === "function") playHidden();
};

window.hbHostShowAnswerV12 = function(){
  if(typeof showAnswer === "function") showAnswer();
};

// Scoreboard compact: host+speler mag pas antwoorden zien na eigen antwoord.
// Na locked/judged krijgt host goed/fout knoppen in scorebord, inclusief zichzelf.
function hbRenderHostPlayerScoreV12(room){
  const score = document.getElementById("dashScoreboard");
  if(!score) return;

  const r = room.currentRound || {};
  if(!r.id){
    score.innerHTML = `<div class="hostPlayerMiniAnswer">Nog geen ronde.</div>`;
    return;
  }

  const ownAnswered = hbOwnAnsweredV12(room,r);
  const canSeeAnswers = ownAnswered || ["locked","judged"].includes(r.status);
  const canJudge = ["locked","judged"].includes(r.status);
  const players = room.players || {};
  const answers = room.answers && room.answers[r.id] ? room.answers[r.id] : {};
  const correct = room.correct && room.correct[r.id] ? room.correct[r.id] : {};

  if(!canSeeAnswers){
    score.innerHTML = `<div class="hostPlayerGateMsg">Vul eerst jouw eigen antwoord in. Daarna zie je de antwoorden van anderen.</div>`;
    return;
  }

  score.innerHTML = Object.entries(players).map(([pid,p]) => {
    const answerText = answers[pid]?.answer || (answers[pid] ? "Leeg antwoord" : "Nog niet ingevuld");
    const st = correct[pid];
    const icon = st === true ? "✅" : st === false ? "❌" : (answers[pid] ? "📝" : "⏳");

    if(canJudge){
      return `<div class="hostPlayerApproveRow">
        <div><strong>${esc(p.name || "Speler")}${pid===hbHostPlayerIdV12 ? " (jij)" : ""}</strong><br>${esc(answerText)}</div>
        <div class="hostPlayerApproveBtns">
          <button class="${st===true ? "goodSelected" : ""}" onclick="hbJudgePlayerV12('${pid}',true)">✅</button>
          <button class="${st===false ? "badSelected" : ""}" onclick="hbJudgePlayerV12('${pid}',false)">❌</button>
        </div>
      </div>`;
    }

    return `<div class="compactScoreRow">
      <div class="compactInitial">${esc((p.name || "?").slice(0,1).toUpperCase())}</div>
      <div class="compactName">${esc(p.name || "Speler")}${pid===hbHostPlayerIdV12 ? " (jij)" : ""}</div>
      <div class="compactAns">${esc(answerText)}</div>
      <div class="compactIcon">${icon}</div>
    </div>`;
  }).join("");
}

window.hbJudgePlayerV12 = function(pid, good){
  const room = hbRoomV12();
  db.ref("rooms/" + room + "/currentRound").once("value").then(s => {
    const r = s.val() || {};
    if(!r.id) throw Error("Geen actieve ronde.");
    ...
    return db.ref().update(updates).then(() => {
      return db.ref("rooms/" + room).once("value");
    }).then(s2 => {
      const data = s2.val() || {};
      const rr = data.currentRound || {};
      if(hbAllPlayersJudgedV12(data, rr)){
        return db.ref("rooms/" + room + "/currentRound/status").set("judged");
      }
    });
  }).catch(e => alert(e.message));
};

// bestaande compact score renderer overriden in host+speler mode
if(typeof renderCompactScoreboard === "function"){
  const oldRenderCompactScoreboardV12 = renderCompactScoreboard;
  renderCompactScoreboard = function(room){
    if(hbHostPlayerModeV12 && document.body.classList.contains("hostPlayerMode")){
      hbRenderHostPlayerScoreV12(room || {});
      return;
    }
    return oldRenderCompactScoreboardV12.apply(this, arguments);
  };
}

// ook renderScore realtime afvangen indien compact dashboard die gebruikt
if(typeof renderScoreRealtime === "function"){
  const oldRenderScoreRealtimeV12 = renderScoreRealtime;
  renderScoreRealtime = function(room){
    if(hbHostPlayerModeV12 && document.body.classList.contains("hostPlayerMode")){
      hbRenderHostPlayerScoreV12(room || {});
      return;
    }
    return oldRenderScoreRealtimeV12.apply(this, arguments);
  };
}

setTimeout(hbEnsureStartChoiceV12, 700);
setTimeout(hbEnsureStartChoiceV12, 1600);
setInterval(hbEnsureStartChoiceV12, 2500);
