...
(function(){
  ...
  async function runSpotifyPreGameTestV99(){
    ...
    try{
      ...
      localStorage.bb_spotify_preflight_ok_at = String(Date.now());
      localStorage.bb_spotify_preflight_room = String(currentRoomCode || '');

      // V104: de Spotify-test is alleen een test. Start nooit automatisch een categorie/ronde.
      // Iedereen moet na de test opnieuw READY klikken.
      try{
        const psSnap = await db.ref('rooms/'+currentRoomCode+'/players').once('value');
        const ps = psSnap.val() || {};
        const up = {};
        Object.keys(ps).forEach(pid => up[`rooms/${currentRoomCode}/players/${pid}/ready`] = false);
        up[`rooms/${currentRoomCode}/currentRound`] = null;
        up[`rooms/${currentRoomCode}/gameReadyMode`] = 'postTestReady';
        up[`rooms/${currentRoomCode}/gameTestOkAt`] = firebase.database.ServerValue.TIMESTAMP;
        await db.ref().update(up);
      }catch(_e){}

      await publishTestStateV99('done', { track: testTrack.name || '', artist: testTrack.artists || '' });
      // Voorkom dat spelers bij opnieuw openen nog een oude "Spotify werkt" pop-up zien.
      setTimeout(()=>{ try{ publishTestStateV99('idle', {}); }catch(_e){} }, 2600);
      setTestStatus('✅ Test geslaagd. Laat iedereen nu op READY klikken.', true);
...
/* =========================
   V103 - Ready na elke ronde + spelerkaarten bekijken + robuuste testpopup
   ========================= */
(function(){
  const q = id => document.getElementById(id);
  const E = s => (typeof esc === 'function' ? esc(s) : String(s ?? '').replace(/[&<>"']/g,m=>({"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;","'":"&#039;"}[m])));
  const HEX2 = {yellow:'#FFCC33', pink:'#00D4C7', purple:'#FF8A1F', blue:'#7ED957', green:'#FF5A5F', free:'#101716'};
  const initial2 = name => String(name||'?').trim().slice(0,1).toUpperCase() || '?';

  function getRoomCodeV103(){
    return String(currentRoomCode || new URLSearchParams(location.search).get('room') || localStorage.hb_player_room || '').toUpperCase();
  }

  // Extra robuuste spelers-popup voor de Spotify-test.
  function ensureTestOverlayV103(){
    let o = q('bbPlayerTestOverlay');
    if(!o){
      o = document.createElement('div');
      o.id = 'bbPlayerTestOverlay';
    ...
    o.innerHTML = `<div class="bbPlayerTestCard v103">
      <img src="bb_logo_gold.png" alt="Bingo Beats" class="bbPlayerTestLogo">
      <div class="bbPlayerTestTitle">HOREN WE IETS?!</div>
      <div class="bbPlayerTestText">Testlied speelt nu.<br><strong>Check even of Spotify geluid geeft 🎵</strong></div>
    </div>`;
    return o;
  }
  function showTestOverlayV103(test){
    const o = ensureTestOverlayV103();
    const title = o.querySelector('.bbPlayerTestTitle');
    const text = o.querySelector('.bbPlayerTestText');
    if(test && test.status === 'playing'){
      title.textContent = 'HOREN WE IETS?!';
      text.innerHTML = 'Testlied speelt nu.<br><strong>Check even of Spotify geluid geeft 🎵</strong>';
      o.classList.remove('hidden');
      requestAnimationFrame(()=>o.classList.add('show'));
      return;
    }
    if(test && test.status === 'done'){
      title.textContent = 'TOP, SPOTIFY WERKT!';
      text.textContent = 'De host kan het spel starten.';
      o.classList.remove('hidden');
      requestAnimationFrame(()=>o.classList.add('show'));
      setTimeout(()=>{ o.classList.remove('show'); setTimeout(()=>o.classList.add('hidden'),250); }, 2200);
      return;
    }
    if(test && test.status === 'error'){
      title.textContent = 'OEPS, GEEN MUZIEK';
      text.textContent = 'De host lost Spotify even op.';
      o.classList.remove('hidden');
      requestAnimationFrame(()=>o.classList.add('show'));
      setTimeout(()=>{ o.classList.remove('show'); setTimeout(()=>o.classList.add('hidden'),250); }, 3600);
      return;
    }
    o.classList.remove('show');
    setTimeout(()=>o.classList.add('hidden'),250);
  }
  let testRoom='', testCb=null;
  function attachTestListenerV103(){
    try{
      if(!db) return;
      const room = getRoomCodeV103();
      if(!room) return;
      if(room === testRoom && testCb) return;
      if(testRoom && testCb) db.ref('rooms/'+testRoom+'/spotifyTest').off('value', testCb);
      testRoom = room;
      testCb = s => showTestOverlayV103(s.val() || null);
      db.ref('rooms/'+room+'/spotifyTest').on('value', testCb);
    }catch(e){}
  }
  setInterval(attachTestListenerV103, 500);
  setTimeout(attachTestListenerV103, 100);
  if(typeof listenPlayer === 'function' && !window.__listenPlayerV103Wrapped){
    const old = listenPlayer;
    window.__listenPlayerV103Wrapped = true;
    listenPlayer = function(){ const ret = old.apply(this, arguments); setTimeout(attachTestListenerV103, 50); return ret; };
  }

  function playerCardHTMLV103(p,{large=false,pick=false,r=null}={}){
    const card = p?.card || [];
    ...
    const cells = Array.from({length:25},(_,i)=>{
      ...
      const isFree = key === 'free';
      const isMarked = !!marked[i] || isFree;
      const canPick = pick && r && key===r.colorKey && !isMarked && !isFree;
      const cls = ['bbStageCell','cell-'+key,isFree?'free':'',isMarked?'marked':'',canPick?'pickable':''].filter(Boolean).join(' ');
      const content = isFree ? '🐵' : (isMarked ? '<img src="bb_logo_gold.png" class="bbCellMiniLogo" alt="BB">' : '');
      return `<button class="${cls}" data-i="${i}" style="--cell:${HEX2[key]||'#777'};background:${HEX2[key]||'#777'}" ${canPick?'':'tabindex="-1"'}>${content}</button>`;
    }).join('');
    return `<div class="bbStageBingo ${large?'large':''}">${cells}</div>`;
  }

  function readyListV103(room){
    const ps = Object.entries(room.players || {});
    // Niet-ready bovenaan, ready onderaan.
    ps.sort((a,b)=>Number(!!a[1].ready)-Number(!!b[1].ready));
    return `<div class="bbStageReadyList">${ps.map(([pid,p])=>`
      <button type="button" class="bbReadyRow ${p.ready?'ready':'wait'}" data-player-card="${E(pid)}">
        <div class="bbAvatar">${E(initial2(p.name))}</div>
        <div class="bbReadyName">${E(p.name||'Speler')}${pid===currentPlayerId?' <span>jij</span>':''}</div>
        <div class="bbReadyState">${p.ready?'Klaar':'Wacht'}</div>
      </button>`).join('') || '<div class="small">Nog geen spelers.</div>'}</div>`;
  }

  function openPlayerCardModalV103(room,pid){
    const p = room.players?.[pid];
    if(!p) return;
    let o = q('bbOtherCardModal');
    if(!o){
      o = document.createElement('div');
      o.id = 'bbOtherCardModal';
      o.className = 'bbOtherCardModal hidden';
      document.body.appendChild(o);
    }
    o.innerHTML = `<div class="bbOtherCardBox">
      <button type="button" class="bbOtherClose">×</button>
      <h2>${E(p.name||'Speler')}</h2>
      <p>bingokaart</p>
      ${playerCardHTMLV103(p,{large:false})}
    </div>`;
    o.classList.remove('hidden');
    requestAnimationFrame(()=>o.classList.add('show'));
    o.querySelector('.bbOtherClose')?.addEventListener('click',()=>{ o.classList.remove('show'); setTimeout(()=>o.classList.add('hidden'),220); });
    o.addEventListener('click',ev=>{ if(ev.target===o){ o.classList.remove('show'); setTimeout(()=>o.classList.add('hidden'),220); } }, {once:true});
  }

  function setMeReadyV103(){
    if(!currentRoomCode || !currentPlayerId) return;
    db.ref('rooms/'+currentRoomCode+'/players/'+currentPlayerId+'/ready').set(true);
  }

  function renderLobbyV103(root,room,{afterRound=false}={}){
    const me = room.players?.[currentPlayerId] || {};
    root.className = 'compactDashboard bbStageDashboard stageLobby';
    root.innerHTML = `<section class="bbStageShell">
      <h2>${afterRound?'Volgende ronde':'Wachten op de host'}</h2>
      <p class="bbStageSub">${afterRound?'Klik op READY als je klaar bent.':'Iedereen klaar? Dan kan de ronde starten.'}</p>
      ${readyListV103(room)}
      <div class="bbCardWrap"><h3>Jouw bingokaart</h3>${playerCardHTMLV103(me,{large:false})}</div>
      <button type="button" id="bbPlayerReadyBtn" class="bbReadyBig ${me.ready?'isReady':''}" ${me.ready?'disabled':''}>${me.ready?'READY ✓':'READY'}</button>
    </section>`;
    root.querySelector('#bbPlayerReadyBtn')?.addEventListener('click',setMeReadyV103);
    root.querySelectorAll('[data-player-card]').forEach(btn=>btn.addEventListener('click',()=>openPlayerCardModalV103(room,btn.dataset.playerCard)));
  }

  function renderPickCardV103(root,room,r){
    const me = room.players?.[currentPlayerId] || {};
    root.className = 'compactDashboard bbStageDashboard stagePickCard';
    root.innerHTML = `<section class="bbStageShell">
      <h2>GOED ANTWOORD!</h2>
      <p class="bbStageSub">Kies één <strong>${E(r.colorName||'')}</strong> vakje</p>
      ${playerCardHTMLV103(me,{large:true,pick:true,r})}
      <p class="bbStageSub bottom">Tik op een opgelicht vakje</p>
    </section>`;
    root.querySelectorAll('.bbStageCell.pickable').forEach(btn=>{
      btn.addEventListener('click',()=> pickCell(Number(btn.dataset.i)) );
  ...
  renderCompactDashboard = function(room,r){
    ...
    if(!root) return;
    document.body.classList.add('bbStageMode');
    if(!r?.id) return renderLobbyV103(root,room,{afterRound:false});
    if(r.status === 'judged'){
      const good = room.correct?.[r.id]?.[currentPlayerId] === true;
      const me = room.players?.[currentPlayerId] || {};
      const picked = me.lastPickedRound === r.id;
      if(good && !picked) return renderPickCardV103(root,room,r);
      return renderLobbyV103(root,room,{afterRound:true});
    }
    return prevRender ? prevRender(room,r) : renderLobbyV103(root,room,{afterRound:false});
  };

  // Na beoordeling is niemand automatisch klaar. Iedereen moet zelf READY drukken.
  ...
  // Na vakje kiezen ook niet automatisch ready; eerst terug naar lobby, dan READY.
  if(typeof pickCell === 'function'){
    pickCell = function(i){
      db.ref('rooms/'+currentRoomCode+'/players/'+currentPlayerId).once('value').then(s=>{
        ...
        return db.ref('rooms/'+currentRoomCode+'/players/'+currentPlayerId).update({marked,bingo,lastPickedRound:activeRound.id,ready:false}).then(()=>{
  ...
  // Host: zodra iedereen READY is na een beoordeelde ronde, automatisch volgende ronde starten.
  let autoStartingRound = '';
  async function hostAutoNextV103(){
    try{
      if(!currentRoomCode || document.body.classList.contains('playerMode')) return;
      const s = await db.ref('rooms/'+currentRoomCode).once('value');
      const room = s.val() || {};
      const r = room.currentRound || {};
      if(r.status !== 'judged' || !r.id) return;
      if(room.bingos && Object.keys(room.bingos).length) return;
      const ps = Object.values(room.players || {});
      if(!ps.length || !ps.every(p=>p.ready)) return;
      if(autoStartingRound === r.id) return;
      autoStartingRound = r.id;
      if(q('hostStatus')) q('hostStatus').textContent = 'Iedereen READY. Volgende ronde start automatisch...';
      setTimeout(()=>{ try{ startRound(); }catch(e){ autoStartingRound=''; } }, 900);
    }catch(e){}
  }
  setInterval(hostAutoNextV103, 1300);
})();


/* =========================
   V104 - Test blijft los van spel + READY na test + auto start bij iedereen ready
   - Na TEST VOOR JE BEGINT wordt currentRound leeggemaakt
   - Alle spelers moeten opnieuw op READY klikken
   - Pas wanneer iedereen READY is start de eerste ronde automatisch
   - Test-pop-up is ook zichtbaar als groot spelersscherm, niet alleen overlay
   ========================= */
(function(){
  const q = id => document.getElementById(id);
  const E = s => (typeof esc === 'function' ? esc(s) : String(s ?? '').replace(/[&<>"']/g,m=>({"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;","'":"&#039;"}[m])));

  function getRoomV104(){
    return (currentRoomCode || new URLSearchParams(location.search).get('room') || localStorage.hb_player_room || '').toUpperCase();
  }

  function isFreshTestV104(test, maxMs){
    const at = Number(test && test.at || 0);
    return !!at && (Date.now() - at < maxMs);
  }

  function renderTestScreenV104(root, test){
    document.body.classList.add('bbStageMode');
    root.className = 'compactDashboard bbStageDashboard stageTestMusic';
    let title = 'HOREN WE IETS?!';
    let sub = 'De host test Spotify met een kort testliedje.';
    if(test?.status === 'done'){
      title = 'TOP, SPOTIFY WERKT!';
      sub = 'Klik zo op READY als jij het testlied hebt gehoord.';
    }
    if(test?.status === 'error'){
      title = 'OEPS, GEEN MUZIEK';
      sub = 'De host lost Spotify even op.';
    }
    root.innerHTML = `<section class="bbStageShell bbTestStageShell">
      <img src="bb_logo.png" class="bbStageLogo" alt="Bingo Beats">
      <div class="bbTestBigIcon">🎵</div>
      <h2>${E(title)}</h2>
      <p class="bbStageSub">${E(sub)}</p>
      <div class="bbTestPulse">Testlied speelt...</div>
    </section>`;
  }

  // Laat test op spelersscherm groot zien. Na de test valt hij terug naar lobby/READY.
  const prevRenderV104 = typeof renderCompactDashboard === 'function' ? renderCompactDashboard : null;
  if(prevRenderV104 && !window.__renderCompactV104Wrapped){
    window.__renderCompactV104Wrapped = true;
    renderCompactDashboard = function(room, r){
      const test = room && room.spotifyTest;
      const root = q('screenDashboard');
      if(root && test && (test.status === 'playing' || (test.status === 'done' && isFreshTestV104(test, 2600)) || (test.status === 'error' && isFreshTestV104(test, 5000)))){
        return renderTestScreenV104(root, test);
      }
      return prevRenderV104(room, r);
    };
  }

  // Verberg oude/stale test-popups die door eerdere listeners nog binnenkomen.
  setInterval(()=>{
    try{
      const o = q('bbPlayerTestOverlay');
      if(!o || o.classList.contains('hidden')) return;
      const txt = (o.textContent || '').toLowerCase();
      // Succes-pop-up is maar kort bedoeld. Blijft hij hangen of is er geen test bezig, verberg hem.
      if(txt.includes('top, spotify werkt') || txt.includes('spotify werkt!')){
        setTimeout(()=>{ o.classList.remove('show'); o.classList.add('hidden'); }, 1200);
      }
    }catch(_e){}
  }, 1000);

  // Na test: start de eerste ronde automatisch zodra iedereen READY is.
  let autoStartFirstKeyV104 = '';
  async function hostAutoStartAfterTestV104(){
    try{
      if(!currentRoomCode || document.body.classList.contains('playerMode')) return;
      const snap = await db.ref('rooms/'+currentRoomCode).once('value');
      const room = snap.val() || {};
      const r = room.currentRound || {};
      if(r && r.id) return;
      if(room.gameReadyMode !== 'postTestReady') return;
      const ps = Object.values(room.players || {});
      if(!ps.length || !ps.every(p => p.ready)) return;
      const key = currentRoomCode + ':' + (room.gameTestOkAt || 'ok');
      if(autoStartFirstKeyV104 === key) return;
      autoStartFirstKeyV104 = key;
      if(q('hostStatus')) q('hostStatus').textContent = 'Iedereen READY. Eerste ronde start automatisch...';
      setTimeout(()=>{ try{ startRound(); }catch(_e){ autoStartFirstKeyV104=''; } }, 900);
    }catch(_e){}
  }
  setInterval(hostAutoStartAfterTestV104, 1000);

  // Bij start ronde: laat gameReadyMode niet nog eens de eerste ronde triggeren.
  if(typeof startRound === 'function' && !window.__startRoundV104Wrapped){
    const oldStartV104 = startRound;
    window.__startRoundV104Wrapped = true;
    startRound = async function(){
      try{
        if(currentRoomCode){
          await db.ref('rooms/'+currentRoomCode+'/gameReadyMode').set(null);
        }
      }catch(_e){}
      return oldStartV104.apply(this, arguments);
    };
  }
})();
