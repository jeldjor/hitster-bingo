...
function restoreHost(){
 let room=localStorage.hb_host_room;
 if(!room || new URLSearchParams(location.search).get("room")) return;
...
Annuleren = Nieuwe kamer`
   ...
   if(ok){
      ...
      if(status) status.textContent="Kamer hervat. Klaar voor nieuwe ronde.";

      const answer=$("hostAnswerArea");
      if(answer) answer.innerHTML="";

      const round=$("hostRoundInfo");
