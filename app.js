...
/* ==================================================
   V47 SCHONE CATEGORIE OPSLAG
   Enige bron: bb_clean_cat_<id>
   Oude keys worden NIET meer gelezen.
   Categorieën blijven staan tot de host zelf iets anders invult.
   ================================================== */
(function(){
  const fields = ["cat-yellow","cat-pink","cat-purple","cat-blue","cat-green"];
  const key = id => "bb_clean_cat_" + id;
  ...
  function saveOne(id){
    ...
    // Zorg dat form reset dezelfde waarde gebruikt.
    el.defaultValue = el.value || "";
    el.setAttribute("value", el.value || "");
  }

  function saveAll(){
    fields.forEach(saveOne);
  }
  ...
  function loadAll(){
    ...
    fields.forEach(id=>{
      ...
      if(saved !== null){
        ...
        // Eerste keer leeg of huidige waarde opslaan.
        localStorage.setItem(key(id), el.value || "");
        el.defaultValue = el.value || "";
        el.setAttribute("value", el.value || "");
      }
  ...
  function bind(){
    fields.forEach(id=>{
      const el = get(id);
      if(!el || el.dataset.bbCleanCatBound === "1") return;
      el.dataset.bbCleanCatBound = "1";

      ["input","change","blur","keyup"].forEach(ev=>{
        el.addEventListener(ev,()=>saveOne(id),true);
      });

      el.addEventListener("paste",()=>setTimeout(()=>saveOne(id),0),true);

      if(el.form && el.form.dataset.bbCleanCatForm !== "1"){
        el.form.dataset.bbCleanCatForm = "1";
  ...
  function patchButtons(){
    document.querySelectorAll("button,a").forEach(btn=>{
      if(btn.dataset.bbCleanCatClick === "1") return;
      btn.dataset.bbCleanCatClick = "1";
  ...
  document.addEventListener("DOMContentLoaded",()=>{
    init();
    if(!window.__bbCleanCatObserver){
      window.__bbCleanCatObserver = true;
...
/* V48 Firebase categorie opslag */
(function(){
 const IDS=["cat-yellow","cat-pink","cat-purple","cat-blue","cat-green"];

 function roomPath(){
   return "globalSettings/categories";
 }
 ...
 function saveCategories(){
   ...
   db.ref(roomPath()).set(data);
   alert("Categorieën opgeslagen");
 }

 function loadCategories(){
   if(typeof db==="undefined") return;
   db.ref(roomPath()).once("value").then(snap=>{
      const data=snap.val()||{};
      IDS.forEach(id=>{
         const el=document.getElementById(id);
 ...
 document.addEventListener("DOMContentLoaded",()=>{
    ...
    if(btn) btn.addEventListener("click", saveCategories);
    setTimeout(loadCategories,500);
 });
})();


/* V49 Firebase categorie opslag - knop zichtbaar/binding robuust */
(function(){
  const IDS=["cat-yellow","cat-pink","cat-purple","cat-blue","cat-green"];
  ...
  function saveCategoriesV49(){
    if(typeof db==="undefined"){
      alert("Firebase is nog niet geladen.");
      return;
    }
    ...
    db.ref(path()).set(data).then(()=>{
      ...
      if(st) st.textContent="Categorieën opgeslagen ✅";
    }).catch(err=>{
      alert("Opslaan mislukt: " + (err.message || err));
    });
  }

  function loadCategoriesV49(){
    if(typeof db==="undefined") return;
  ...
  function bindSaveButtonV49(){
    const btn=document.getElementById("saveCategoriesBtn");
    if(btn && btn.dataset.v49Bound!=="1"){
      btn.dataset.v49Bound="1";
      btn.addEventListener("click",saveCategoriesV49);
    }
  }

  document.addEventListener("DOMContentLoaded",()=>{
    bindSaveButtonV49();
    setTimeout(loadCategoriesV49,600);
  });

  setTimeout(()=>{bindSaveButtonV49();loadCategoriesV49();},500);
  setTimeout(bindSaveButtonV49,1500);
})();
