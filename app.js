...
const COLORS=[{key:"yellow",name:"GOUD",emoji:"🟡",input:"cat-yellow",hex:"#FFCC33",target:"gold"},{key:"pink",name:"AQUA",emoji:"🩵",input:"cat-pink",hex:"#00D4C7",target:"aqua"},{key:"purple",name:"ORANJE",emoji:"🟠",input:"cat-purple",hex:"#FF8A1F",target:"orange"},{key:"blue",name:"LIME",emoji:"🟢",input:"cat-blue",hex:"#7ED957",target:"lime"},{key:"green",name:"KORAAL",emoji:"🔴",input:"cat-green",hex:"#FF5A5F",target:"coral"}];
  ...
  box.innerHTML = `
    <div style="display:grid;grid-template-columns:1fr 126px;gap:14px;align-items:center">
      <div>
        ...
        <div style="font-size:42px;font-weight:900;color:#73ff73;letter-spacing:3px;line-height:1">${esc(code)}</div>
...
function hbHostPickerPopupPickingV7I(mode){
  const inner = typeof sharedPickerHTML === "function"
    ? sharedPickerHTML(mode)
    : (typeof pickerHTML === "function" ? pickerHTML(mode) : `<div class="discoIcon">🪩</div><div class="pickerTitle">Discobal kiest...</div>`);
...
function hbShowAnimHTMLV8(mode){
  ...
  return `<div class="showAnimStage"><div class="hostPickerPopupTitle">${hbAnimLabelV8("disco")}</div><div class="showAnimBig">🪩</div><div class="showAnimTitle">Discobal kiest...</div></div>`;
}
...
function hbShowAnimHTMLV8C(mode, compact=false){
  ...
  return `${wrapStart}<div class="hostPickerPopupTitle">${hbAnimLabelV8C("disco")}</div><div class="showDiscoBall">🪩</div><div class="showAnimTitle">Discobal kiest...</div>${wrapEnd}`;
}
...
function hbAnimHTMLV9C(mode, compact=false){
  ...
  return `<div class="${wrap}"><div class="hostPickerPopupTitle">${hbAnimLabelV9C("disco")}</div><div class="showDiscoBall">🪩</div><div class="showAnimTitle">Discobal kiest...</div></div>`;
}
...
if(typeof renderCompactPicker === "function"){
  renderCompactPicker = function(r){
    const area=document.getElementById("dashPickerArea");
    if(!area)return;
    if(!r||!r.id){area.innerHTML=`<div class="dashDisco">🪩</div><div class="dashCatText">Wachten op host...</div>`;return;}
...
/* BINGO BEATS BRAND PATCH */
(function(){
  document.title = "Bingo Beats";
  const h1 = document.querySelector("header h1");
  if(h1) h1.textContent = "🪩 Bingo Beats";
  setInterval(() => {
    const h = document.querySelector("header h1");
    if(h && h.textContent.includes("Hitster")) h.textContent = "🪩 Bingo Beats";
  }, 2000);
})();


/* ==================================================
   BINGO BEATS V2.1 - DIER DRAAIT AAN RAD
   ================================================== */

const BB_ANIMALS_V21 = ["🦜","🐒","🦩","🐢","🐬","🦁","🐯","🐼","🦊","🦥"];

function bbPickAnimalV21(){
  const last = localStorage.getItem("bb_last_animal_v21") || "";
  const options = BB_ANIMALS_V21.filter(a => a !== last);
  const chosen = pick(options.length ? options : BB_ANIMALS_V21);
  localStorage.setItem("bb_last_animal_v21", chosen);
  return chosen;
}

function bbTropicalPickerHTMLV21(animal, compact=false){
  return `
    <div class="tropicalPicker ${compact ? "compact" : ""}">
      <div class="tropicalTitle">${animal} draait het rad!</div>
      <div class="tropicalScene">
        <div class="tropicalPointer"></div>
        <div class="tropicalWheel"></div>
        <div class="tropicalHandle"></div>
        <div class="tropicalAnimal">${animal}</div>
      </div>
      <div class="tropicalSub">Welke tropische kleur wordt het?</div>
    </div>
  `;
}

// Oude pickerfuncties vervangen door dierenrad
pickerHTML = function(mode){
  return bbTropicalPickerHTMLV21(mode || "🦜", false);
};

if(typeof sharedPickerHTML === "function"){
  sharedPickerHTML = function(mode){
    return bbTropicalPickerHTMLV21(mode || "🦜", true);
  };
}

renderPlayerPicker = function(round){
  const animal = round.pickerMode || round.animal || "🦜";
  const area = document.getElementById("playerPickerArea");
  if(area) area.innerHTML = bbTropicalPickerHTMLV21(animal, false);
  const dash = document.getElementById("dashPickerArea");
  if(dash) dash.innerHTML = bbTropicalPickerHTMLV21(animal, true);
};

renderPlayerPickerFixed = renderPlayerPicker;
...
if(typeof renderCompactPicker === "function"){
  renderCompactPicker = function(r){
    ...
    if(r.status === "picking"){
      area.innerHTML = bbTropicalPickerHTMLV21(r.pickerMode || r.animal || "🦜", true);
      return;
    }
...
// Start ronde: altijd dierenrad, elk rondje ander dier
startRoundVisual = function(room){
  document.getElementById("hostAnswerArea").innerHTML = "";
  document.getElementById("playBtn").disabled = true;
  document.getElementById("showAnswerBtn").disabled = true;
  ...
  const animal = bbPickAnimalV21();
  const pickerMarkup = bbTropicalPickerHTMLV21(animal, true);

  document.getElementById("hostPickerArea").innerHTML = bbTropicalPickerHTMLV21(animal, false);

  if(typeof hbHostPickerPopupShowV7I === "function"){
    hbHostPickerPopupShowV7I(bbTropicalPickerHTMLV21(animal, false));
  }
  ...
  setTimeout(() => {
    ...
    if(typeof hbHostPickerPopupShowV7I === "function"){
      hbHostPickerPopupShowV7I(`
        <div class="showAnimStage">
          <div class="hostPickerPopupTitle">${animal} koos:</div>
          <div class="showResultColor">${esc(color.emoji || "")}</div>
          <div class="showResultName">${esc(color.name || "")}</div>
          <div class="showResultCat">${esc(cat || "Geen categorie")}</div>
          <div class="hostPickerPopupSub">Maak je klaar om het nummer te starten</div>
        </div>
      `);
    }
    ...
    if(typeof hbHostPickerPopupHideV7I === "function"){
      setTimeout(hbHostPickerPopupHideV7I, 3000);
    }
  }, 3500);
};
...
/* ==================================================
   BINGO BEATS V2.2 - TROPISCH EILAND DIER KIEST KLEUR
   ================================================== */

const BB_TROPICAL_ANIMALS_V22 = [
  {emoji:"🦜", name:"Papegaai", cls:"parrot"},
  {emoji:"🐒", name:"Aap", cls:"monkey"},
  {emoji:"🦩", name:"Flamingo", cls:"flamingo"},
  {emoji:"🐢", name:"Schildpad", cls:"turtle"},
  {emoji:"🐬", name:"Dolfijn", cls:"dolphin"},
  {emoji:"🦁", name:"Leeuw", cls:"run"},
  {emoji:"🐯", name:"Tijger", cls:"run"},
  {emoji:"🦊", name:"Vos", cls:"run"}
];

function bbPickAnimalV22(){
  const last = localStorage.getItem("bb_last_animal_v22") || "";
  const options = BB_TROPICAL_ANIMALS_V22.filter(a => a.emoji !== last);
  const chosen = pick(options.length ? options : BB_TROPICAL_ANIMALS_V22);
  localStorage.setItem("bb_last_animal_v22", chosen.emoji);
  return chosen;
}

function bbAnimalByEmojiV22(emoji){
  return BB_TROPICAL_ANIMALS_V22.find(a => a.emoji === emoji) || BB_TROPICAL_ANIMALS_V22[0];
}

function bbTropicalIslandHTMLV22(animalEmoji, compact=false){
  const a = bbAnimalByEmojiV22(animalEmoji || "🦜");
  return `
    <div class="tropicalIslandPicker ${compact ? "compact" : ""}">
      <div class="islandTitle">${a.emoji} ${a.name} kiest de kleur</div>
      <div class="islandStage">
        <div class="islandSun"></div>
        <div class="islandPalm">🌴</div>
        <div class="islandTrail"></div>
        <div class="colorTargets">
          <div class="colorTarget gold"></div>
          <div class="colorTarget aqua"></div>
          <div class="colorTarget orange"></div>
          <div class="colorTarget lime"></div>
          <div class="colorTarget coral"></div>
        </div>
        <div class="islandAnimal ${a.cls}">${a.emoji}</div>
      </div>
      <div class="islandHint">Welk kleurvak wordt gekozen?</div>
    </div>
  `;
}

// Vervang oude picker door tropische dieren-keuze
pickerHTML = function(mode){
  return bbTropicalIslandHTMLV22(mode || "🦜", false);
};

if(typeof sharedPickerHTML === "function"){
  sharedPickerHTML = function(mode){
    return bbTropicalIslandHTMLV22(mode || "🦜", true);
  };
}

renderPlayerPicker = function(round){
  const animal = round.animal || round.pickerMode || "🦜";
  const area = document.getElementById("playerPickerArea");
  if(area) area.innerHTML = bbTropicalIslandHTMLV22(animal, false);

  const dash = document.getElementById("dashPickerArea");
  if(dash) dash.innerHTML = bbTropicalIslandHTMLV22(animal, true);
};
...
if(typeof renderCompactPicker === "function"){
  renderCompactPicker = function(r){
    ...
    if(r.status === "picking"){
      area.innerHTML = bbTropicalIslandHTMLV22(r.animal || r.pickerMode || "🦜", true);
      return;
    }
...
// Start ronde: 5 seconden eilandanimatie
startRoundVisual = function(room){
  document.getElementById("hostAnswerArea").innerHTML = "";
  document.getElementById("playBtn").disabled = true;
  document.getElementById("showAnswerBtn").disabled = true;
  ...
  const animal = bbPickAnimalV22();
  const pickerMarkup = bbTropicalIslandHTMLV22(animal.emoji, true);
  const hostMarkup = bbTropicalIslandHTMLV22(animal.emoji, false);

  document.getElementById("hostPickerArea").innerHTML = hostMarkup;

  if(typeof hbHostPickerPopupShowV7I === "function"){
    hbHostPickerPopupShowV7I(hostMarkup);
  }
  ...
  setTimeout(() => {
    ...
    if(typeof hbHostPickerPopupShowV7I === "function"){
      hbHostPickerPopupShowV7I(`
        <div class="showAnimStage">
          <div class="hostPickerPopupTitle">${animal.emoji} ${animal.name} koos:</div>
          <div class="showResultColor">${esc(color.emoji || "")}</div>
          <div class="showResultName">${esc(color.name || "")}</div>
          <div class="showResultCat">${esc(cat || "Geen categorie")}</div>
          <div class="hostPickerPopupSub">Maak je klaar om het nummer te starten</div>
...
/* V23 FORCE TROPICAL */
window.BB_FORCE_TROPICAL = true;
localStorage.setItem("bb_force_tropical","1");


/* ==================================================
   BINGO BEATS V25 CLEAN LOGIC
   Kleur eerst kiezen, dier naar exact die kleur.
   Bingokaarten op echte kleur-key.
   ================================================== */

const BB_TARGETS_V25 = {
  yellow:{cls:"gold", left:"18%", emoji:"🟡", name:"GOUD"},
  pink:{cls:"aqua", left:"34%", emoji:"🩵", name:"AQUA"},
  purple:{cls:"orange", left:"50%", emoji:"🟠", name:"ORANJE"},
  blue:{cls:"lime", left:"66%", emoji:"🟢", name:"LIME"},
  green:{cls:"coral", left:"82%", emoji:"🔴", name:"KORAAL"}
};

const BB_ANIMALS_V25 = [
  {emoji:"🦜", name:"Papegaai", cls:"parrot"},
  {emoji:"🐒", name:"Aap", cls:"monkey"},
  {emoji:"🦩", name:"Flamingo", cls:"flamingo"},
  {emoji:"🐢", name:"Schildpad", cls:"turtle"},
...
function bbApplyLabelsV25(){
  [["cat-yellow","🟡 Goud"],["cat-pink","🩵 Aqua"],["cat-purple","🟠 Oranje"],["cat-blue","🟢 Lime"],["cat-green","🔴 Koraal"]].forEach(([id,text])=>{
    const input=document.getElementById(id);
    if(!input)return;
    const label=input.closest("label");
    if(!label)return;
    label.setAttribute("data-bb-label",text);
    [...label.childNodes].forEach(n=>{
      if(n.nodeType===Node.TEXT_NODE && n.textContent.trim()) n.textContent=text;
    });
  });
}
setTimeout(bbApplyLabelsV25,100);
setTimeout(bbApplyLabelsV25,800);
setInterval(bbApplyLabelsV25,2000);

function bbPickAnimalV25(){
  const last=localStorage.getItem("bb_last_animal_v25")||"";
  const options=BB_ANIMALS_V25.filter(a=>a.emoji!==last);
  const chosen=pick(options.length?options:BB_ANIMALS_V25);
  localStorage.setItem("bb_last_animal_v25",chosen.emoji);
  return chosen;
}

function bbAnimalV25(emoji){
  return BB_ANIMALS_V25.find(a=>a.emoji===emoji)||BB_ANIMALS_V25[0];
}

function bbTargetV25(key){
  return BB_TARGETS_V25[key]||BB_TARGETS_V25.yellow;
}

function bbIslandHTMLV25(animalEmoji,colorKey,compact=false){
  const a=bbAnimalV25(animalEmoji||"🦜");
  const t=bbTargetV25(colorKey||"yellow");
  const targets=["yellow","pink","purple","blue","green"].map(k=>{
    const x=bbTargetV25(k);
    return `<div class="v25Target ${x.cls} ${k===colorKey?"chosen":""}"></div>`;
  }).join("");

  return `
    <div class="v25IslandPicker ${compact?"compact":""}">
      <div class="v25Title">${a.emoji} ${a.name} kiest ${t.emoji}</div>
      <div class="v25Stage" style="--target-left:${t.left}">
        <div class="v25Sun"></div>
        <div class="v25Palm">🌴</div>
        <div class="v25Targets">${targets}</div>
        <div class="v25Animal ${a.cls}">${a.emoji}</div>
      </div>
      <div class="v25Hint">Het dier loopt naar de gekozen kleur...</div>
    </div>`;
}

/* Render card override: echte kleur-key classes */
...
/* Compact card override indien aanwezig */
function bbRenderCompactCardV25(el,card,marked,click=false,r=null){
  if(!el)return;
  el.innerHTML=(card||[]).map((c,i)=>{
    const m=marked&&marked[i];
    const pickable=click&&r&&c===r.colorKey&&c!=="free"&&!m;
...
/* Als oude compact renderfuncties bestaan, vervang ze voorzichtig */
if(typeof renderCompactCard === "function"){
  renderCompactCard = function(idOrEl,card,marked,click=false,r=null){
    const el = typeof idOrEl === "string" ? document.getElementById(idOrEl) : idOrEl;
    bbRenderCompactCardV25(el,card,marked,click,r);
  };
}

if(typeof renderDashCard === "function"){
  renderDashCard = function(room,r){
    const me=room.players?.[currentPlayerId]||{};
    const good=room.correct?.[r.id]?.[currentPlayerId]===true;
    const picked=me.lastPickedRound===r.id;
    const canPick=r.status==="judged"&&good&&!picked;
    const hint=document.getElementById("dashCardHint");
    const status=document.getElementById("dashCardStatus");
    if(hint) hint.textContent=canPick?`Kies 1 ${r.colorEmoji} ${r.colorName} vakje.`:"Wachten op beoordeling...";
    if(status) status.textContent=canPick?"Tik op een geldig vakje.":(picked?"Vakje gekozen":"Nog geen vakje gekozen");
    bbRenderCompactCardV25(document.getElementById("dashOwnCard"),me.card||[],me.marked||{},canPick,r);
  };
}

/* Picker render */
pickerHTML = function(mode){
  // mode kan dier zijn; kleur volgt via active round meestal.
  return bbIslandHTMLV25(mode||"🦜","yellow",false);
};

if(typeof sharedPickerHTML === "function"){
  sharedPickerHTML = function(mode){
    return bbIslandHTMLV25(mode||"🦜","yellow",true);
  };
}
...
renderPlayerPicker = function(round){
  ...
  const area=document.getElementById("playerPickerArea");
  if(area) area.innerHTML=bbIslandHTMLV25(animal,key,false);
  const dash=document.getElementById("dashPickerArea");
  if(dash) dash.innerHTML=bbIslandHTMLV25(animal,key,true);
};
renderPlayerPickerFixed = renderPlayerPicker;
...
if(typeof renderCompactPicker === "function"){
  renderCompactPicker = function(r){
    ...
    if(r.status==="picking"){
      area.innerHTML=bbIslandHTMLV25(r.animal||r.pickerMode||"🦜",r.colorKey||r.targetColorKey||"yellow",true);
      return;
    }
    area.innerHTML=`<div class="dashColorBig">${esc(r.colorEmoji||"🌴")}</div><div class="dashColorText">${esc(r.colorName||"KLEUR")}</div><div class="dashCatText">${esc(r.category||"Wachten...")}</div>`;
  };
...
/* Start ronde: KLEUR EERST, dan animatie naar die kleur */
startRoundVisual = function(room){
  document.getElementById("hostAnswerArea").innerHTML="";
  document.getElementById("playBtn").disabled=true;
  document.getElementById("showAnswerBtn").disabled=true;
  ...
  const animal=bbPickAnimalV25();
  const color=pick(COLORS);
  const cat=document.getElementById(color.input).value||"Geen categorie";
  const pickerMarkup=bbIslandHTMLV25(animal.emoji,color.key,true);
  const hostMarkup=bbIslandHTMLV25(animal.emoji,color.key,false);
...
/* ==================================================
   V27 INLINE BINGOKAART KLEUREN - HARD FIX
   Kleurt bestaande en nieuwe kaartvakjes direct via JS.
   ================================================== */

const BB_CARD_COLORS_V27 = {
  yellow:"#FFCC33",
  pink:"#00D4C7",
  purple:"#FF8A1F",
  blue:"#7ED957",
...
function bbCellTextV27(k, marked){
  if(marked) return "✅";
  return ({yellow:"🟡",pink:"🩵",purple:"🟠",blue:"🟢",green:"🔴",free:"👑"}[k] || "⬜");
}

function bbPaintCellV27(cell,k,marked=false,pickable=false){
  const color = BB_CARD_COLORS_V27[k] || "#333";
  cell.classList.add("bbColoredCell","cell-"+k);
  if(marked) cell.classList.add("marked");
  if(pickable) cell.classList.add("pickable","pickableCell");

  cell.style.setProperty("background", k==="free"
    ? "linear-gradient(135deg,#fff7c8,#FFCC33)"
    : `linear-gradient(145deg,${color},${color}99)`, "important");
  cell.style.setProperty("border", "1px solid rgba(255,255,255,.55)", "important");
  cell.style.setProperty("box-shadow", marked
    ? "0 0 0 2px #fff inset,0 0 24px rgba(255,204,51,.85)"
    : "inset 0 0 12px rgba(0,0,0,.22),0 0 10px rgba(0,0,0,.18)", "important");
  cell.style.setProperty("color", k==="free" || marked ? "#171300" : "#fff", "important");
  cell.style.setProperty("text-shadow", k==="free" || marked ? "none" : "0 1px 2px rgba(0,0,0,.6)", "important");
  cell.style.setProperty("opacity", "1", "important");
}

function bbRenderCardHTMLV27(card,marked,compact=false,click=false,r=null){
  return (card||[]).map((k,i)=>{
    const m = !!(marked && marked[i]);
    const pickable = !!(click && r && k===r.colorKey && k!=="free" && !m);
    const blocked = !!(click && !pickable && k!=="free" && !m);
    const cls = compact ? "compactCell" : "bingoCell";
    const bg = k==="free"
      ? "linear-gradient(135deg,#fff7c8,#FFCC33)"
      : `linear-gradient(145deg,${BB_CARD_COLORS_V27[k]||"#333"},${BB_CARD_COLORS_V27[k]||"#333"}99)`;
    return `<div class="${cls} bbColoredCell cell-${k} ${k==="free"?"free":""} ${m?"marked":""} ${pickable?(compact?"pickable":"pickableCell"):""} ${blocked?"blockedCell":""}" data-i="${i}" data-color-key="${k}" style="background:${bg}!important;border:1px solid rgba(255,255,255,.55)!important;color:${k==="free"||m?"#171300":"#fff"}!important;">${bbCellTextV27(k,m)}</div>`;
  }).join("");
}

// Override klassieke renderCard
renderCard = function(id,card,marked,click=false,r=null){
  const el = document.getElementById(id);
  if(!el) return;
  el.innerHTML = bbRenderCardHTMLV27(card,marked,false,click,r);
  if(click){
    el.querySelectorAll(".pickableCell").forEach(x=>x.addEventListener("click",()=>pickCell(Number(x.dataset.i))));
  }
  setTimeout(()=>bbForcePaintVisibleCardsV27(),50);
};

// Override compact card renderer
function bbRenderCompactCardV27(el,card,marked,click=false,r=null){
  if(!el) return;
  el.innerHTML = bbRenderCardHTMLV27(card,marked,true,click,r);
  if(click){
    el.querySelectorAll(".pickable").forEach(x=>x.addEventListener("click",()=>pickCell(Number(x.dataset.i))));
  }
  setTimeout(()=>bbForcePaintVisibleCardsV27(),50);
}

// Override / patch dashboard card rendering
function bbRenderDashCardV27(room,r){
  const me = room.players?.[currentPlayerId] || {};
  const good = room.correct?.[r.id]?.[currentPlayerId] === true;
  const picked = me.lastPickedRound === r.id;
  const canPick = r.status==="judged" && good && !picked;
  ...
  const hint = document.getElementById("dashCardHint");
  const status = document.getElementById("dashCardStatus");
  if(hint) hint.textContent = canPick ? `Kies 1 ${r.colorEmoji} ${r.colorName} vakje.` : "Jouw bingokaart";
  if(status) status.textContent = canPick ? "Tik op een geldig vakje." : (picked ? "Vakje gekozen" : "READY");

  bbRenderCompactCardV27(document.getElementById("dashOwnCard"), me.card || [], me.marked || {}, canPick, r);
}

if(typeof renderDashCard === "function"){
  renderDashCard = bbRenderDashCardV27;
}

if(typeof renderCompactCard === "function"){
  renderCompactCard = function(idOrEl,card,marked,click=false,r=null){
    const el = typeof idOrEl==="string" ? document.getElementById(idOrEl) : idOrEl;
    bbRenderCompactCardV27(el,card,marked,click,r);
  };
}

// Sommige versies bouwen de kaart in renderDashboard/renderCompactDashboard; repaint na iedere Firebase update.
function bbForcePaintVisibleCardsV27(){
  const room = currentRoomCode || localStorage.getItem("hb_player_room") || "";
  if(!room || !currentPlayerId || !db) return;

  db.ref("rooms/"+room+"/players/"+currentPlayerId).once("value").then(s=>{
    const p = s.val() || {};
    const card = p.card || [];
    const marked = p.marked || {};

    const dash = document.getElementById("dashOwnCard");
    if(dash && card.length){
      // Als oude renderer cellen heeft geplaatst zonder data, vervang hele inhoud.
      bbRenderCompactCardV27(dash, card, marked, false, activeRound || {});
    }

    const own = document.getElementById("ownCard");
    if(own && card.length){
      renderCard("ownCard", card, marked, false, activeRound || {});
    }

    const pick = document.getElementById("pickCard");
    if(pick && card.length && activeRound?.id){
      const good = false;
      // pick screen wordt door originele flow opnieuw met click=true gebouwd; hier alleen kleuren afdwingen als fallback.
      pick.querySelectorAll(".bingoCell,.compactCell").forEach((cell,i)=>{
        if(card[i]) bbPaintCellV27(cell, card[i], !!marked[i], cell.classList.contains("pickableCell") || cell.classList.contains("pickable"));
      });
    }
  }).catch(()=>{});
}

// Patch listenPlayer zodat na elke render de kaart opnieuw hard gekleurd wordt
if(typeof listenPlayer === "function" && !window.__bbV27ListenPatch){
  window.__bbV27ListenPatch = true;
  const oldListenPlayerV27 = listenPlayer;
  listenPlayer = function(){
    const out = oldListenPlayerV27.apply(this, arguments);
    setTimeout(bbForcePaintVisibleCardsV27,300);
    setTimeout(bbForcePaintVisibleCardsV27,1000);
    return out;
  };
}

setTimeout(bbForcePaintVisibleCardsV27,700);
setTimeout(bbForcePaintVisibleCardsV27,1800);
setInterval(bbForcePaintVisibleCardsV27,2500);


/* ==================================================
   V27 PATCH - SURPRISE KLEURKIEZER + PICK FIX
   Werkt bovenop V27 zonder hele app te rebuilden.
   ================================================== */

function bbPatchAnimalV27(emoji){
  if(typeof bbAnimalV25 === "function") return bbAnimalV25(emoji || "🦜");
  const animals = {
    "🦜":{emoji:"🦜",name:"Papegaai",cls:"parrot"},
    "🐒":{emoji:"🐒",name:"Aap",cls:"monkey"},
    "🐬":{emoji:"🐬",name:"Dolfijn",cls:"dolphin"}
  };
  return animals[emoji] || {emoji:emoji || "🦜", name:"Dier", cls:"run"};
}

function bbPatchTargetV27(key){
  if(typeof bbTargetV25 === "function") return bbTargetV25(key || "yellow");
  return {
    yellow:{cls:"gold", left:"18%", emoji:"🟡", name:"GOUD"},
    pink:{cls:"aqua", left:"34%", emoji:"🩵", name:"AQUA"},
    purple:{cls:"orange", left:"50%", emoji:"🟠", name:"ORANJE"},
...
function bbPatchIslandHTMLV27(animalEmoji, colorKey, compact=false, reveal=false){
  const a = bbPatchAnimalV27(animalEmoji || "🦜");
  const t = bbPatchTargetV27(colorKey || "yellow");
  const keys = ["yellow","pink","purple","blue","green"];

  const targets = keys.map(k => {
    const x = bbPatchTargetV27(k);
    const chosen = reveal && k === colorKey;
    return `<div class="v25Target ${x.cls} ${chosen ? "chosen" : "v27PatchMysteryTarget"}"></div>`;
  }).join("");

  const title = reveal
    ? `${a.emoji} ${a.name} koos ${t.emoji}`
    : `${a.emoji} ${a.name} kiest een kleur...`;

  const animalClass = reveal ? a.cls : `${a.cls} v27PatchWander`;
  const targetLeft = reveal ? t.left : "50%";

  return `
    <div class="v25IslandPicker ${compact ? "compact" : ""}">
      <div class="v25Title">${title}</div>
      <div class="v25Stage" style="--target-left:${targetLeft}">
        <div class="v25Sun"></div>
        <div class="v25Palm">🌴</div>
        <div class="v25Targets">${targets}</div>
        <div class="v25Animal ${animalClass}">${a.emoji}</div>
      </div>
      <div class="v25Hint">${reveal ? "De kleur is gekozen!" : "Nog even spannend..."}</div>
    </div>
  `;
}

// Override picking render: tijdens picking nooit gekozen kleur tonen
renderPlayerPicker = function(round){
  const animal = round.animal || round.pickerMode || "🦜";
  const key = round.colorKey || round.targetColorKey || "yellow";

  const area = document.getElementById("playerPickerArea");
  if(area) area.innerHTML = bbPatchIslandHTMLV27(animal, key, false, false);

  const dash = document.getElementById("dashPickerArea");
  if(dash) dash.innerHTML = bbPatchIslandHTMLV27(animal, key, true, false);
};
renderPlayerPickerFixed = renderPlayerPicker;
...
renderCompactPicker = function(r){
  ...
  if(r.status === "picking"){
    area.innerHTML = bbPatchIslandHTMLV27(r.animal || r.pickerMode || "🦜", r.colorKey || r.targetColorKey || "yellow", true, false);
    return;
  }
...
// Override startRoundVisual: kleur blijft intern gekozen, maar animatie verklapt niks
startRoundVisual = function(room){
  document.getElementById("hostAnswerArea").innerHTML = "";
  document.getElementById("playBtn").disabled = true;
  document.getElementById("showAnswerBtn").disabled = true;
  ...
  const animal = (typeof bbPickAnimalV25 === "function")
    ? bbPickAnimalV25()
    : {emoji:"🦜", name:"Papegaai"};
  ...
  const pickerMarkup = bbPatchIslandHTMLV27(animal.emoji, color.key, true, false);
  const hostMarkup = bbPatchIslandHTMLV27(animal.emoji, color.key, false, false);
  ...
  setTimeout(() => {
    ...
    if(typeof hbHostPickerPopupShowV7I === "function"){
      hbHostPickerPopupShowV7I(bbPatchIslandHTMLV27(animal.emoji, color.key, false, true));
    }
...
// Pick fix: kaart renderen met click=true zodra speler goed is en nog niet gekozen heeft
function bbPatchForceCardV27(){
  const room = currentRoomCode || localStorage.getItem("hb_player_room") || "";
  if(!room || !currentPlayerId || !db) return;

  db.ref("rooms/" + room).once("value").then(s => {
    const data = s.val() || {};
    const p = data.players?.[currentPlayerId] || {};
    const card = p.card || [];
    const marked = p.marked || {};
    const r = data.currentRound || activeRound || {};
    if(!card.length || !r.id) return;

    const good = data.correct?.[r.id]?.[currentPlayerId] === true;
    const picked = p.lastPickedRound === r.id;
    const canPick = r.status === "judged" && good && !picked;

    if(typeof bbRenderCompactCardV27 === "function"){
      const dash = document.getElementById("dashOwnCard");
      if(dash) bbRenderCompactCardV27(dash, card, marked, canPick, r);
    }

    const pickCard = document.getElementById("pickCard");
    if(pickCard && typeof renderCard === "function"){
      renderCard("pickCard", card, marked, canPick, r);
    }

    const hint = document.getElementById("dashCardHint");
    const status = document.getElementById("dashCardStatus");
    if(hint) hint.textContent = canPick ? `Kies 1 ${r.colorEmoji} ${r.colorName} vakje.` : "Jouw bingokaart";
    if(status) status.textContent = canPick ? "Tik op een geldig vakje." : (picked ? "Vakje gekozen" : "Wachten op beoordeling...");
  }).catch(()=>{});
}

bbForcePaintVisibleCardsV27 = bbPatchForceCardV27;

setTimeout(bbPatchForceCardV27, 500);
setTimeout(bbPatchForceCardV27, 1400);
setInterval(bbPatchForceCardV27, 2200);

/* V27 CLICK HOTFIX */
document.addEventListener('click', function(e){
  const cell=e.target.closest('.pickableCell,.pickable');
  if(!cell) return;
...
/* ==================================================
   V29 REAL TAP FIX
   Centrale tap-handler voor bingokaart.
   Werkt ook als de kaart opnieuw wordt gerenderd.
   ================================================== */

function bbV29Room(){
  return currentRoomCode || localStorage.getItem("hb_player_room") || new URLSearchParams(location.search).get("room") || "";
}

function bbV29CanPickFromData(data){
  const r = data.currentRound || activeRound || {};
  if(!r || !r.id || r.status !== "judged") return {can:false, round:r, player:null};
  const p = data.players && currentPlayerId ? data.players[currentPlayerId] : null;
  if(!p) return {can:false, round:r, player:null};
  const good = data.correct && data.correct[r.id] && data.correct[r.id][currentPlayerId] === true;
  const picked = p.lastPickedRound === r.id;
  return {can:!!(good && !picked), round:r, player:p};
}

function bbV29PaintPickables(data){
  const check = bbV29CanPickFromData(data);
  const r = check.round || {};
  const p = check.player || {};
  const card = p.card || [];
  const marked = p.marked || {};
  const canPick = check.can;

  const roots = [document.getElementById("dashOwnCard"), document.getElementById("pickCard")].filter(Boolean);

  roots.forEach(root => {
    const cells = Array.from(root.querySelectorAll(".compactCell,.bingoCell"));
    cells.forEach((cell,i) => {
      const colorKey = cell.dataset.colorKey || card[i] || "";
      const isPickable = canPick && colorKey === r.colorKey && colorKey !== "free" && !marked[i];

      cell.dataset.i = String(i);
      cell.dataset.colorKey = colorKey;

      cell.classList.toggle("bbCanPick", isPickable);
      cell.classList.toggle("pickable", isPickable);
      cell.classList.toggle("pickableCell", isPickable);
  ...
  const hint = document.getElementById("dashCardHint");
  const status = document.getElementById("dashCardStatus");
  if(hint && canPick) hint.textContent = `Kies 1 ${r.colorEmoji || ""} ${r.colorName || ""} vakje.`;
  if(status && canPick) status.textContent = "Tik op een geldig vakje.";
}

function bbV29PickIndex(i){
  const room = bbV29Room();
  if(!room || !currentPlayerId || !db) return;
  ...
  db.ref("rooms/" + room).once("value").then(s => {
    ...
    if(!check.can) {
      const st = document.getElementById("dashCardStatus");
      if(st) st.textContent = "Je mag nu geen vakje kiezen.";
      return;
    }

    if(card[i] !== r.colorKey || marked[i] || card[i] === "free"){
      const st = document.getElementById("dashCardStatus");
      if(st) st.textContent = "Kies een opgelicht vakje.";
      return;
    }
    ...
    return db.ref("rooms/" + room + "/players/" + currentPlayerId).update({
      marked,
      bingo,
      lastPickedRound:r.id,
      ready:true
    }).then(() => {
      if(bingo){
        return db.ref("rooms/" + room + "/bingos").push({
          name:currentPlayerName || p.name || "Speler",
          roundId:r.id,
      ...
      const st = document.getElementById("dashCardStatus");
      if(st) st.textContent = "Vakje gekozen ✅";
      try{ showScreen("screenLobby"); }catch(e){}
    });
  }).catch(err => {
    alert("Vakje kiezen fout: " + (err.message || err));
  });
}

function bbV29TapHandler(e){
  const root = e.target.closest("#dashOwnCard,#pickCard");
  if(!root) return;

  const cell = e.target.closest(".compactCell,.bingoCell,.bbColoredCell");
  if(!cell || !root.contains(cell)) return;

  const i = Number(cell.dataset.i);
  if(!Number.isFinite(i)) return;

  // Alleen ingrijpen op opgelichte vakjes of tijdens pick status.
  if(cell.classList.contains("bbCanPick") || cell.classList.contains("pickable") || cell.classList.contains("pickableCell")){
    e.preventDefault();
    e.stopPropagation();
    bbV29PickIndex(i);
  }
...
document.addEventListener("click", bbV29TapHandler, true);
document.addEventListener("touchend", function(e){
  const root = e.target.closest && e.target.closest("#dashOwnCard,#pickCard");
  const cell = e.target.closest && e.target.closest(".compactCell,.bingoCell,.bbColoredCell");
  if(root && cell){
    if(cell.classList.contains("bbCanPick") || cell.classList.contains("pickable") || cell.classList.contains("pickableCell")){
      e.preventDefault();
      e.stopPropagation();
      bbV29PickIndex(Number(cell.dataset.i));
    }
  }
}, {capture:true, passive:false});

function bbV29WatchRoom(){
  const room = bbV29Room();
  if(!room || !db || window.__bbV29Watching === room) return;
  window.__bbV29Watching = room;

  db.ref("rooms/" + room).on("value", snap => {
    const data = snap.val() || {};
    setTimeout(() => bbV29PaintPickables(data), 80);
    setTimeout(() => bbV29PaintPickables(data), 350);
  });
}

setTimeout(bbV29WatchRoom, 600);
setTimeout(bbV29WatchRoom, 1600);
setInterval(() => {
  bbV29WatchRoom();
  const room = bbV29Room();
  if(room && db){
    db.ref("rooms/" + room).once("value").then(s => bbV29PaintPickables(s.val() || {})).catch(()=>{});
  }
}, 2000);
