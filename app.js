...
/* =========================
   V4 AUTO ANTWOORD NA TIMER
   - Timer voorbij = ronde locked
   - Correct Spotify-antwoord direct zichtbaar bij spelers
   ========================= */

function getCurrentSpotifyAnswer(){
...
function publishCorrectAnswerToRound(){
  const room = currentRoomCode || localStorage.getItem("hb_host_room");
  const ans = getCurrentSpotifyAnswer();
...
// Override lockRound: bij timer voorbij ook antwoord tonen
lockRound = function(){
  const room = currentRoomCode || localStorage.getItem("hb_host_room");
  if(!db || !room) return;

  publishCorrectAnswerToRound().then(() => {
    return db.ref("rooms/" + room + "/currentRound/status").set("locked");
  }).then(() => {
    const st = document.getElementById("hostStatus");
    if(st) st.textContent = "Tijd voorbij. Antwoord automatisch getoond bij spelers.";
  }).catch(e => alert("Ronde vergrendelen mislukt: " + e.message));
};

// Override showAnswer: knop blijft bestaan, maar gebruikt dezelfde functie
showAnswer = function(){
  publishCorrectAnswerToRound().catch(e => alert("Antwoord tonen mislukt: " + e.message));
};

// Override playHidden zodat timer altijd nieuwe lockRound gebruikt
playHidden = async function(){
  ...
  const dur = (Number($("duration").value) || 20) * 1000;
  let pos = 0;
...
// Zorg dat spelers bij locked direct het antwoord zien in scorebord
const oldRenderScoreAutoAnswer = renderScore;
renderScore = function(room){
  oldRenderScoreAutoAnswer(room);

  const r = room.currentRound || {};
  const box = document.getElementById("correctAnswerBox");

  if(box && r.correctAnswer){
...
// Rebind knoppen naar overrides
setTimeout(() => {
  const play = document.getElementById("playBtn");
  if(play){
    const fresh = play.cloneNode(true);
    play.parentNode.replaceChild(fresh, play);
    fresh.addEventListener("click", playHidden);
  }

  const answer = document.getElementById("showAnswerBtn");
  if(answer){
    const fresh = answer.cloneNode(true);
    answer.parentNode.replaceChild(fresh, answer);
    fresh.addEventListener("click", showAnswer);
  }

  const lock = document.getElementById("lockBtn");
  if(lock){
    const fresh = lock.cloneNode(true);
    lock.parentNode.replaceChild(fresh, lock);
    fresh.addEventListener("click", lockRound);
  }
}, 500);
