...
(function(){
  ...
  function loadCategories(){
    fields.forEach(id=>{
      ...
      if(saved!==null){
        ...
        // Eerste keer huidige waarde opslaan. Daarna nooit meer automatisch overschrijven.
  ...
  function bindCategories(){
    fields.forEach(id=>{
      const el=document.getElementById(id);
      if(!el || el.dataset.bbFinalCategoryMemory==="1") return;

      el.dataset.bbFinalCategoryMemory="1";
  ...
  document.addEventListener("DOMContentLoaded",initCategoryMemory);
  window.addEventListener("pageshow",initCategoryMemory);
  window.addEventListener("focus",initCategoryMemory);

  setTimeout(initCategoryMemory,50);
  setTimeout(initCategoryMemory,300);
  setTimeout(initCategoryMemory,1000);
})();


/* ==================================================
   V43 HARD LOCK CATEGORIEËN
...
(function(){
  ...
  function restore(id){
    ...
    let saved = localStorage.getItem(KEY(id));

    // Eerste keer: bewaar wat er NU staat.
    if(saved === null){
      saved = input.value || "";
      localStorage.setItem(KEY(id), saved);
    }
  ...
  function protectInput(id){
    const input = el(id);
    if(!input || input.dataset.bbHardLocked === "1") return;
    input.dataset.bbHardLocked = "1";

    // Bewaar eerste waarde als er nog niets is.
    ...
    if(desc && desc.get && desc.set){
      try{
        Object.defineProperty(input, "value", {
          ...
          set(v){
            const currentSaved = localStorage.getItem(KEY(id));

            // Toestaan als gebruiker in dit veld bezig is of onze eigen restore loopt.
            if(restoring || userIsTyping || document.activeElement === input){
              desc.set.call(this, v);
              return;
            }

            // Automatische code probeert iets anders te zetten: blokkeren.
            if(currentSaved !== null && String(v ?? "") !== currentSaved){
              desc.set.call(this, currentSaved);
              return;
            }
    ...
    const beginUser = () => { userIsTyping = true; };
    const endUser = () => {
      save(id);
      setTimeout(()=>{ userIsTyping = false; }, 250);
    };

    ["keydown","pointerdown","mousedown","touchstart","focus"].forEach(ev=>{
      input.addEventListener(ev, beginUser, true);
    });

    ["input","change","keyup","blur"].forEach(ev=>{
      input.addEventListener(ev, endUser, true);
    });

    input.addEventListener("paste",()=>{
      userIsTyping = true;
      setTimeout(()=>{ save(id); userIsTyping=false; }, 50);
    },true);
  }

  function init(){
    IDS.forEach(protectInput);
    restoreAll();
  }

  // Voor elke knopklik eerst bewaren, daarna herstellen.
  function patchClicks(){
    document.querySelectorAll("button,a").forEach(btn=>{
      if(btn.dataset.bbCategoryClickLock === "1") return;
      btn.dataset.bbCategoryClickLock = "1";
  ...
  document.addEventListener("DOMContentLoaded",()=>{init();patchClicks();});
  window.addEventListener("pageshow",()=>setTimeout(()=>{init();patchClicks();},50));
  window.addEventListener("focus",()=>setTimeout(()=>{init();patchClicks();},50));

  setTimeout(()=>{init();patchClicks();},50);
  setTimeout(()=>{init();patchClicks();},300);
  setTimeout(()=>{init();patchClicks();},1000);
  setInterval(()=>{init();patchClicks();restoreAll();},1000);
})();
