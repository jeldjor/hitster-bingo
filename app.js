...
/* =========================
   V3 FIX 1
   - Goed/fout knoppen host
   - Kleurkiezer zichtbaar bij spelers
   ========================= */

// Host goed/fout fix
markAnswer = function(pid, good){
  const room = currentRoomCode || localStorage.hb_host_room;
  if(!db || !room) return alert("Geen actieve kamer gevonden.");

  db.ref("rooms/" + room).once("value").then(s => {
    const rid = (s.val() || {}).currentRound?.id;
    if(!rid) throw new Error("Geen actieve ronde gevonden.");
    return db.ref("rooms/" + room + "/correct/" + rid + "/" + pid).set(good);
  }).catch(e => alert("Goed/fout opslaan mislukt: " + e.message));
};

// Resultaten naar spelers fix
publishResults = function(){
  const room = currentRoomCode || localStorage.hb_host_room;
  if(!db || !room) return alert("Geen actieve kamer gevonden.");
  ...
  db.ref("rooms/" + room).once("value").then(s => {
    const data = s.val() || {};
    const up = {};
    up["rooms/" + room + "/currentRound/status"] = "judged";
    Object.keys(data.players || {}).forEach(pid => {
      up["rooms/" + room + "/players/" + pid + "/ready"] = false;
    });
    return db.ref().update(up);
  }).then(() => {
    if($("hostStatus")) $("hostStatus").textContent = "Resultaten verzonden. Goede spelers mogen een vakje kiezen.";
  }).catch(e => alert("Resultaten verzenden mislukt: " + e.message));
};

// Host ronde starten met 'picking' status zodat spelers kleurenkiezer zien
startRoundVisual = function(room){
  $("hostAnswerArea").innerHTML = "";
  $("playBtn").disabled = true;
  $("showAnswerBtn").disabled = true;
  $("pickerArea").innerHTML = '<div class="disco">🪩</div><div class="pickerTitle">Kleur kiezen...</div>';
...
// Speler kleurenkiezer animatie
function renderPickingFixed(){
  $("colorDisplay").innerHTML =
    `<div class="playerPickerAnimation"><div class="playerPickerBall">🪩</div><div class="playerPickerText">Kleur wordt gekozen...</div></div>`;
  $("categoryDisplay").textContent = "Wacht op de categorie...";
}

// Speler listener opnieuw met picking-status
listenPlayer = function(){
  db.ref("rooms/" + currentRoomCode).on("value", s => {
    const room = s.val() || {};
...
// Rebind publish button to fixed function
setTimeout(() => {
  const pub = $("publishBtn");
  if(pub){
    const fresh = pub.cloneNode(true);
    pub.parentNode.replaceChild(fresh, pub);
