...
/* =========================
   ROOM RESUME V2
   Spotify ongemoeid laten
   ========================= */

function hbRememberRoomV2(){
  try{
    if(currentRoomCode){
      localStorage.setItem("hb_last_room", currentRoomCode);
    }
  }catch(e){}
}

setInterval(hbRememberRoomV2, 2000);

function hbResumePromptV2(){
  if(new URLSearchParams(location.search).get("room")) return;

  const saved = localStorage.getItem("hb_last_room");
  if(!saved) return;

  const already = sessionStorage.getItem("hb_resume_prompt_shown");
  if(already) return;

  sessionStorage.setItem("hb_resume_prompt_shown","1");

  setTimeout(()=>{
    const resume = confirm(
`Actieve kamer gevonden

${saved}

OK = Hervatten
Annuleren = Nieuwe kamer`
    );
    ...
    if(resume){
      ...
      try{
        if(typeof showRoom === "function") showRoom();
      }catch(e){}

      try{
        if(typeof listenHost === "function") listenHost(saved);
      }catch(e){}

      try{
        if(typeof listenBingo === "function") listenBingo(saved);
      }catch(e){}

      const st = document.getElementById("hostStatus");
      if(st){
        st.textContent = "Kamer hervat. Klaar voor nieuwe ronde.";
      }
    }else{
      localStorage.removeItem("hb_last_room");
    }
  },600);
}

setTimeout(hbResumePromptV2,1500);
