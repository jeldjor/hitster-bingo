...
function joinPlayer(){let name=$("playerNameInput").value.trim();if(!name){alert("Vul je naam in.");return}currentPlayerName=name;if(!currentPlayerId)currentPlayerId="p_"+Math.random().toString(36).slice(2,10)+Date.now().toString(36).slice(-4);localStorage.hb_player_id=currentPlayerId;localStorage.hb_player_name=name;localStorage.hb_player_room=currentRoomCode;let ref=db.ref("rooms/"+currentRoomCode+"/players/"+currentPlayerId);ref.once("value").then(s=>{let ex=s.val()||{};return ref.update({name,online:true,ready:false,joinedAt:ex.joinedAt||firebase.database.ServerValue.TIMESTAMP,lastSeen:firebase.database.ServerValue.TIMESTAMP,card:ex.card||genCard(),marked:ex.marked||{}})}).then(()=>{ref.child("online").onKleurenmixer startnnect().set(false);listenPlayer();showScreen("screenLobby")})}
...
function pickerHTML(mode){return mode==="wheel"?`<div class="wheelWrap"><div class="pointer"></div><div class="wheel"></div></div><div class="pickerTitle">Draairad draait...</div>`:`<div class="discoIcon">🪩</div><div class="pickerTitle">Kleurenmixer start...</div>`}
...
/* =========================
   V4 PICKER FIX
   Spelers zien dezelfde Kleurenmixer start/draairad animatie als host
   ========================= */
...
function renderCompactAnswer(room, r){
  ...
  if(r.status === "picking"){
    block.innerHTML = `<div class="answerSubmitted"><h3>🪩 Kleurkiezer</h3><p>De host Kleurenmixer start</p></div>`;
    return;
  }
...
function renderCompactPicker(r){
  ...
  if(!r || !r.id){
    area.innerHTML = `<div class="dashKleurenmixer start">🪩</div><div class="dashCatText">Wachten op host</div>`;
    return;
  }
  ...
  if(r.status === "picking"){
    if(r.pickerMode === "wheel"){
      ...
      area.innerHTML = `<div class="dashKleurenmixer start">🪩</div><div class="dashCatText">Kleurenmixer start...</div>`;
    }
    return;
  }
...
joinPlayer = function(){
  ...
  ref.once("value").then(s => {
    ...
    ref.child("online").onKleurenmixer startnnect().set(false);
    listenPlayer();
    showCompactDashboard();
...
joinPlayer = async function(){
  try{
    ...
    ref.child("online").onKleurenmixer startnnect().set(false);
    ref.child("lastSeen").onKleurenmixer startnnect().set(firebase.database.ServerValue.TIMESTAMP);
...
function hbHostPickerPopupPickingV7I(mode){
  const inner = typeof sharedPickerHTML === "function"
    ? sharedPickerHTML(mode)
    : (typeof pickerHTML === "function" ? pickerHTML(mode) : `<div class="discoIcon">🪩</div><div class="pickerTitle">Kleurenmixer start...</div>`);
...
function hbAnimLabelV8(mode){
  return {
    disco:"🪩 Kleurenmixer startbal", wheel:"🎡 Kleurenrad", vinyl:"💿 Vinyl Picker",
...
function hbShowAnimHTMLV8C(mode, compact=false){
  ...
  return `${wrapStart}<div class="hostPickerPopupTitle">${hbAnimLabelV8C("disco")}</div><div class="showKleurenmixer startBall">🪩</div><div class="showAnimTitle">Kleurenmixer start...</div>${wrapEnd}`;
}
...
function hbAnimHTMLV9C(mode, compact=false){
  ...
  return `<div class="${wrap}"><div class="hostPickerPopupTitle">${hbAnimLabelV9C("disco")}</div><div class="showKleurenmixer startBall">🪩</div><div class="showAnimTitle">Kleurenmixer start...</div></div>`;
}
...
if(typeof renderCompactPicker === "function"){
  renderCompactPicker = function(r){
    const area=document.getElementById("dashPickerArea");
    if(!area)return;
    if(!r||!r.id){area.innerHTML=`<div class="dashKleurenmixer start">🪩</div><div class="dashCatText">Wachten op host...</div>`;return;}
...
(function(){
  ...
  function pickerHTML2(animalEmoji,colorKey,compact=false,reveal=false){
    ...
    return `<div class="bbPicker ${compact?"compact":""}">
      <div class="bbPickerTitle">${reveal?`${a.emoji} ${a.name} koos de kleur!`:`${a.emoji} ${a.name} Kleurenmixer start`}</div>
...
(function(){
  ...
  function loadCats(){
    fields.forEach(id=>{
      const el=document.getElementById(id);
      if(!el) return;
      const saved=localStorage.getItem("bb_"+id);
  ...
  function bindCats(){
    fields.forEach(id=>{
      const el=document.getElementById(id);
      if(!el || el.dataset.bbMemory==="1") return;

      el.dataset.bbMemory="1";

      const save=()=>localStorage.setItem("bb_"+id, el.value);
  ...
  setTimeout(loadCats,100);
  setTimeout(bindCats,100);

  setTimeout(loadCats,1000);
  setTimeout(bindCats,1000);

  setInterval(bindCats,3000);
})();


/* ==================================================
   V34 SPOTIFY LOGIN CATEGORY KEEP
   Voorkomt dat categorieën resetten bij Spotify-login.
   ================================================== */

(function(){
  const BB_CAT_FIELDS_V34 = ["cat-yellow","cat-pink","cat-purple","cat-blue","cat-green"];

  function bbSaveCatsV34(){
    BB_CAT_FIELDS_V34.forEach(id=>{
      const el=document.getElementById(id);
      if(el) localStorage.setItem("bb_"+id, el.value || "");
    });
  }

  function bbLoadCatsV34(){
    BB_CAT_FIELDS_V34.forEach(id=>{
      const el=document.getElementById(id);
      if(!el) return;
      const saved=localStorage.getItem("bb_"+id);
  ...
  function bbBindCatsV34(){
    BB_CAT_FIELDS_V34.forEach(id=>{
      const el=document.getElementById(id);
      if(!el || el.dataset.bbCatKeep==="1") return;
      el.dataset.bbCatKeep="1";
      el.addEventListener("input",bbSaveCatsV34);
      el.addEventListener("change",bbSaveCatsV34);
      el.addEventListener("blur",bbSaveCatsV34);
    });
  }

  function bbPatchSpotifyButtonsV34(){
    document.querySelectorAll("button,a").forEach(btn=>{
      const txt=(btn.textContent||"").toLowerCase();
      const href=(btn.getAttribute && (btn.getAttribute("href")||"").toLowerCase()) || "";
      const looksSpotify = txt.includes("spotify") || href.includes("spotify");
      if(!looksSpotify || btn.dataset.bbSpotifyCatPatch==="1") return;

      btn.dataset.bbSpotifyCatPatch="1";

      btn.addEventListener("pointerdown",bbSaveCatsV34,{capture:true});
      btn.addEventListener("touchstart",bbSaveCatsV34,{capture:true,passive:true});
      btn.addEventListener("mousedown",bbSaveCatsV34,{capture:true});
      btn.addEventListener("click",()=>{
        bbSaveCatsV34();
        setTimeout(bbLoadCatsV34,50);
        setTimeout(bbLoadCatsV34,250);
        setTimeout(bbLoadCatsV34,900);
      },true);
    });
  }

  // Ook bij elke pagina-verandering/terugkomst na Spotify auth herstellen.
  window.addEventListener("pageshow",()=>{bbLoadCatsV34(); setTimeout(bbLoadCatsV34,500);});
  window.addEventListener("focus",()=>{bbLoadCatsV34(); setTimeout(bbLoadCatsV34,500);});
  document.addEventListener("visibilitychange",()=>{
    if(document.visibilityState==="hidden") bbSaveCatsV34();
    if(document.visibilityState==="visible") setTimeout(bbLoadCatsV34,100);
  });

  setTimeout(()=>{bbLoadCatsV34();bbBindCatsV34();bbPatchSpotifyButtonsV34();},100);
  setTimeout(()=>{bbLoadCatsV34();bbBindCatsV34();bbPatchSpotifyButtonsV34();},1000);
  setInterval(()=>{bbBindCatsV34();bbPatchSpotifyButtonsV34();},2000);
})();
...
/* ==================================================
   V37 FINAL CLEAN: ONLY CARTOON PICKER + NEVER RESET CATS
   ================================================== */
(function(){
  const CAT_IDS=["cat-yellow","cat-pink","cat-purple","cat-blue","cat-green"];
  const DEFAULT_CATS={
    "cat-yellow":"Jaren 80",
    "cat-pink":"Vrouwelijke artiest",
    "cat-purple":"Nederlandse artiest",
    "cat-blue":"Dance",
    "cat-green":"Nummer 1-hit"
  };
  ...
  function loadCats37(){
    CAT_IDS.forEach(id=>{
      ...
      const saved=localStorage.getItem("bb_"+id);
      if(saved!==null) el.value=saved;
      else if(el.value) localStorage.setItem("bb_"+id, el.value);
    });
  }
  function bindCats37(){
    CAT_IDS.forEach(id=>{
      const el=document.getElementById(id);
      if(!el || el.dataset.bb37cat==="1") return;
      el.dataset.bb37cat="1";
      ["input","change","blur"].forEach(ev=>el.addEventListener(ev,saveCats37));
    });
  }
  // If any old popup/default function tries to reset categories, restore immediately.
  function protectCats37(){
    loadCats37();
    bindCats37();
    document.querySelectorAll("button,a").forEach(el=>{
      const txt=(el.textContent||"").toLowerCase();
      if(txt.includes("spotify") || txt.includes("login")){
        if(el.dataset.bb37spotify==="1") return;
        el.dataset.bb37spotify="1";
        ["pointerdown","mousedown","touchstart","click"].forEach(ev=>{
          el.addEventListener(ev,()=>{saveCats37(); setTimeout(loadCats37,0); setTimeout(loadCats37,60); setTimeout(loadCats37,300); setTimeout(loadCats37,1200);},true);
        });
      }
    });
  }
  ...
  const C37={
    yellow:{name:"GOUD",emoji:"🟡",rot:"0deg"},
    pink:{name:"AQUA",emoji:"🩵",rot:"-72deg"},
    purple:{name:"ORANJE",emoji:"🟠",rot:"-144deg"},
    blue:{name:"LIME",emoji:"🟢",rot:"-216deg"},
    green:{name:"KORAAL",emoji:"🔴",rot:"-288deg"}
  };
  const A37=[{key:"monkey",name:"Aap"},{key:"parrot",name:"Papegaai"},{key:"turtle",name:"Schildpad"}];
  function p37(arr){return typeof pick==="function"?pick(arr):arr[Math.floor(Math.random()*arr.length)]}
  function animal37(){
    const last=localStorage.getItem("bb37_last_animal")||"";
    const opts=A37.filter(a=>a.key!==last);
    const a=p37(opts.length?opts:A37);
    localStorage.setItem("bb37_last_animal",a.key);
    return a;
  }
  function animalHTML37(k){
    if(k==="parrot")return `<div class="bb37Animal bb37Parrot"><div class="bb37Shadow"></div><div class="wingL"></div><div class="wingR"></div><div class="tail"></div><div class="body"></div><div class="head"></div><div class="beak"></div><div class="eye"></div></div>`;
    if(k==="turtle")return `<div class="bb37Animal bb37Turtle"><div class="bb37Shadow"></div><div class="leg1"></div><div class="leg2"></div><div class="leg3"></div><div class="leg4"></div><div class="tail"></div><div class="shell"></div><div class="head"></div><div class="eye"></div></div>`;
    return `<div class="bb37Animal bb37Monkey"><div class="bb37Shadow"></div><div class="tail"></div><div class="earL"></div><div class="earR"></div><div class="armL"></div><div class="armR"></div><div class="legL"></div><div class="legR"></div><div class="body"></div><div class="belly"></div><div class="head"></div><div class="face"></div><div class="eyeL"></div><div class="eyeR"></div><div class="mouth"></div></div>`;
  }
  function html37(animalKey,colorKey,compact=false,reveal=false){
    const color=C37[colorKey]||C37.yellow;
    const animal=A37.find(a=>a.key===animalKey)||A37[0];
    return `<div class="bb37Picker ${compact?"compact":""}" data-bb37-picker="1">
      <div class="bb37Title">${reveal?`${animal.name} koos de kleur!`:`${animal.name} start de kleurenmixer...`}</div>
      <div class="bb37Stage">
        <div class="bb37Sun"></div><div class="bb37Palm">🌴</div>
        ${animalHTML37(animal.key)}
        <div class="bb37Lever"></div>
        <div class="bb37WheelWrap"><div class="bb37Pointer"></div><div class="bb37Wheel ${reveal?"reveal":""}" style="--bb-final-rot:${color.rot}"></div><div class="bb37WheelCenter"></div></div>
        ${reveal?`<div class="bb37Badge">${color.emoji} ${color.name}</div>`:""}
      </div>
      <div class="bbHint">${reveal?"Kleur gekozen!":"Nog even spannend..."}</div>
    </div>`;
  }

  function hideSound37(){
    document.querySelectorAll("button,a,div,p,span").forEach(el=>{
      const txt=(el.textContent||"").toLowerCase();
      const id=(el.id||"").toLowerCase();
      const cls=(el.className||"").toString().toLowerCase();
      if(txt.includes("activeer host")||txt.includes("geluid nog niet geactiveerd")||id.includes("sound")||id.includes("audio")||cls.includes("sound")||cls.includes("audio")){
  ...
  // Remove old picker functions from runtime by assigning only these.
  window.pickerHTML=function(mode){return html37(mode||"monkey","yellow",false,false)};
  window.sharedPickerHTML=function(mode){return html37(mode||"monkey","yellow",true,false)};
  window.renderPlayerPicker=function(r){
    const a=r.animalKey||r.animal||r.pickerMode||"monkey", k=r.colorKey||r.targetColorKey||"yellow";
    const p=document.getElementById("playerPickerArea"); if(p)p.innerHTML=html37(a,k,false,false);
    const d=document.getElementById("dashPickerArea"); if(d)d.innerHTML=html37(a,k,true,false);
  };
  window.renderPlayerPickerFixed=window.renderPlayerPicker;
  window.renderCompactPicker=function(r){
    const area=document.getElementById("dashPickerArea"); if(!area)return;
    if(!r||!r.id){area.innerHTML=`<div class="dashCatText">Wachten op host...</div>`;return;}
    if(r.status==="picking"){area.innerHTML=html37(r.animalKey||r.animal||r.pickerMode||"monkey",r.colorKey||r.targetColorKey||"yellow",true,false);return;}
    area.innerHTML=`<div class="dashColorBig">${esc(r.colorEmoji||"")}</div><div class="dashColorText">${esc(r.colorName||"KLEUR")}</div><div class="dashCatText">${esc(r.category||"Wachten...")}</div>`;
  };
  window.startRoundVisual=function(room){
    protectCats37(); saveCats37();
    const a=animal37();
    const color=p37(COLORS);
    const cat=document.getElementById(color.input).value||"Geen categorie";
    currentRoundId="r_"+Date.now();
    const hostMarkup=html37(a.key,color.key,false,false);
    const playerMarkup=html37(a.key,color.key,true,false);
    ...
    document.getElementById("playBtn").textContent="🎵 Speel verborgen nummer";
    if(typeof hbHostPickerPopupShowV7I==="function")hbHostPickerPopupShowV7I(hostMarkup);
    db.ref("rooms/"+room+"/currentRound").set({id:currentRoundId,status:"picking",pickerMode:a.key,animal:a.key,animalKey:a.key,animalName:a.name,targetColorKey:color.key,colorKey:color.key,colorName:color.name,colorEmoji:color.emoji,category:cat,pickerMarkup:playerMarkup,pickerStartedAt:firebase.database.ServerValue.TIMESTAMP,seconds:Number(document.getElementById("duration").value)||20});
    setTimeout(()=>{
      flash();
      if(hostArea)hostArea.innerHTML=`<div class="colorDisplay">${color.emoji}<br>${color.name}</div><div class="categoryDisplay">${esc(cat)}</div>`;
      if(typeof hbHostPickerPopupShowV7I==="function")hbHostPickerPopupShowV7I(html37(a.key,color.key,false,true));
      db.ref("rooms/"+room+"/currentRound").set({id:currentRoundId,status:"ready",pickerMode:a.key,animal:a.key,animalKey:a.key,animalName:a.name,targetColorKey:color.key,colorKey:color.key,colorName:color.name,colorEmoji:color.emoji,category:cat,pickerMarkup:playerMarkup,seconds:Number(document.getElementById("duration").value)||20});
      document.getElementById("playBtn").disabled=false;
      document.getElementById("showAnswerBtn").disabled=false;
      document.getElementById("hostScorePanel").classList.remove("hidden");
      const st=document.getElementById("hostStatus"); if(st)st.textContent="Kleur bekend. Klik nu op Speel verborgen nummer.";
  ...
  // catch old rendered picker after any update
  function killOldPicker37(){
    ["hostPickerArea","dashPickerArea","playerPickerArea"].forEach(id=>{
      const el=document.getElementById(id); if(!el)return;
      const html=el.innerHTML||"";
      if(!html.includes("data-bb37-picker") && (html.includes("dashDisco")||html.includes("Discobal")||html.includes("Vos")||html.includes("v25IslandPicker")||html.includes("tropicalIslandPicker")||html.includes("tropicalPicker"))){
        el.innerHTML=html37("monkey","yellow",id!=="hostPickerArea",false);
      }
    });
  }

  protectCats37(); hideSound37();
  setTimeout(()=>{protectCats37();hideSound37();killOldPicker37();},100);
  setTimeout(()=>{protectCats37();hideSound37();killOldPicker37();},700);
  setInterval(()=>{protectCats37();hideSound37();killOldPicker37();},1500);
})();
