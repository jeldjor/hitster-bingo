...
/* =========================
   MEEDOEN KNOP HARD FIX
   ========================= */

function hbSafePlayerId(){
  return "p_" + Math.random().toString(36).slice(2,10) + Date.now().toString(36).slice(-5);
}

function hbSafeCard(){
  const colors = ["yellow","pink","purple","blue","green"];
  const pool = [];
  for(let i=0;i<24;i++) pool.push(colors[i % colors.length]);
  ...
  const card = [];
  let p = 0;
  for(let i=0;i<25;i++) card.push(i===12 ? "free" : pool[p++]);
  return card;
}

function hbShowScreen(id){
  document.querySelectorAll("#playerAppPanel .playerScreen").forEach(s => s.classList.add("hidden"));
  document.getElementById(id)?.classList.remove("hidden");
}

function hbRenderLobby(room){
  const players = room.players || {};
  const me = players[currentPlayerId] || {};

  const lobby = document.getElementById("lobbyPlayers");
  if(lobby){
    lobby.innerHTML = Object.entries(players).map(([pid,p]) => `
      <div class="screenPlayerRow ${p.ready ? "ready" : "notReady"}" onclick="hbShowOtherCard('${pid}')">
        <strong>${esc(p.name || "Speler")}${pid===currentPlayerId ? " (jij)" : ""}</strong>
        <span>${p.ready ? "✅ READY" : "⏳ wacht"}</span>
      </div>
    `).join("") || "Nog geen spelers.";
  ...
  if(status) status.textContent = `${ready}/${total} spelers ready`;

  if(typeof renderSimpleCard === "function"){
    renderSimpleCard("screenOwnCard", me.card || [], me.marked || {});
  }else{
    hbRenderCard("screenOwnCard", me.card || [], me.marked || {});
  }

  const readyBtn = document.getElementById("readyBtn");
  if(readyBtn){
    readyBtn.disabled = !!me.ready;
    readyBtn.classList.toggle("readyActive", !!me.ready);
  }
}

function hbRenderCard(targetId, card, marked){
  const target = document.getElementById(targetId);
  if(!target) return;
  const em = k => ({yellow:"🟨",pink:"🩷",purple:"🟪",blue:"🟦",green:"🟩",free:"⭐"}[k] || "⬜");
  target.innerHTML = (card || []).map((c,i)=>{
    const m = marked && marked[i];
    return `<div class="bingoCell ${c==="free"?"free":""}">${m ? "✅" : em(c)}</div>`;
  }).join("");
}

function hbShowOtherCard(pid){
  const room = window.hbLastRoom || {};
  const p = room.players && room.players[pid] ? room.players[pid] : null;
  if(!p) return;
  document.getElementById("screenOtherCardBox")?.classList.remove("hidden");
  const title = document.getElementById("screenOtherCardTitle");
  if(title) title.textContent = "Kaart van " + (p.name || "Speler");
  hbRenderCard("screenOtherCard", p.card || [], p.marked || {});
}
window.hbShowOtherCard = hbShowOtherCard;

async function hbJoinNow(){
  try{
    const room = new URLSearchParams(window.location.search).get("room");
    if(!room){
      alert("Geen spelcode gevonden.");
      return;
    }
    ...
    currentPlayerName = name;
    currentPlayerId = localStorage.getItem("hb_player_id") || hbSafePlayerId();
    ...
    const st = document.getElementById("screenJoinStatus");
    if(st) st.textContent = "Je doet mee!";

    hbShowScreen("playerScreenLobby");

    db.ref("rooms/" + currentRoomCode).on("value", s => {
      const data = s.val() || {};
      window.hbLastRoom = data;
      hbRenderLobby(data);

      // Als de echte player screen listener bestaat, mag die ook draaien
      try{
        if(typeof listenPlayerScreens === "function"){
          // niet opnieuw oneindig starten; alleen als marker niet gezet
          if(!window.hbScreenListenerStarted){
            window.hbScreenListenerStarted = true;
            listenPlayerScreens();
          }
        }
      }catch(e){}
...
setTimeout(() => {
  const btn = document.getElementById("screenJoinBtn");
  if(btn){
    btn.onclick = null;
    btn.addEventListener("click", hbJoinNow);
  }
  ...
  if(ready){
    ...
    ready.addEventListener("click", () => {
      ...
      if(db && room && pid){
        db.ref("rooms/" + room + "/players/" + pid + "/ready").set(true);
      }
    });
  }
}, 300);
...
setTimeout(() => {
  ...
  if(room){
    document.body.classList.add("playerMode");
    document.getElementById("playerAppPanel")?.classList.remove("hidden");
    document.getElementById("playerScreenJoin")?.classList.remove("hidden");
    const rc = document.getElementById("screenRoomCode");
    if(rc) rc.textContent = room.toUpperCase();
  }
}, 600);
...
function setPlayerReadyFixed(){
  ...
  if(!db){
    try{
      ...
      alert("Firebase niet actief: " + e.message);
      return;
    }
  }

  if(!room || !pid){
    alert("Je bent nog niet goed verbonden. Vul je naam opnieuw in via de spelerlink.");
    return;
  }
  ...
  db.ref("rooms/" + currentRoomCode + "/players/" + currentPlayerId + "/ready").set(true)
    .then(() => {
      ["playerReadyStatus","scoreReadyStatus","joinStatus"].forEach(id => {
        const el = document.getElementById(id);
        if(el) el.textContent = "✅ Je bent READY.";
      });
      ["playerReadyBtn","scoreReadyBtn"].forEach(id => {
        const btn = document.getElementById(id);
        if(btn){
          btn.classList.add("readyActive");
          btn.textContent = "✅ READY";
          btn.disabled = true;
        }
      });
    })
    .catch(e => alert("READY zetten mislukt: " + e.message));
}

function updateReadyButtonsFixed(player){
  const isReady = !!(player && player.ready);
  ["playerReadyBtn","scoreReadyBtn"].forEach(id => {
    const btn = document.getElementById(id);
    if(btn){
      btn.disabled = isReady;
      btn.classList.toggle("readyActive", isReady);
      btn.textContent = "✅ READY";
    }
  });
  ["playerReadyStatus","scoreReadyStatus"].forEach(id => {
    const el = document.getElementById(id);
    if(el) el.textContent = isReady ? "✅ Je bent READY." : "Klik op READY als je klaar bent.";
  });
}

function listenReadyStatusFixed(){
  const room =
    currentRoomCode ||
    localStorage.getItem("hb_player_room") ||
    new URLSearchParams(window.location.search).get("room");
  ...
  if(!db || !room || !pid) return;

  db.ref("rooms/" + room.toUpperCase() + "/players/" + pid).on("value", snap => {
    updateReadyButtonsFixed(snap.val() || {});
  });
}

function hostReadyLockFixed(room){
  const startBtn = document.getElementById("startBtn");
  if(!startBtn) return;

  const round = room.currentRound || {};
  if(round.status === "answering" || round.status === "locked") return;

  const players = room.players || {};
  const list = Object.values(players);
  if(!list.length) return;

  const notReady = list.filter(p => !p.ready).map(p => p.name || "Speler");
...
setTimeout(() => {
  ["playerReadyBtn","scoreReadyBtn"].forEach(id => {
    ...
    if(btn){
      btn.onclick = null;
      btn.addEventListener("click", setPlayerReadyFixed);
    }
  });

  listenReadyStatusFixed();

  const hostRoom = currentRoomCode || localStorage.getItem("hb_host_room");
  if(db && hostRoom && !new URLSearchParams(window.location.search).get("room")){
    db.ref("rooms/" + hostRoom).on("value", snap => {
      hostReadyLockFixed(snap.val() || {});
    });
  }
}, 700);

const oldJoinRoomReadyFix = typeof joinRoom === "function" ? joinRoom : null;
if(oldJoinRoomReadyFix){
  joinRoom = function(re=false){
    oldJoinRoomReadyFix.apply(this, arguments);
    setTimeout(() => {
      const room = currentRoomCode || localStorage.getItem("hb_player_room");
      const pid = currentPlayerId || localStorage.getItem("hb_player_id");
      if(db && room && pid){
        db.ref("rooms/" + room + "/players/" + pid + "/ready").set(false);
        listenReadyStatusFixed();
      }
    }, 900);
  };
...
if(typeof markBingoCellV5 === "function"){
  const originalMarkBingoCellV5Ready = markBingoCellV5;
  markBingoCellV5 = function(index){
    const res = originalMarkBingoCellV5Ready.apply(this, arguments);
    setTimeout(setPlayerReadyFixed, 700);
    return res;
  };
}
...
/* =========================
   MEEDOEN HARD FIX
   Deze fix koppelt beide mogelijke Meedoen-knoppen opnieuw.
   ========================= */

function hbFixGetRoom(){
  const urlRoom = new URLSearchParams(window.location.search).get("room");
  return (urlRoom || currentRoomCode || localStorage.getItem("hb_player_room") || "").toUpperCase();
}

function hbFixGetName(){
  const screenInput = document.getElementById("screenNameInput");
  const oldInput = document.getElementById("playerNameInput");
  return ((screenInput && screenInput.value) || (oldInput && oldInput.value) || currentPlayerName || "").trim();
}

function hbFixPlayerId(){
  let pid = currentPlayerId || localStorage.getItem("hb_player_id");
  if(!pid){
    pid = "p_" + Math.random().toString(36).slice(2,10) + Date.now().toString(36).slice(-5);
  }
  return pid;
}

function hbFixGenCard(){
  if(typeof genCard === "function") return genCard();
...
function hbFixRenderCard(targetId, card, marked){
  const target = document.getElementById(targetId);
  if(!target) return;
...
function hbFixShowLobby(roomData){
  const players = (roomData && roomData.players) || {};
  const me = players[currentPlayerId] || {};

  // Nieuwe schermen
  const playerApp = document.getElementById("playerAppPanel") || document.getElementById("playerApp");
  if(playerApp) playerApp.classList.remove("hidden");

  const lobbyScreen = document.getElementById("playerScreenLobby") || document.getElementById("screenLobby");
  const joinScreen = document.getElementById("playerScreenJoin") || document.getElementById("screenJoin");

  if(lobbyScreen && joinScreen){
    document.querySelectorAll("#playerAppPanel .playerScreen, #playerApp .screen").forEach(s => s.classList.add("hidden"));
  ...
  if(lobbyPlayers){
    lobbyPlayers.innerHTML = Object.entries(players).map(([pid,p]) => `
      <div class="screenPlayerRow ${p.ready ? "ready" : "notReady"}">
        <strong>${esc(p.name || "Speler")}${pid===currentPlayerId ? " (jij)" : ""}</strong>
        <span>${p.ready ? "✅ READY" : "⏳ wacht"}</span>
      </div>
    `).join("") || "Nog geen spelers.";
  ...
  if(lobbyStatus) lobbyStatus.textContent = `${ready}/${total} spelers ready`;

  hbFixRenderCard("screenOwnCard", me.card || [], me.marked || {});
  hbFixRenderCard("ownCard", me.card || [], me.marked || {});

  // Oude schermen
  const oldJoin = document.getElementById("playerJoinPanel");
  const oldCard = document.getElementById("playerCardPanel");
  if(oldJoin) oldJoin.classList.add("hidden");
  if(oldCard){
    oldCard.classList.remove("hidden");
    const nameView = document.getElementById("playerNameView");
    if(nameView) nameView.textContent = "Speler: " + currentPlayerName;
    hbFixRenderCard("bingoCard", me.card || [], me.marked || {});
  }

  const readyBtn = document.getElementById("playerReadyBtn") || document.getElementById("readyBtn");
  if(readyBtn) readyBtn.classList.remove("hidden");
}

async function hbFixedJoinPlayer(){
  try{
    const room = hbFixGetRoom();
    const name = hbFixGetName();

    if(!room){
      alert("Geen spelcode gevonden. Open opnieuw de spelerlink van de host.");
      return;
    }
    if(!name){
      alert("Vul eerst je naam in.");
    ...
    await ref.update({
      ...
      joinedAt: existing.joinedAt || firebase.database.ServerValue.TIMESTAMP,
      lastSeen: firebase.database.ServerValue.TIMESTAMP,
      card: existing.card || hbFixGenCard(),
      marked: existing.marked || {}
    });
    ...
    ["screenJoinStatus","joinStatus"].forEach(id => {
      const el = document.getElementById(id);
      if(el) el.textContent = "Je doet mee!";
    });

    db.ref("rooms/" + currentRoomCode).on("value", s => {
      hbFixShowLobby(s.val() || {});
    });

    if(typeof listenReadyStatusFixed === "function") listenReadyStatusFixed();
...
function hbWireJoinButtons(){
  const buttons = ["screenJoinBtn","joinRoomBtn"];
  buttons.forEach(id => {
    const btn = document.getElementById(id);
    if(btn){
      const fresh = btn.cloneNode(true);
      btn.parentNode.replaceChild(fresh, btn);
      fresh.addEventListener("click", hbFixedJoinPlayer);
    }
  });
  ...
  if(room){
    ...
    const screenRoom = document.getElementById("screenRoomCode");
    if(screenRoom) screenRoom.textContent = room;

    const oldRoom = document.getElementById("playerRoomCode");
    if(oldRoom) oldRoom.textContent = room;
  }
}

document.addEventListener("DOMContentLoaded", hbWireJoinButtons);
setTimeout(hbWireJoinButtons, 300);
setTimeout(hbWireJoinButtons, 1000);
