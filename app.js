...
/* ==================================================
   BINGO BEATS V39 SCHONE DEFINITIEVE FIX
   - categorieën resetten nooit door Spotify/startpopup
   - oude picker wordt nooit meer aangeroepen
   - host/speler schermen worden NIET overschreven
   ================================================== */
(function(){
  const CAT_IDS = ["cat-yellow","cat-pink","cat-purple","cat-blue","cat-green"];

  function bb39SaveCats(){
    CAT_IDS.forEach(id=>{
      const el=document.getElementById(id);
      if(el) localStorage.setItem("bb_"+id, el.value || "");
    });
  ...
  function bb39LoadCats(){
    CAT_IDS.forEach(id=>{
      ...
      if(saved !== null){
        ...
        localStorage.setItem("bb_"+id, el.value);
      }
    });
  }

  function bb39BindCats(){
    CAT_IDS.forEach(id=>{
      const el=document.getElementById(id);
      if(!el || el.dataset.bb39cat==="1") return;
      el.dataset.bb39cat="1";
      ["input","change","blur"].forEach(ev=>el.addEventListener(ev,bb39SaveCats));
    });
  }

  function bb39ProtectSpotifyAndPopups(){
    document.querySelectorAll("button,a").forEach(el=>{
      const txt=(el.textContent||"").toLowerCase();
      const href=(el.getAttribute && (el.getAttribute("href")||"").toLowerCase()) || "";
      const isSpotify = txt.includes("spotify") || href.includes("spotify") || txt.includes("login");
      if(!isSpotify || el.dataset.bb39spotify==="1") return;
      el.dataset.bb39spotify="1";
      ["pointerdown","mousedown","touchstart","click"].forEach(ev=>{
        el.addEventListener(ev,()=>{
          bb39SaveCats();
          setTimeout(bb39LoadCats,0);
          setTimeout(bb39LoadCats,50);
          setTimeout(bb39LoadCats,200);
          setTimeout(bb39LoadCats,700);
          setTimeout(bb39LoadCats,1500);
        },true);
      });
    });
  }

  function bb39HideSound(){
    document.querySelectorAll("button,a,div,p,span").forEach(el=>{
      const txt=(el.textContent||"").toLowerCase();
      const id=(el.id||"").toLowerCase();
      const cls=(el.className||"").toString().toLowerCase();
      if(txt.includes("activeer host") || txt.includes("geluid nog niet geactiveerd") ||
         id.includes("sound") || id.includes("audio") || cls.includes("sound") || cls.includes("audio")){
        el.style.setProperty("display","none","important");
        el.style.setProperty("height","0","important");
        el.style.setProperty("min-height","0","important");
        el.style.setProperty("padding","0","important");
  ...
  function bb39Pick(arr){
    return typeof pick==="function" ? pick(arr) : arr[Math.floor(Math.random()*arr.length)];
  }

  function bb39PickAnimal(){
    const last=localStorage.getItem("bb39_last_animal")||"";
    const options=BB39_ANIMALS.filter(a=>a.key!==last);
    const chosen=bb39Pick(options.length?options:BB39_ANIMALS);
    localStorage.setItem("bb39_last_animal",chosen.key);
    return chosen;
  }

  function bb39AnimalHtml(key){
    if(key==="parrot"){
      return `<div class="bb39Animal bb39Parrot"><div class="bb39Shadow"></div><div class="wingL"></div><div class="wingR"></div><div class="tail"></div><div class="body"></div><div class="head"></div><div class="beak"></div><div class="eye"></div></div>`;
    }
    if(key==="turtle"){
      return `<div class="bb39Animal bb39Turtle"><div class="bb39Shadow"></div><div class="leg1"></div><div class="leg2"></div><div class="leg3"></div><div class="leg4"></div><div class="tail"></div><div class="shell"></div><div class="head"></div><div class="eye"></div></div>`;
    }
    return `<div class="bb39Animal bb39Monkey"><div class="bb39Shadow"></div><div class="tail"></div><div class="earL"></div><div class="earR"></div><div class="armL"></div><div class="armR"></div><div class="legL"></div><div class="legR"></div><div class="body"></div><div class="belly"></div><div class="head"></div><div class="face"></div><div class="eyeL"></div><div class="eyeR"></div><div class="mouth"></div></div>`;
  }

  function bb39PickerHtml(animalKey,colorKey,compact=false,reveal=false){
    const color=BB39_COLORS[colorKey]||BB39_COLORS.yellow;
    const animal=BB39_ANIMALS.find(a=>a.key===animalKey)||BB39_ANIMALS[0];

    return `<div class="bb39Picker ${compact?"compact":""}" data-bb39-picker="1">
      <div class="bb39Title">${reveal ? `${animal.name} koos de kleur!` : `${animal.name} start de kleurenmixer...`}</div>
      <div class="bb39Stage">
        <div class="bb39Sun"></div>
        <div class="bb39Palm">🌴</div>
        ${bb39AnimalHtml(animal.key)}
        <div class="bb39Lever"></div>
        <div class="bb39WheelWrap">
          <div class="bb39Pointer"></div>
          <div class="bb39Wheel ${reveal?"reveal":""}" style="--bb-final-rot:${color.rot}"></div>
          <div class="bb39WheelCenter"></div>
        </div>
        ${reveal ? `<div class="bb39Badge">${color.emoji} ${color.name}</div>` : ""}
      </div>
      <div class="bbHint">${reveal ? "Kleur gekozen!" : "Nog even spannend..."}</div>
    </div>`;
  }

  // Deze functies worden door de bestaande host/speler-flow gebruikt.
  window.pickerHTML = function(mode){
    return bb39PickerHtml(mode||"monkey","yellow",false,false);
  };

  window.sharedPickerHTML = function(mode){
    return bb39PickerHtml(mode||"monkey","yellow",true,false);
  };

  window.renderPlayerPicker = function(round){
    const animal = round.animalKey || round.animal || round.pickerMode || "monkey";
    const key = round.colorKey || round.targetColorKey || "yellow";

    const area = document.getElementById("playerPickerArea");
    if(area) area.innerHTML = bb39PickerHtml(animal,key,false,false);

    const dash = document.getElementById("dashPickerArea");
    if(dash) dash.innerHTML = bb39PickerHtml(animal,key,true,false);
  };
  ...
  window.renderCompactPicker = function(r){
    ...
    if(r.status === "picking"){
      area.innerHTML = bb39PickerHtml(r.animalKey||r.animal||r.pickerMode||"monkey", r.colorKey||r.targetColorKey||"yellow", true, false);
      return;
    }
  ...
  // Belangrijk: hbRenderPicker wordt in de compacte speler gebruikt. Deze overschrijven we ook.
  window.hbRenderPicker = function(r){
    window.renderCompactPicker(r);
  };
  if(typeof hbRenderPicker !== "undefined"){
    hbRenderPicker = window.hbRenderPicker;
  }

  window.startRoundVisual = function(room){
    bb39SaveCats();

    const animal = bb39PickAnimal();
    const color = bb39Pick(COLORS);
    const cat = document.getElementById(color.input).value || "Geen categorie";

    currentRoundId = "r_" + Date.now();

    const hostMarkup = bb39PickerHtml(animal.key,color.key,false,false);
    const playerMarkup = bb39PickerHtml(animal.key,color.key,true,false);

    const hostAnswer = document.getElementById("hostAnswerArea");
    ...
    setTimeout(()=>{
      ...
      if(typeof hbHostPickerPopupShowV7I === "function"){
        hbHostPickerPopupShowV7I(bb39PickerHtml(animal.key,color.key,false,true));
      }
      ...
      const score = document.getElementById("hostScorePanel");
      if(score) score.classList.remove("hidden");
  ...
  function bb39RepairOldPickerIfRendered(){
    ["hostPickerArea","dashPickerArea","playerPickerArea"].forEach(id=>{
      const el=document.getElementById(id);
      if(!el) return;
      ...
      const html=el.innerHTML || "";
      const old =
        html.includes("bbOldPickerRemoved") ||
        html.includes("dashDisco") ||
        html.includes("discoIcon") ||
        html.includes("dashWheel") ||
        html.includes("wheelWrap") ||
        html.includes("tropicalPicker") ||
        html.includes("tropicalIslandPicker") ||
        html.includes("v25IslandPicker");

      if(old && !html.includes("data-bb39-picker")){
        el.innerHTML = bb39PickerHtml("monkey","yellow",id!=="hostPickerArea",false);
      }
    });
  }
  ...
  bb39LoadCats();
  bb39BindCats();
  bb39ProtectSpotifyAndPopups();
  bb39HideSound();

  setTimeout(()=>{bb39LoadCats();bb39BindCats();bb39ProtectSpotifyAndPopups();bb39HideSound();bb39RepairOldPickerIfRendered();},100);
  setTimeout(()=>{bb39LoadCats();bb39ProtectSpotifyAndPopups();bb39HideSound();bb39RepairOldPickerIfRendered();},700);
  setTimeout(()=>{bb39LoadCats();bb39ProtectSpotifyAndPopups();bb39HideSound();bb39RepairOldPickerIfRendered();},1500);

  setInterval(()=>{
    bb39BindCats();
    bb39ProtectSpotifyAndPopups();
    bb39HideSound();
    bb39RepairOldPickerIfRendered();
  },1500);
})();
