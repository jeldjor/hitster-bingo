...
(function(){
  ...
  function localCardHTML(room,r,{pick=false,large=false}={}){
    const p=room?.players?.[currentPlayerId]||{}, c=p.card||[], marked=p.marked||{};
    return `<div class="bbStageBingo v108 ${large?'large':''}">${Array.from({length:25},(_,i)=>{
      ...
      return `<button type="button" class="bbStageCell v108cell ${isFree?'free':''} ${isMarked?'marked':''} ${canPick?'pickable':''}" data-i="${i}" style="--cell:${HEX[key]||'#777'};background:${HEX[key]||'#777'}" ${canPick?'':'tabindex="-1"'}>${content}</button>`;
    }).join('')}</div>`;
  }
  ...
  pickCell = function(i){
    db.ref('rooms/'+currentRoomCode+'/players/'+currentPlayerId).once('value').then(s=>{
      const p=s.val()||{}, marked=p.marked||{}, card=p.card||[];
      if(!activeRound?.id) return;
