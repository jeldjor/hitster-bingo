...
(function(){
  ...
  function animateLights(card,targetKey){
    ...
    const totalSteps=72+targetIdx;
    const t0=performance.now();
    function setActive(i){nodes.forEach(n=>n.classList.remove('active','final'));const n=nodes[((i%nodes.length)+nodes.length)%nodes.length];if(n)n.classList.add('active')}
    function frame(now){
      const t=Math.min(1,(now-t0)/INTRO_MS);
      const e=easeOutCubic(t);
      ...
      setActive(step);
      if(t<1) rafId=requestAnimationFrame(frame);
      else{nodes.forEach(n=>n.classList.remove('active'));nodes[targetIdx]?.classList.add('active','final');rafId=null;}
    }
    rafId=requestAnimationFrame(frame);
  }
