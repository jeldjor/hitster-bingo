...
let hostSelfPlayerId = localStorage.getItem("hb_host_self_pid") || "";
let hostSelfActive = localStorage.getItem("hb_host_play_mode") === "play";
let hostSelfTab = localStorage.getItem("hb_host_tab") || "control";

function hbHostModeEnsureUIV10(){
  if(new URLSearchParams(location.search).get("room")) return;

  const settingsPanel = Array.from(document.querySelectorAll("#hostApp > .panel")).find(p => p.textContent.includes("Instellingen"));
  if(settingsPanel && !document.getElementById("hostModeBox")){
    const box = document.createElement("div");
    box.id = "hostModeBox";
    box.className = "hostModeBox";
    box.innerHTML = `
      <strong>HOST MODUS</strong>
      <label class="hostModeChoice"><input type="radio" name="hostModeV10" id="hostModeOnlyV10" value="only"> Alleen host</label>
      <label class="hostModeChoice"><input type="radio" name="hostModeV10" id="hostModePlayV10" value="play"> Host speelt mee</label>
    `;
    settingsPanel.appendChild(box);
    document.getElementById("hostModeOnlyV10").addEventListener("change", () => hbSetHostPlayModeV10(false));
    document.getElementById("hostModePlayV10").addEventListener("change", () => hbSetHostPlayModeV10(true));
  }

  const hostApp = document.getElementById("hostApp");
  if(hostApp && !document.getElementById("hostTabs")){
    const tabs = document.createElement("div");
    tabs.id = "hostTabs";
    tabs.className = "hostTabs";
    tabs.innerHTML = `
      <button id="hostTabControlBtn" class="hostTabBtn">🎮 Besturing</button>
      <button id="hostTabSelfBtn" class="hostTabBtn">👤 Mijn kaart</button>
    `;
    hostApp.insertBefore(tabs, hostApp.firstChild);
    document.getElementById("hostTabControlBtn").addEventListener("click", () => hbSetHostTabV10("control"));
    document.getElementById("hostTabSelfBtn").addEventListener("click", () => hbSetHostTabV10("self"));
  }

  document.getElementById("hostJoinAsPlayerBtn")?.addEventListener("click", hbHostJoinAsPlayerV10);
  document.getElementById("hostSelfSubmitBtn")?.addEventListener("click", hbHostSubmitAnswerV10);

  hbApplyHostPlayModeV10();
}

function hbSetHostPlayModeV10(on){
  hostSelfActive = !!on;
  localStorage.setItem("hb_host_play_mode", on ? "play" : "only");
  if(on) hbSetHostTabV10(localStorage.getItem("hb_host_tab") || "control");
  hbApplyHostPlayModeV10();
}

function hbSetHostTabV10(tab){
  hostSelfTab = tab;
  localStorage.setItem("hb_host_tab", tab);
  hbApplyHostPlayModeV10();
}

function hbApplyHostPlayModeV10(){
  document.body.classList.toggle("hostPlayMode", hostSelfActive);
  document.body.classList.toggle("hostSelfTab", hostSelfActive && hostSelfTab === "self");
  document.body.classList.toggle("hostControlTab", !hostSelfActive || hostSelfTab === "control");

  const only = document.getElementById("hostModeOnlyV10");
  const play = document.getElementById("hostModePlayV10");
  if(only) only.checked = !hostSelfActive;
  if(play) play.checked = hostSelfActive;

  document.getElementById("hostTabControlBtn")?.classList.toggle("active", hostSelfTab === "control");
  document.getElementById("hostTabSelfBtn")?.classList.toggle("active", hostSelfTab === "self");

  const panel = document.getElementById("hostPlayerPanel");
  if(panel) panel.classList.toggle("hidden", !hostSelfActive);

  const nameInput = document.getElementById("hostPlayerNameInput");
  if(nameInput && !nameInput.value){
    nameInput.value = localStorage.getItem("hb_host_self_name") || localStorage.getItem("hb_player_name") || "";
  }
}

function hbHostRoomV10(){
  return currentRoomCode || localStorage.getItem("hb_host_room") || localStorage.getItem("hb_last_stable_room") || "";
}

function hbHostJoinAsPlayerV10(){
  const room = hbHostRoomV10();
  if(!room){
    alert("Maak of hervat eerst een kamer.");
    return;
  }

  const name = (document.getElementById("hostPlayerNameInput")?.value || "").trim();
  if(!name){
    alert("Vul je naam in.");
    return;
  }
  ...
  if(!hostSelfPlayerId){
    hostSelfPlayerId = "host_" + Math.random().toString(36).slice(2,10) + Date.now().toString(36).slice(-4);
    localStorage.setItem("hb_host_self_pid", hostSelfPlayerId);
  }

  localStorage.setItem("hb_host_self_name", name);

  const ref = db.ref("rooms/" + room + "/players/" + hostSelfPlayerId);
  ref.once("value").then(s => {
    const ex = s.val() || {};
    return ref.update({
      name,
    ...
    const st = document.getElementById("hostSelfStatus");
    if(st) st.textContent = "Je speelt mee als: " + name;
    hbListenHostSelfV10(room);
  });
}

function hbListenHostSelfV10(room){
  if(!room || !hostSelfPlayerId || window.__hbHostSelfListeningV10 === room) return;
  window.__hbHostSelfListeningV10 = room;

  db.ref("rooms/" + room).on("value", snap => {
    const data = snap.val() || {};
    hbRenderHostSelfV10(data);
  });
}

function hbRenderHostSelfV10(room){
  const me = room.players && room.players[hostSelfPlayerId] ? room.players[hostSelfPlayerId] : null;
  const r = room.currentRound || {};
  const status = document.getElementById("hostSelfStatus");

  if(!me){
    if(status) status.textContent = "Nog niet aangemeld als speler.";
    hbRenderCardHostSelfV10([], {});
    return;
  }

  if(status) status.textContent = `Je speelt mee als: ${me.name || "Host"}`;

  const answerBox = document.getElementById("hostSelfAnswerBox");
  const answerInfo = document.getElementById("hostSelfRoundInfo");
  const answerInput = document.getElementById("hostSelfAnswerInput");
  const submit = document.getElementById("hostSelfSubmitBtn");

  const ownAnswer = r.id && room.answers && room.answers[r.id] ? room.answers[r.id][hostSelfPlayerId] : null;

  if(answerBox){
    answerBox.classList.toggle("hidden", !(r && r.id && r.status === "answering"));
  }

  if(r && r.id && r.status === "answering"){
    if(answerInfo) answerInfo.textContent = `${r.colorEmoji || ""} ${r.colorName || ""} — ${r.category || ""}`;
    if(answerInput){
      answerInput.disabled = !!ownAnswer;
      if(ownAnswer) answerInput.value = ownAnswer.answer || "";
      else answerInput.value = "";
    }
    if(submit) submit.disabled = !!ownAnswer;
  }

  const good = r && r.id && room.correct && room.correct[r.id] ? room.correct[r.id][hostSelfPlayerId] === true : false;
  const picked = me.lastPickedRound === r.id;
  const canPick = r && r.status === "judged" && good && !picked;
  const pickInfo = document.getElementById("hostSelfPickInfo");
  if(pickInfo){
    pickInfo.textContent = canPick
      ? `Kies 1 ${r.colorEmoji || ""} ${r.colorName || ""} vakje.`
      : "Jouw bingokaart";
  }

  hbRenderCardHostSelfV10(me.card || [], me.marked || {}, canPick, r);
}

function hbRenderCardHostSelfV10(card, marked, click=false, r=null){
  const el = document.getElementById("hostSelfCard");
  if(!el) return;
  el.innerHTML = (card || []).map((c,i) => {
    ...
    return `<div class="bingoCell ${c==="free"?"free":""} ${pickable?"pickableCell":""} ${blocked?"blockedCell":""}" data-i="${i}">${m?"👑":emoji(c)}</div>`;
  }).join("");

  if(click){
    el.querySelectorAll(".pickableCell").forEach(x => {
      x.addEventListener("click", () => hbHostPickCellV10(Number(x.dataset.i)));
    });
  }
}

function hbHostSubmitAnswerV10(){
  const room = hbHostRoomV10();
  const val = document.getElementById("hostSelfAnswerInput")?.value || "";
  if(!room || !hostSelfPlayerId || !activeRound?.id){
    alert("Geen actieve ronde.");
    return;
  }

  db.ref("rooms/" + room + "/answers/" + activeRound.id + "/" + hostSelfPlayerId).set({
    answer:val,
    submittedAt:firebase.database.ServerValue.TIMESTAMP
  });
}

function hbHostPickCellV10(i){
  const room = hbHostRoomV10();
  if(!room || !hostSelfPlayerId || !activeRound?.id) return;

  db.ref("rooms/" + room + "/players/" + hostSelfPlayerId).once("value").then(s => {
    const p = s.val() || {};
    const marked = p.marked || {};
    const card = p.card || [];
    ...
    return db.ref("rooms/" + room + "/players/" + hostSelfPlayerId).update({
      marked,
      bingo,
...
// Host-self listener starten zodra kamer bestaat
function hbMaybeStartHostSelfV10(){
  hbHostModeEnsureUIV10();
  const room = hbHostRoomV10();
  if(hostSelfActive && room && hostSelfPlayerId){
    hbListenHostSelfV10(room);
  }
}

setTimeout(hbMaybeStartHostSelfV10, 500);
setTimeout(hbMaybeStartHostSelfV10, 1500);
setInterval(hbMaybeStartHostSelfV10, 3000);
