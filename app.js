...
/* ==================================================
   V44 DEFINITIEVE CATEGORIE OPSLAG
   Categorieën veranderen alleen door handmatige invoer.
   Startpopup/Spotify/nieuwe kamer mag ze niet terugzetten.
   ================================================== */
(function(){
  const FIELDS = ["cat-yellow","cat-pink","cat-purple","cat-blue","cat-green"];
  const KEY = id => "bb_category_permanent_" + id;
  const OLD_KEYS = id => ["bb_"+id, "perm_"+id, "bb_persist_"+id, "bb_locked_category_"+id, "bb_category_final_"+id];
  ...
  function getSaved(id){
    let v = localStorage.getItem(KEY(id));
    if(v !== null) return v;

    for(const k of OLD_KEYS(id)){
      const old = localStorage.getItem(k);
      if(old !== null){
        localStorage.setItem(KEY(id), old);
        return old;
      }
    }
    return null;
  ...
  function apply(){
    ...
    FIELDS.forEach(id=>{
      ...
      // Ook de HTML attribute aanpassen zodat popup/form reset dezelfde waarde gebruikt.
      if(el.getAttribute("value") !== saved){
        el.setAttribute("value", saved);
        el.defaultValue = saved;
      }
    });
  ...
  function bind(){
    FIELDS.forEach(id=>{
      const el = getEl(id);
      if(!el || el.dataset.bbV44Category === "1") return;
      el.dataset.bbV44Category = "1";

      // Zorg dat form reset niet teruggaat naar oude defaults.
      ...
      const commitManual = () => {
        ...
        setTimeout(()=>{ manual = false; }, 300);
      };

      ["focus","keydown","pointerdown","mousedown","touchstart"].forEach(ev=>{
        el.addEventListener(ev, startManual, true);
      });
      ["input","change","keyup","blur"].forEach(ev=>{
        el.addEventListener(ev, commitManual, true);
      });
      el.addEventListener("paste",()=>setTimeout(commitManual,0),true);

      const form = el.form;
      if(form && form.dataset.bbV44Form !== "1"){
        form.dataset.bbV44Form = "1";
  ...
  // Patch buttons/popups: save before, apply after.
  function patchClicks(){
    document.querySelectorAll("button,a").forEach(btn=>{
      if(btn.dataset.bbV44Click === "1") return;
      btn.dataset.bbV44Click = "1";
  ...
  // Patch startpopup specifically by observing body mutations and immediately applying defaults.
  ...
  function init(){
    ...
    apply();
    patchClicks();
    if(!window.__bbV44ObserverStarted){
      window.__bbV44ObserverStarted = true;
      observer.observe(document.body,{childList:true,subtree:true,attributes:false});
    }
  }

  document.addEventListener("DOMContentLoaded",init);
  window.addEventListener("pageshow",()=>setTimeout(init,20));
  window.addEventListener("focus",()=>setTimeout(init,20));
  document.addEventListener("visibilitychange",()=>{
    if(document.visibilityState === "hidden") saveAll();
    if(document.visibilityState === "visible") setTimeout(init,20);
  });
