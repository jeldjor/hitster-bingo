...
/* =========================
   V111 - AI Jury + compact scoreflow
   - Timer afgelopen: kort groot antwoordscherm
   - Daarna compact scorebord met AI-beoordeling voor iedereen
   - Goede spelers kiezen daarna vakje, foute spelers gaan lobby/READY
   - Host kan vóór volgende ronde live ✅/❌ aanpassen
   ========================= */
(function(){
  const q = id => document.getElementById(id);
  const E = s => String(s ?? '').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m]));
  ...
  function simplifyTrackTitle(s){
    let t = String(s||'');
    // Haal versies uit titel voor spelantwoord: Radio Edit, Remix, Live, Acoustic, etc.
    // Ook "(with ...)", "feat." en "ft." hoeven niet in het antwoord te staan.
    t = t.replace(/\s*[\(\[]\s*(with|feat\.?|ft\.?|featuring)\b[^\)\]]*[\)\]]/ig,'');
    t = t.replace(/\s*[\(\[][^\)\]]*\b(radio\s*edit|remix|mix|live|remaster(?:ed)?|edit|version|single|album|explicit|clean|acoustic|extended|instrumental|vip|club)\b[^\)\]]*[\)\]]/ig,'');
    t = t.replace(/\s*[-–—:]\s*\b(radio\s*edit|remix|mix|live|remaster(?:ed)?|edit|version|single|album|explicit|clean|acoustic|extended|instrumental|vip|club)\b.*$/ig,'');
    t = t.replace(/\s*[-–—:]\s*\b(with|feat\.?|ft\.?|featuring)\b.*$/ig,'');
    t = t.replace(/\s{2,}/g,' ').trim();
    return t;
  }
  function cleanText(s){
    let raw = simplifyTrackTitle(String(s||''));
    return stripDiacritics(raw.toLowerCase())
      .replace(/&/g,' and ')
      .replace(/\b(feat|ft|featuring|with)\b.*$/g,'')
      .replace(/\b(remaster(ed)?|radio edit|single version|album version|explicit|clean|remix|mix|live|edit|version|acoustic|extended|instrumental|club)\b/g,'')
      .replace(/[’'`´]/g,'')
      .replace(/[^a-z0-9]+/g,' ')
      .trim();
  }
  function phonetic(s){
    return cleanText(s)
      .replace(/\bkwien\b/g,'queen')
      .replace(/\bkween\b/g,'queen')
      .replace(/\bqween\b/g,'queen')
      .replace(/\bqeen\b/g,'queen')
      .replace(/\bkci\b/g,'k ci')
      .replace(/\bjo jo\b/g,'jojo');
  }
  function words(s){ return phonetic(s).split(/\s+/).filter(Boolean); }
  function joined(s){ return words(s).join(''); }
  function lev(a,b){
  ...
  const weakStart = new Set(['the','a','an','de','het','een']);
  function titleOk(answer, correct){
    const aw=words(answer), cw=words(correct);
    if(!aw.length || !cw.length) return false;
    if(joined(answer)===joined(correct)) return true;
    // Eén los woord of duidelijk onvolledig is fout bij meervoudige titels.
    if(cw.length >= 2 && aw.length < Math.max(2, Math.ceil(cw.length*0.72))) return false;
    // Als het eerste woord belangrijk is, moet dat ook aanwezig/duidelijk zijn.
    if(cw[0] && !weakStart.has(cw[0]) && !fuzzyWordOk(aw[0]||'',cw[0])) return false;
    if(Math.abs(aw.length-cw.length)>1) return false;
    let used = new Set(), good = 0;
    for(const c of cw){
      let best=-1, bestScore=-1;
      for(let i=0;i<aw.length;i++){
        if(used.has(i)) continue;
        const sc = sim(phonetic(aw[i]), phonetic(c));
        if(sc>bestScore){bestScore=sc; best=i;}
      }
      if(best>=0 && fuzzyWordOk(aw[best],c)){ used.add(best); good++; }
    }
    return good >= Math.max(2, Math.ceil(cw.length*0.82));
  }
  function artistOk(answer, correct){
    const aw=words(answer), cw=words(correct);
    if(!aw.length || !cw.length) return false;
    if(joined(answer)===joined(correct)) return true;
    if(Math.abs(aw.length-cw.length)>1) return false;
    ...
    for(const c of cw){
      let hit=-1;
      for(let i=0;i<aw.length;i++) if(!used.has(i) && fuzzyWordOk(aw[i],c)){ hit=i; break; }
      if(hit>=0){ used.add(hit); good++; }
    }
    return good >= Math.ceil(cw.length*0.86);
  }
  function parseYear(v, correctYear){
    let s=String(v||'').trim().toLowerCase();
    const m=s.match(/(?:^|\D)(\d{2}|\d{4})(?:\D|$)/);
    if(!m) return null;
    let y=Number(m[1]);
    if(m[1].length===2){
      const base = Math.floor(Number(correctYear||2000)/100)*100;
      const opts=[base+y,base-100+y,base+100+y];
      y = opts.sort((a,b)=>Math.abs(a-Number(correctYear))-Math.abs(b-Number(correctYear)))[0];
    }
    return y;
  }
  function decadeOf(y){ y=Number(y); if(!y) return null; return Math.floor(y/10)*10; }
  function decadeAnswer(v, correctYear){
    const s=cleanText(v);
    let m=s.match(/(?:jaren\s*)?(\d{2,4})s?/);
    if(!m) return null;
    let n=Number(m[1]);
    if(String(m[1]).length===2) n = (n<=30?2000+n:1900+n);
    return decadeOf(n);
  }
  function beforeAfterOk(answer, year){
    const s=cleanText(answer), y=Number(year||0);
    if(!y) return false;
    const isBefore = y < 2001;
    if(/\bvoor\b/.test(s)) return isBefore;
    if(/\bna\b/.test(s)) return !isBefore;
    if(/\bvoor\s*2001\b/.test(s)) return isBefore;
    if(/\bna\s*2001\b/.test(s)) return !isBefore;
    return false;
  }
  function questionType(category){
    ...
    if(c.includes('jaar')) return 'year';
    if(c.includes('voor') && c.includes('na')) return 'beforeafter';
    if(c.includes('titel') || c.includes('track') || c.includes('lied')) return 'title';
    return 'title';
  }
  function judgeOne(answer, r){
    const a = r?.correctAnswer || {};
    const type = questionType(r?.category || '');
    const val = String(answer||'').trim();
    if(!val) return false;
    if(type==='artist') return artistOk(val, a.artist||'');
    if(type==='year'){
      const cy = Number(a.year || String(a.release_date||'').slice(0,4));
      const y = parseYear(val, cy);
      return y!==null && Math.abs(y-cy)<=2;
    }
    if(type==='decade'){
      const cy = Number(a.year || String(a.release_date||'').slice(0,4));
      const d = decadeAnswer(val, cy);
      return d!==null && d===decadeOf(cy);
    }
    if(type==='beforeafter') return beforeAfterOk(val, a.year);
    return titleOk(val, simplifyTrackTitle(a.track||'') || (a.track||''));
  }
  function compactAnswer(r){
    const a=r?.correctAnswer||{};
    if(!a.track && !a.artist) return '';
    const title = simplifyTrackTitle(a.track||'-') || (a.track||'-');
    return `<div class="bbV111CorrectMini"><div class="bbV111Label">Juiste antwoord</div><strong>${E(title)}</strong><span>${E(a.artist||'-')}${a.year?' • '+E(a.year):''}</span></div>`;
  }
  function bigAnswerHTML(r){
    const a=r?.correctAnswer||{};
    const title = simplifyTrackTitle(a.track||'-') || (a.track||'-');
    return `<section class="bbV111BigAnswer"><img src="bb_logo.png" alt="Bingo Beats"><div>Juiste antwoord</div><h1>${E(title)}</h1><p>${E(a.artist||'-')}${a.year?' • '+E(a.year):''}</p></section>`;
  }
  function playerRows(room,r){
    const ps=Object.entries(room?.players||{}), ans=r?.id?(room.answers?.[r.id]||{}):{}, cor=r?.id?(room.correct?.[r.id]||{}):{};
    return `<div class="bbV111ScoreRows">${ps.map(([pid,p])=>{
      const text = String(ans[pid]?.answer||'').trim() || 'Geen antwoord';
      const good = cor[pid]===true;
      const bad = cor[pid]===false;
      return `<div class="bbV111ScoreRow ${good?'good':bad?'bad':'pending'}"><div class="bbAvatar">${E(String(p.name||'S').slice(0,1).toUpperCase())}</div><div><strong>${E(p.name||'Speler')}${pid===currentPlayerId?' <span>jij</span>':''}</strong><small>${E(text)}</small></div><b>${good?'✓':bad?'×':'…'}</b></div>`;
    }).join('')}</div>`;
  }
  function renderV111Score(root,room,r){
    root.className = 'compactDashboard bbStageDashboard bbV111ScoreScreen';
    root.innerHTML = `<section class="bbStageShell">${compactAnswer(r)}<h2>Scorebord</h2>${playerRows(room,r)}<p class="bbStageSub">Daarna kies je je bingovakje als je goed zat.</p></section>`;
  }
  function normalizeCardV113(raw, room){
    let arr = [];
    if(Array.isArray(raw)) arr = raw.slice(0,25);
    else if(raw && typeof raw === 'object') arr = Array.from({length:25},(_,i)=>raw[i] || raw[String(i)]);
    const valid = ['yellow','pink','purple','blue','green','free'];
    const ok = arr.length===25 && arr.every((x,i)=> i===12 ? (x==='free'||!x||valid.includes(x)) : valid.includes(x));
    if(ok){ arr[12]='free'; return arr; }
    const fresh = (typeof genCard === 'function') ? genCard() : Array.from({length:25},(_,i)=>i===12?'free':['yellow','pink','purple','blue','green'][i%5]);
    try{
      if(currentRoomCode && currentPlayerId && db){
        setTimeout(()=>db.ref('rooms/'+currentRoomCode+'/players/'+currentPlayerId+'/card').set(fresh),0);
      }
    }catch(_e){}
    return fresh;
  }
  function localCardHTML(room,r,{pick=false,large=false}={}){
    const p=room?.players?.[currentPlayerId]||{}, c=normalizeCardV113(p.card, room), marked=p.marked||{};
    return `<div class="bbStageBingo v108 ${large?'large':''}">${Array.from({length:25},(_,i)=>{
      const raw=c[i] || (i===12?'free':'yellow');
      const key=(i===12||raw==='free')?'free':raw;
      const isFree=key==='free', isMarked=!!marked[i]||isFree;
      const canPick=pick && r?.id && key===r.colorKey && !isMarked && !isFree;
      const content=isFree||isMarked?'<span class="bbMiniMark">BB</span>':'';
      return `<button type="button" class="bbStageCell v108cell cell-${E(key)} ${isFree?'free':''} ${isMarked?'marked':''} ${canPick?'pickable':''}" data-i="${i}" style="--cell:${HEX[key]||'#777'};background:${HEX[key]||'#777'}" ${canPick?'':'tabindex="-1"'}>${content}</button>`;
    }).join('')}</div>`;
  }
  function renderV111Pick(root,room,r){
    root.className = 'compactDashboard bbStageDashboard stagePickCard';
    root.innerHTML = `<section class="bbStageShell"><h2>GOED ANTWOORD!</h2><p class="bbStageSub">Kies één <strong>${E(r.colorName||COLOR_LABEL[r.colorKey]||'kleur')}</strong> vakje</p>${localCardHTML(room,r,{large:true,pick:true})}<p class="bbStageSub bottom">Tik op een opgelicht vakje</p></section>`;
    root.querySelectorAll('.pickable').forEach(cell=>cell.addEventListener('click',()=>pickCell(Number(cell.dataset.i))));
  }
  function renderV111Lobby(root,room,msg='Wachten op READY'){
    const ps=Object.entries(room?.players||{});
    const p=room?.players?.[currentPlayerId]||{};
    root.className = 'compactDashboard bbStageDashboard stageLobby';
    root.innerHTML = `<section class="bbStageShell"><img src="bb_logo.png" class="bbStageLogo" alt="Bingo Beats"><h2>${E(msg)}</h2><div class="bbStageReadyList v108">${ps.map(([pid,pl])=>`<div class="bbReadyRow ${pl.ready?'ready':'wait'}"><div class="bbAvatar">${E(String(pl.name||'S').slice(0,1).toUpperCase())}</div><div class="bbReadyName">${E(pl.name||'Speler')}${pid===currentPlayerId?' <span>jij</span>':''}</div><div class="bbReadyState">${pl.ready?'READY':'Wacht'}</div></div>`).join('')}</div><div class="bbCardWrap"><h3>Jouw bingokaart</h3>${localCardHTML(room,null,{large:false})}</div>${p.ready?'<button class="bbReadyBig isReady" disabled>READY ✓</button>':'<button id="bbV111Ready" class="bbReadyBig">READY</button>'}</section>`;
    q('bbV111Ready')?.addEventListener('click',()=>db.ref('rooms/'+currentRoomCode+'/players/'+currentPlayerId+'/ready').set(true));
  }

  // Timer afgelopen: publiceer correct antwoord, AI-jury en kort antwoordmoment. Daarna automatisch compact scorebord.
  lockRound = async function(){
    if(!currentRoomCode) return;
    try{
      await stopPlayback().catch(()=>{});
      const snap = await db.ref('rooms/'+currentRoomCode).once('value');
      const room = snap.val() || {}, r0 = room.currentRound || {};
      if(!r0.id) return;
      const ansObj = answerObject();
      if(!ansObj) return;
      const r = {...r0, correctAnswer:ansObj};
      const answers = room.answers?.[r.id] || {};
      const up = {};
      Object.keys(room.players||{}).forEach(pid=>{
        const val = answers[pid]?.answer || '';
        up[`rooms/${currentRoomCode}/correct/${r.id}/${pid}`] = judgeOne(val,r);
      });
      up[`rooms/${currentRoomCode}/currentRound/correctAnswer`] = ansObj;
      up[`rooms/${currentRoomCode}/currentRound/correctAnswerShown`] = true;
      up[`rooms/${currentRoomCode}/currentRound/status`] = 'revealing';
      up[`rooms/${currentRoomCode}/currentRound/revealUntilMs`] = Date.now() + REVEAL_MS;
      up[`rooms/${currentRoomCode}/currentRound/scoreUntilMs`] = Date.now() + REVEAL_MS + SCORE_MS;
      await db.ref().update(up);
      showHostAnswer(ansObj);
      if(q('hostStatus')) q('hostStatus').textContent = 'AI Jury beoordeelt antwoorden automatisch.';
      setTimeout(()=>{
        db.ref('rooms/'+currentRoomCode+'/currentRound').once('value').then(s=>{
          const r=s.val()||{};
          if(r.id===r0.id && r.status==='revealing') db.ref('rooms/'+currentRoomCode+'/currentRound/status').set('judged');
        });
      }, REVEAL_MS);
    }catch(e){ alert('AI Jury/antwoord fout: '+(e.message||e)); }
  };

  showAnswer = function(){ lockRound(); };
  publishResults = function(){ lockRound(); };

  // Host-scorebord: compact correct antwoord + directe toggle. Correcties werken live.
  renderHostScore = function(room){
    const r=room.currentRound||{};
    if(!r.id || !q('hostScoreboard')) return;
    q('hostScorePanel')?.classList.remove('hidden');
    if(q('hostRoundInfo')) q('hostRoundInfo').innerHTML = `${r.colorEmoji||''} ${E(r.colorName||'')} — ${E(r.category||'')} — ${E(r.status||'')}`;
    const ps=room.players||{}, ans=room.answers?.[r.id]||{}, cor=room.correct?.[r.id]||{};
    let html = (r.correctAnswer ? `<div class="bbV111HostAnswer">⭐ <strong>${E(simplifyTrackTitle(r.correctAnswer.track||'-') || (r.correctAnswer.track||'-'))}</strong><br><span>${E(r.correctAnswer.artist||'-')}${r.correctAnswer.year?' • '+E(r.correctAnswer.year):''}</span></div>` : '');
    html += Object.entries(ps).map(([pid,p])=>{
      const st=cor[pid], cls=st===true?'scoreGood':st===false?'scoreBad':'scorePending';
      return `<div class="scoreCard ${cls}"><div>${E(p.name||'Speler')}</div><div>${E(ans[pid]?.answer||'Geen antwoord')}</div><div><button type="button" class="goodBtn ${st===true?'goodSelected':''}" data-pid="${pid}" data-good="true">✅</button><button type="button" class="badBtn ${st===false?'badSelected':''}" data-pid="${pid}" data-good="false">❌</button></div></div>`;
    }).join('');
    q('hostScoreboard').innerHTML=html;
  };
  ...
  scoreboardClick = function(e){
    ...
    db.ref('rooms/'+currentRoomCode).once('value').then(async s=>{
      const room=s.val()||{}, r=room.currentRound||{}, p=room.players?.[pid]||{};
      if(!r.id) throw Error('Geen actieve ronde.');
      const up={};
      up[`rooms/${currentRoomCode}/correct/${r.id}/${pid}`]=good;
      if(good){
        // Was fout en wordt goed: speler moet vakje kunnen kiezen en opnieuw READY klikken.
        if(p.lastPickedRound !== r.id){ up[`rooms/${currentRoomCode}/players/${pid}/ready`]=false; }
      }else{
        // Was goed en wordt fout: eventueel gekozen vakje uit deze ronde terugdraaien.
        const cell = p.lastPickedCell;
        if(p.lastPickedRound === r.id && cell !== undefined && cell !== null){
          up[`rooms/${currentRoomCode}/players/${pid}/marked/${cell}`]=null;
          up[`rooms/${currentRoomCode}/players/${pid}/lastPickedRound`]=null;
          up[`rooms/${currentRoomCode}/players/${pid}/lastPickedCell`]=null;
          up[`rooms/${currentRoomCode}/players/${pid}/bingo`]=false;
          up[`rooms/${currentRoomCode}/players/${pid}/ready`]=false;
        }
      }
      await db.ref().update(up);
    }).then(()=>{ if(q('hostStatus')) q('hostStatus').textContent='Beoordeling aangepast.'; }).catch(e=>alert(e.message||e));
  };

  // Vakje kiezen onthoudt welke cel bij deze ronde hoorde, zodat host-correctie hem terug kan draaien.
  pickCell = function(i){
    db.ref('rooms/'+currentRoomCode+'/players/'+currentPlayerId).once('value').then(s=>{
      const p=s.val()||{}, marked=p.marked||{}, card=normalizeCardV113(p.card);
      if(!activeRound?.id) return;
      if(card[i]!==activeRound.colorKey || marked[i]) return;
      marked[i]=true;
      const bingo=checkBingo(marked);
      return db.ref('rooms/'+currentRoomCode+'/players/'+currentPlayerId).update({marked,bingo,lastPickedRound:activeRound.id,lastPickedCell:i,ready:false}).then(()=>{
        if(bingo) return db.ref('rooms/'+currentRoomCode+'/bingos').push({name:currentPlayerName,roundId:activeRound.id,at:firebase.database.ServerValue.TIMESTAMP});
      });
    });
  };
  ...
  renderCompactDashboard = function(room,r){
    ...
    if(r?.status === 'revealing'){
      root.className='compactDashboard bbStageDashboard bbV111RevealScreen';
      root.innerHTML=bigAnswerHTML(r);
      return;
    }
    if(r?.status === 'judged'){
      const now=Date.now();
      if(r.scoreUntilMs && now < Number(r.scoreUntilMs)) return renderV111Score(root,room,r);
      const good = room.correct?.[r.id]?.[currentPlayerId] === true;
      const p = room.players?.[currentPlayerId] || {};
      const picked = p.lastPickedRound === r.id;
      if(good && !picked) return renderV111Pick(root,room,r);
      return renderV111Lobby(root,room, good ? 'Vakje gekozen' : 'Helaas!');
    }
...
/* =========================
   V112 - titelversie schoon + scorebord door naar lobby
   - Radio Edit / Remix / Live / with... niet nodig in antwoord
   - Kort scorebord blijft niet hangen; daarna automatisch lobby/vakje kiezen
   ========================= */
(function(){
  const q = id => document.getElementById(id);
  function scheduleRerenderV112(room,r,ms){
    if(!r?.id || !q('screenDashboard')) return;
    clearTimeout(window.__bbV112ScoreTimer);
    window.__bbV112ScoreTimer = setTimeout(()=>{
      try{ if(typeof renderCompactDashboard === 'function') renderCompactDashboard(room,r); }catch(_e){}
    }, Math.max(250, ms||250));
  }
  if(typeof renderCompactDashboard === 'function' && !window.__bbV112RenderWrapped){
    window.__bbV112RenderWrapped = true;
    const prev = renderCompactDashboard;
    renderCompactDashboard = function(room,r){
      if(r?.status === 'judged'){
        const root=q('screenDashboard');
        if(root && r.scoreUntilMs){
          const left = Number(r.scoreUntilMs) - Date.now();
          if(left > 0){
            const res = prev(room,r);
            scheduleRerenderV112(room,r,left+80);
            return res;
          }
        }
      }
