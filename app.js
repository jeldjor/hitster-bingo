...
(function(){
  ...
  // Visuele aftelling live updaten zonder volledige re-render. Geen geluid, alleen korte trilling bij GO.
  let lastGoV115 = '';
  setInterval(()=>{
    try{
      const r = activeRound || {};
      ...
      if(!left && r.id && lastGoV115 !== r.id){
        lastGoV115 = r.id;
        try{ if(navigator.vibrate) navigator.vibrate(70); }catch(_e){}
      }
    }catch(e){}
  }, 180);
})();
...
/* =========================
   V122 - DEFINITIEVE RAD LOCK FIX
   - Laatste code wint: alle oude radfuncties verwijzen naar 1 nieuwe render
   - Host + speler krijgen exact dezelfde 60-vaks rad HTML
   - Het rad wordt per ronde/index maar 1x gerenderd, dus geen doorschieten na stoppen
   - Als oude code toch het oude rad terugzet, zet V122 het terug naar het nieuwe rad
   ========================= */
(function(){
  const q = id => document.getElementById(id);
  ...
  function normIndex(i){
    i = Number(i);
    return Number.isFinite(i) ? Math.max(0, Math.min(COUNT - 1, Math.round(i))) : 0;
  }
  function firstIndexForKey(key){
    const arr = WHEEL.map((k,i)=>k===key?i:-1).filter(i=>i>=0);
    return arr.length ? arr[Math.floor(arr.length / 2)] : 0;
  }
  function colorObj(key){
    const base = (typeof COLORS !== 'undefined' ? COLORS.find(c=>c.key===key) : null) || {};
  ...
  function gradient(){
    return 'conic-gradient(from 0deg,' + WHEEL.map((key,i)=>`${HEX[key]} ${(i*SLICE).toFixed(2)}deg ${((i+1)*SLICE).toFixed(2)}deg`).join(',') + ')';
  }
  function stopForIndex(index){
    index = normIndex(index);
    const center = index * SLICE + SLICE / 2;
  ...
  function wheelHTML(index, roundId){
    index = normIndex(index);
    const key = WHEEL[index] || 'yellow';
    return `<div class="bbWheelV122" data-round="${esc2(roundId||'')}" data-index="${index}" data-key="${esc2(key)}">
      <div class="bbWheelPointerV122"></div>
      <div class="bbWheelDiscV122" style="--stop:${stopForIndex(index)}deg;--chosen:${HEX[key]};background:${gradient()}">
        <div class="bbWheelHubV122"></div>
      </div>
      <div class="bbWheelTextV122">Categorie wordt gekozen...</div>
    </div>`;
  }
  function resultHTML(color, cat){
    const hex = color?.hex || HEX[color?.key] || '#FFCC33';
    return `<div class="bbWheelResultV122" style="--result:${hex}">
      <div class="bbWheelResultDotV122"></div>
      <div class="bbWheelResultNameV122">${esc2(color?.emoji||'')} ${esc2(color?.name||'KLEUR')}</div>
      <div class="bbWheelResultCatV122">${esc2(cat||'')}</div>
    </div>`;
  }
  function overlay(){
    let o = q('bbWheelFullOverlayV92');
  ...
  function showWheel(index, roundId){
    index = normIndex(index);
    const k = String(roundId||'') + ':' + index;
    const o = overlay();
    if(lastOverlayKey !== k || !o.querySelector('.bbWheelV122')){
      o.innerHTML = wheelHTML(index, roundId);
      lastOverlayKey = k;
    }
    o.classList.remove('hidden');
    o.classList.add('show');
  ...
  function showResult(color, cat){
    const o = overlay();
    o.innerHTML = resultHTML(color, cat);
    lastOverlayKey = 'result:' + (color?.key||'') + ':' + (cat||'');
    o.classList.remove('hidden');
    o.classList.add('show');
    o.style.display = 'flex';
    o.style.zIndex = '2147483647';
  ...
  function writeHostWheel(index, roundId){
    const el = q('hostPickerArea');
    if(!el) return;
    const k = String(roundId||'') + ':' + normIndex(index);
    if(lastHostKey !== k || !el.querySelector('.bbWheelV122')){
      el.innerHTML = wheelHTML(index, roundId);
      lastHostKey = k;
    }
  }
  function writeHostResult(color, cat){
    const el = q('hostPickerArea');
    if(!el) return;
    el.innerHTML = resultHTML(color, cat);
    lastHostKey = 'result:' + (color?.key||'') + ':' + (cat||'');
  }

  // Laat ALLE oude calls naar het nieuwe rad lopen.
  window.__bbWheelV122 = {showWheel,showResult,hideWheel,wheelHTML,resultHTML,WHEEL};
  window.__bbWheelV92 = {showWheelFull:(x)=>showWheel(Number.isFinite(Number(x))?Number(x):firstIndexForKey(x||'yellow'), ''), showWheelResult:showResult, hideWheelFull:hideWheel};
  window.__bbWheelV110 = {showWheelV110:(x)=>showWheel(Number.isFinite(Number(x))?Number(x):firstIndexForKey(x||'yellow'), ''), showResultV110:showResult, hideWheelV110:hideWheel, wheelHTMLV110:(x)=>wheelHTML(Number.isFinite(Number(x))?Number(x):firstIndexForKey(x||'yellow'), '')};
  window.__bbWheelV119 = {showWheel:(x)=>showWheel(Number(x)||0, ''), showResult, hideWheel, wheelHTML:(x)=>wheelHTML(Number(x)||0, ''), resultHTML, WHEEL_60:WHEEL};
  if(typeof pickerHTML === 'function') pickerHTML = function(){return wheelHTML(0,'')};

  // Eén echte startRoundVisual. Geen kleur achteraf opnieuw kiezen.
  startRoundVisual = function(room){
    if(q('hostAnswerArea')) q('hostAnswerArea').innerHTML = '';
    if(q('playBtn')){q('playBtn').disabled = true; q('playBtn').textContent = '🎵 Nummer start zo...';}
    if(q('showAnswerBtn')) q('showAnswerBtn').disabled = true;
    ...
    showWheel(winningIndex, currentRoundId);
    writeHostWheel(winningIndex, currentRoundId);

    db.ref('rooms/'+room+'/currentRound').set({
      id: currentRoundId,
      status: 'picking',
      pickerMode: 'wheelV122',
      wheelSegmentCount: COUNT,
      winningIndex,
      pendingColorKey: color.key,
      pendingColorName: color.name,
  ...
  // Laatste render-wacht: als picking actief is, V122 blijft winnen over oude renderers.
  const prevRender = typeof renderCompactDashboard === 'function' ? renderCompactDashboard : null;
  renderCompactDashboard = function(room, r){
    if(r && r.status === 'picking'){
      const idx = Number.isFinite(Number(r.winningIndex)) ? normIndex(r.winningIndex) : firstIndexForKey(r.pendingColorKey || r.colorKey || 'yellow');
      const root = q('screenDashboard');
      if(root){
        document.body.classList.add('bbStageMode');
        root.className = 'compactDashboard bbStageDashboard stageWheelV122';
        root.innerHTML = `<section class="bbWheelInlineV122">${wheelHTML(idx, r.id||'')}</section>`;
      }
      showWheel(idx, r.id||'');
      return;
    }
  ...
  function activeRoomCode(){return (currentRoomCode || new URLSearchParams(location.search).get('room') || localStorage.hb_player_room || localStorage.hb_host_room || '').toUpperCase();}
  function syncFromRoom(room){
    ...
    db.ref('rooms/'+room+'/currentRound').once('value').then(s=>{
      ...
      if(r.status === 'picking'){
        const idx = Number.isFinite(Number(r.winningIndex)) ? normIndex(r.winningIndex) : firstIndexForKey(r.pendingColorKey || r.colorKey || 'yellow');
        showWheel(idx, r.id||'');
        if(!document.body.classList.contains('playerMode')) writeHostWheel(idx, r.id||'');
  ...
  function attach(){
    try{
      ...
      if(!room || room === activeRoom) return;
      if(activeRoom) db.ref('rooms/'+activeRoom+'/currentRound').off('value.bbWheelV122Final');
      activeRoom = room;
      db.ref('rooms/'+room+'/currentRound').on('value.bbWheelV122Final', s=>{
        const r = s.val() || {};
        if(r.status === 'picking'){
          const idx = Number.isFinite(Number(r.winningIndex)) ? normIndex(r.winningIndex) : firstIndexForKey(r.pendingColorKey || r.colorKey || 'yellow');
          showWheel(idx, r.id||'');
  ...
  setInterval(()=>{attach(); syncFromRoom(activeRoomCode());}, 500);
})();

/* =========================
   V123 - ENIG RAD / WHEEL60 ONLY
   - Oude rad-overlays worden tijdens picking actief verborgen.
   - Host en speler renderen hetzelfde 60-vaks rad vanuit één component.
   - Rad duurt 12 seconden.
   - winningIndex wordt 1x per ronde gekozen en daarna niet meer aangepast.
   ========================= */
(function(){
  const q = id => document.getElementById(id);
  const E = s => (typeof esc === 'function' ? esc(String(s ?? '')) : String(s ?? '').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m])));
  ...
  const COUNT = WHEEL60.length;
  const SLICE = 360 / COUNT;
  let currentPickKey = '';
  let listenRoom = '';

  function normIndex(v){ v = Number(v); return Number.isFinite(v) ? Math.max(0, Math.min(COUNT - 1, Math.round(v))) : 0; }
  function colorObj(key){
    const base = (typeof COLORS !== 'undefined' ? COLORS.find(c=>c.key===key) : null) || {};
    return {key, name:base.name||NAME[key]||'KLEUR', emoji:base.emoji||EMOJI[key]||'', hex:base.hex||HEX[key]||'#FFCC33', input:base.input||INPUT[key]};
  }
  function categoryFor(color){ try{return q(color.input)?.value || 'Geen categorie';}catch(_e){return 'Geen categorie';} }
  function wheelGradient(){
    return 'conic-gradient(from -90deg,' + WHEEL60.map((key,i)=>`${HEX[key]} ${(i*SLICE).toFixed(2)}deg ${((i+1)*SLICE).toFixed(2)}deg`).join(',') + ')';
  }
  function stopDeg(index){
    index = normIndex(index);
    const center = (index * SLICE) + (SLICE / 2);
    // Eindigt met het gekozen segment precies onder de pijl bovenaan.
    return -center;
  }
  function oldWheelOverlay(){ return q('bbWheelFullOverlayV92'); }
  function hideOldWheel(){
    const old = oldWheelOverlay();
    if(old){ old.classList.add('hidden'); old.classList.remove('show'); old.style.display='none'; old.innerHTML=''; }
  }
  function overlay(){
    let o = q('bbWheelV123Overlay');
    if(!o){ o = document.createElement('div'); o.id='bbWheelV123Overlay'; o.className='bbWheelV123Overlay hidden'; document.body.appendChild(o); }
    return o;
  }
  function wheelHTML(index, roundId=''){
    index = normIndex(index);
    const key = WHEEL60[index] || 'yellow';
    return `<div class="bbWheelV123" data-round="${E(roundId)}" data-index="${index}" data-key="${E(key)}">
      <div class="bbWheelPointerV123"></div>
      <div class="bbWheelDiscV123" style="--bb-stop:${stopDeg(index)}deg;--bb-chosen:${HEX[key]};background:${wheelGradient()}">
        <div class="bbWheelHubV123"></div>
      </div>
      <div class="bbWheelTextV123">Categorie wordt gekozen...</div>
    </div>`;
  }
  function resultHTML(color, cat){
    const hex = color?.hex || HEX[color?.key] || '#FFCC33';
    return `<div class="bbWheelResultV123" style="--bb-result:${hex}">
      <div class="bbWheelResultDotV123"></div>
      <div class="bbWheelResultNameV123">${E(color?.emoji||'')} ${E(color?.name||'KLEUR')}</div>
      <div class="bbWheelResultCatV123">${E(cat||'')}</div>
    </div>`;
  }
  function showWheel(index, roundId=''){
    index = normIndex(index);
    hideOldWheel();
    const key = `wheel:${index}`;
    const o = overlay();
    if(currentPickKey !== key || !o.querySelector('.bbWheelV123')){
      o.innerHTML = wheelHTML(index, roundId);
      currentPickKey = key;
    }
    o.classList.remove('hidden');
  ...
  function showResult(color, cat){
    ...
    const o = overlay();
    o.innerHTML = resultHTML(color, cat);
    currentPickKey = 'result:' + (color?.key||'') + ':' + (cat||'');
    o.classList.remove('hidden');
    o.classList.add('show');
    o.style.display='flex';
    o.style.zIndex='2147483647';
  ...
  function hideWheel(){
    const o = q('bbWheelV123Overlay');
    if(o){ o.classList.remove('show'); o.classList.add('hidden'); o.style.display='none'; }
    currentPickKey = '';
    hideOldWheel();
  }
  function renderHostWheel(index, roundId=''){
    const el = q('hostPickerArea');
    if(!el) return;
    const key = `wheel:${normIndex(index)}`;
    // Host toont hetzelfde grote overlay-rad; in het hostpaneel alleen een stabiele status, geen tweede draaiend rad.
    if(el.dataset.bbWheelKey !== key){
      el.innerHTML = '<div class="bbWheelHostStatusV124">Categorie wordt gekozen...</div>';
      el.dataset.bbWheelKey = key;
    }
  }
  function renderHostResult(color, cat){
    const el = q('hostPickerArea');
    if(!el) return;
    el.innerHTML = resultHTML(color, cat);
    el.dataset.bbWheelKey = 'result:' + (color?.key||'') + ':' + (cat||'');
  }
  function indexFromKey(key){
    const matches = WHEEL60.map((k,i)=>k===key?i:-1).filter(i=>i>=0);
    return matches.length ? matches[Math.floor(matches.length/2)] : 0;
  }

  // Eén publieke wheel-API: oude calls worden niet meer gerenderd, maar doorgeleid naar Wheel60.
  window.__bbWheelV123 = {showWheel, showResult, hideWheel, wheelHTML, resultHTML, WHEEL60, DURATION_MS};
  window.__bbWheelV92 = {showWheelFull:(x)=>showWheel(Number.isFinite(Number(x))?Number(x):indexFromKey(x||'yellow'), ''), showWheelResult:showResult, hideWheelFull:hideWheel};
  window.__bbWheelV110 = {showWheelV110:(x)=>showWheel(Number.isFinite(Number(x))?Number(x):indexFromKey(x||'yellow'), ''), showResultV110:showResult, hideWheelV110:hideWheel, wheelHTMLV110:(x)=>wheelHTML(Number.isFinite(Number(x))?Number(x):indexFromKey(x||'yellow'), '')};
  window.__bbWheelV119 = {showWheel:(x)=>showWheel(Number(x)||0,''), showResult, hideWheel, wheelHTML:(x)=>wheelHTML(Number(x)||0,''), resultHTML, WHEEL_60:WHEEL60};
  if(typeof pickerHTML === 'function') pickerHTML = function(){ return wheelHTML(0,''); };

  startRoundVisual = function(room){
    if(q('hostAnswerArea')) q('hostAnswerArea').innerHTML = '';
    if(q('hostScoreboard')) q('hostScoreboard').innerHTML = '';
    if(q('hostRoundInfo')) q('hostRoundInfo').textContent = '';
    q('hostScorePanel')?.classList.add('hidden');
    if(q('playBtn')){ q('playBtn').disabled = true; q('playBtn').textContent = '🎵 Nummer start zo...'; }
    if(q('showAnswerBtn')) q('showAnswerBtn').disabled = true;

    const winningIndex = Math.floor(Math.random() * COUNT);
    const colorKey = WHEEL60[winningIndex];
    const color = colorObj(colorKey);
    const cat = categoryFor(color);
    currentRoundId = 'r_' + Date.now();

    showWheel(winningIndex, currentRoundId);
    renderHostWheel(winningIndex, currentRoundId);

    db.ref('rooms/'+room+'/currentRound').set({
      id: currentRoundId,
      status: 'picking',
      pickerMode: 'wheelV123',
      wheelSegmentCount: COUNT,
      wheelDurationMs: DURATION_MS,
      winningIndex,
      pendingColorKey: color.key,
      pendingColorName: color.name,
      pendingCategory: cat,
    ...
    setTimeout(()=>{
      try{ if(typeof flash === 'function') flash(); }catch(_e){}
      showResult(color, cat);
      renderHostResult(color, cat);
      const ends = Date.now() + PRECOUNT_MS;
      db.ref('rooms/'+room+'/currentRound').set({
        id: currentRoundId,
        status: 'precount',
        pickerMode: 'wheelV123',
        ...
        q('hostScorePanel')?.classList.remove('hidden');
        if(q('hostStatus')) q('hostStatus').textContent = 'Categorie gekozen. 3-2-1... daarna start het nummer.';
        setTimeout(()=>{ hideWheel(); if(typeof playHidden === 'function') playHidden(); }, PRECOUNT_MS);
      });
  ...
  renderCompactDashboard = function(room, r){
    if(r && r.status === 'picking'){
      const idx = Number.isFinite(Number(r.winningIndex)) ? normIndex(r.winningIndex) : indexFromKey(r.pendingColorKey || r.colorKey || 'yellow');
      const root = q('screenDashboard');
      if(root){
        document.body.classList.add('bbStageMode');
        root.className = 'compactDashboard bbStageDashboard stageWheelV123';
        const k = 'wheel:' + idx;
        if(root.dataset.bbWheelScreenKey !== k){
          // Geen tweede inline-rad meer: dat veroorzaakte groot/klein flikkeren.
          root.innerHTML = `<section class="bbWheelPlayerStatusV124">Categorie wordt gekozen...</section>`;
          root.dataset.bbWheelScreenKey = k;
        }
      }
      showWheel(idx, r.id||'');
      return;
  ...
  function activeRoomCode(){ return (currentRoomCode || new URLSearchParams(location.search).get('room') || localStorage.hb_player_room || localStorage.hb_host_room || '').toUpperCase(); }
  function applyRoundState(r){
    if(r && r.status === 'picking'){
      const idx = Number.isFinite(Number(r.winningIndex)) ? normIndex(r.winningIndex) : indexFromKey(r.pendingColorKey || r.colorKey || 'yellow');
      showWheel(idx, r.id||'');
      if(!document.body.classList.contains('playerMode')) renderHostWheel(idx, r.id||'');
    }else{
      hideWheel();
    }
  }
  function attach(){
    try{
      if(!db) return;
      const room = activeRoomCode();
      if(!room || room === listenRoom) return;
      if(listenRoom) db.ref('rooms/'+listenRoom+'/currentRound').off('value.bbWheelV123Only');
      listenRoom = room;
      db.ref('rooms/'+room+'/currentRound').on('value.bbWheelV123Only', s=>applyRoundState(s.val()||{}));
    }catch(_e){}
  }
  document.addEventListener('DOMContentLoaded',()=>setTimeout(attach,250));
  setInterval(()=>{
    attach();
    // Als oudere code toch een oud rad laat zien, blijft V123 erboven en wordt het oude overlay direct verborgen.
    if(q('bbWheelV123Overlay') && !q('bbWheelV123Overlay').classList.contains('hidden')) hideOldWheel();
  },300);
})();

/* =========================
   V125 - Wheel60 single source final
   - Oude rad overlays volledig uitgeschakeld
   - Één nieuw overlay-rad voor host en speler
   - Geen inline draaiend rad meer, dus geen groot/klein flikkeren
   - Animatie 12 seconden
   ========================= */
(function(){
  const q = id => document.getElementById(id);
  const E = s => (typeof esc === 'function' ? esc(String(s ?? '')) : String(s ?? '').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m])));
  ...
  function stopForIndex(i){
    i = normIndex(i);
    // V128: kleurmatch-fix. CSS rotate/conic gebruikt hier de tegengestelde richting
    // voor wat visueel onder de pijl komt. Daarom draaien we positief naar het
    // midden van het gekozen vakje, zodat de pijl exact op WHEEL[i] eindigt.
  ...
  function categoryFor(c){ try{return q(c.input)?.value || 'Geen categorie';}catch(_e){return 'Geen categorie';} }
  function randomIndex(){ return Math.floor(Math.random()*COUNT); }
  function ensureOverlay(){
    let o = q('bbWheelV125Overlay');
    if(!o){ o=document.createElement('div'); o.id='bbWheelV125Overlay'; o.className='hidden'; document.body.appendChild(o); }
    return o;
  }
  function wheelHTML(index, roundId){
    index = normIndex(index);
    const key = WHEEL[index] || 'yellow';
    return `<div class="bbWheelV125" data-index="${index}" data-round="${E(roundId||'')}">
      <div class="bbWheelPointerV125"></div>
      <div class="bbWheelDiscV125" style="--bb-stop:${stopForIndex(index)}deg;--bb-chosen:${HEX[key]};background:${gradient()}">
        <div class="bbWheelHubV125"></div>
      </div>
      <div class="bbWheelTextV125">Categorie wordt gekozen...</div>
    </div>`;
  }
  function resultHTML(color, cat){
    const hex = color?.hex || HEX[color?.key] || '#FFCC33';
    return `<div class="bbWheelResultV125" style="--bb-result:${hex}">
      <div class="bbWheelResultDotV125"></div>
      <div class="bbWheelResultNameV125">${E(color?.emoji||'')} ${E(color?.name||'KLEUR')}</div>
      <div class="bbWheelResultCatV125">${E(cat||'')}</div>
    </div>`;
  }
  function killOldWheels(){
    try{
      const old = q('bbWheelFullOverlayV92');
      if(old){ old.classList.add('hidden'); old.classList.remove('show'); old.style.setProperty('display','none','important'); old.style.setProperty('visibility','hidden','important'); old.innerHTML=''; }
      document.querySelectorAll('.bbWheelInlineV110,.bbWheelInlineV121,.bbWheelInlineV122,.bbWheelInlineV123,.bbWheelFullCardV92,.bbWheelV123Overlay').forEach(el=>{
        if(el.closest('#bbWheelV125Overlay')) return;
  ...
  function showWheel(index, roundId){
    index = normIndex(index);
    const key = `${roundId||''}:${index}`;
    const o = ensureOverlay();
    killOldWheels();
    if(activeKey !== key || !o.classList.contains('show')){
      activeKey = key;
      o.innerHTML = wheelHTML(index, roundId);
    }
    o.classList.remove('hidden'); o.classList.add('show');
    o.style.display='flex'; o.style.visibility='visible'; o.style.zIndex='2147483647';
  }
  function showResult(color,cat){
    activeKey='result';
    const o=ensureOverlay();
  ...
  function hideWheel(){
    activeKey='';
    const o=q('bbWheelV125Overlay');
    if(o){ o.classList.remove('show'); o.classList.add('hidden'); o.style.display='none'; o.style.visibility='hidden'; }
    killOldWheels();
  }

  // Alle oude publieke wheel APIs naar één nieuwe API sturen.
  window.__bbWheelV125 = {showWheel, showResult, hideWheel, wheelHTML, resultHTML, WHEEL};
  window.__bbWheelV92 = {showWheelFull:(x)=>showWheel(normIndex(x),''), showWheelResult:showResult, hideWheelFull:hideWheel};
  window.__bbWheelV110 = {showWheelV110:(x)=>showWheel(normIndex(x),''), showResultV110:showResult, hideWheelV110:hideWheel, wheelHTMLV110:(x)=>wheelHTML(normIndex(x),'')};
  window.__bbWheelV119 = {showWheel:(x)=>showWheel(normIndex(x),''), showResult, hideWheel, wheelHTML:(x)=>wheelHTML(normIndex(x),''), resultHTML, WHEEL_60:WHEEL};

  if(typeof pickerHTML === 'function') pickerHTML = function(){ return '<div class="bbWheelHostStatusV125">Categorie wordt gekozen...</div>'; };
  ...
  if(typeof startRoundVisual === 'function'){
    startRoundVisual = function(room){
      ...
      if(q('playBtn')){ q('playBtn').disabled=true; q('playBtn').textContent='🎵 Nummer start zo...'; }
      if(q('showAnswerBtn')) q('showAnswerBtn').disabled=true;
      if(q('hostPickerArea')) q('hostPickerArea').innerHTML='<div class="bbWheelHostStatusV125">Categorie wordt gekozen...</div>';

      const winningIndex = randomIndex();
      const colorKey = WHEEL[winningIndex];
      const color = colorObj(colorKey);
      const cat = categoryFor(color);
      currentRoundId = 'r_' + Date.now();
      ...
      db.ref('rooms/'+room+'/currentRound').set({
        ...
        pickerStartedAt:firebase.database.ServerValue.TIMESTAMP,
        seconds:Number(q('duration')?.value)||20
      }).then(()=>showWheel(winningIndex,currentRoundId));

      setTimeout(()=>{
        try{ if(typeof flash==='function') flash(); }catch(_e){}
        showResult(color,cat);
        if(q('hostPickerArea')) q('hostPickerArea').innerHTML=`<div class="bbWheelHostResultV125">${E(color.emoji)} ${E(color.name)}<br><span>${E(cat)}</span></div>`;
        const ends = Date.now()+3000;
        db.ref('rooms/'+room+'/currentRound').set({
          id:currentRoundId,
          status:'precount',
  ...
  renderCompactDashboard = function(room,r){
    if(r && r.status === 'picking'){
      const idx = Number.isFinite(Number(r.winningIndex)) ? normIndex(r.winningIndex) : 0;
      const root=q('screenDashboard');
      if(root){
        document.body.classList.add('bbStageMode');
        root.className='compactDashboard bbStageDashboard stageWheelV125';
        root.innerHTML='<section class="bbWheelPlayerStatusV125">Categorie wordt gekozen...</section>';
      }
      showWheel(idx,r.id||'');
      return;
    }
  ...
  let listening='';
  const OLD_NS=['bbV109Wheel','bbV110Wheel','bbV119Wheel','bbWheelV121'];
  function roomCodeActive(){ return (currentRoomCode || new URLSearchParams(location.search).get('room') || localStorage.hb_player_room || localStorage.hb_host_room || '').toUpperCase(); }
  function detachOld(room){
    try{ OLD_NS.forEach(ns=>db.ref('rooms/'+room+'/currentRound').off('value.'+ns)); }catch(_e){}
  }
  function attach(){
    try{
      if(!db) return;
      const room=roomCodeActive();
      if(!room) return;
      detachOld(room);
      if(room===listening) return;
      if(listening) db.ref('rooms/'+listening+'/currentRound').off('value.bbWheelV125');
      listening=room;
      db.ref('rooms/'+room+'/currentRound').on('value.bbWheelV125',s=>{
        const r=s.val()||{};
        detachOld(room);
        if(r.status==='picking'){
          const idx=Number.isFinite(Number(r.winningIndex)) ? normIndex(r.winningIndex) : 0;
  ...
  document.addEventListener('DOMContentLoaded',()=>setTimeout(attach,250));
  setInterval(()=>{ attach(); killOldWheels(); },350);
})();

/* V128: Wheel60 kleur-match fix: eindhoek draait positief zodat zichtbare stopkleur exact gelijk is aan categorie. */

/* =========================
   V129 - Geen wiel meer: Kleurencarrousel + Lichtronde
   - Host en speler zien dezelfde intro
   - 12 seconden, vloeiend, geen oude wheel overlays
   - Uitkomst vooraf gekozen en zichtbaar exact dezelfde kleur
   ========================= */
(function(){
  const q = id => document.getElementById(id);
  const E = s => (typeof esc === 'function' ? esc(String(s ?? '')) : String(s ?? '').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m])));
  ...
  function pickIntroType(){ return Math.random() < 0.5 ? 'carousel' : 'lightRing'; }
  function roomCodeActive(){ return (currentRoomCode || new URLSearchParams(location.search).get('room') || localStorage.hb_player_room || localStorage.hb_host_room || '').toUpperCase(); }

  function ensureOverlay(){
    let o = q('bbIntroV129Overlay');
    if(!o){ o = document.createElement('div'); o.id='bbIntroV129Overlay'; o.className='bbIntroV129Overlay hidden'; document.body.appendChild(o); }
    return o;
  }
  function hideOldWheels(){
    try{
      ['bbWheelV125Overlay','bbWheelV123Overlay','bbWheelFullOverlayV92'].forEach(id=>{
        const el=q(id); if(el){ el.classList.add('hidden'); el.classList.remove('show'); el.style.setProperty('display','none','important'); el.style.setProperty('visibility','hidden','important'); }
      });
      document.querySelectorAll('[class*="Wheel"],.wheel,.wheelWrap,.bbWheelV91,.bbWheelFullCardV92').forEach(el=>{
        if(el.closest('#bbIntroV129Overlay')) return;
        if(el.id === 'bbIntroV129Overlay') return;
        el.style.setProperty('display','none','important');
        el.style.setProperty('visibility','hidden','important');
      });
    }catch(_e){}
  ...
  function hideIntro(){
    const o=q('bbIntroV129Overlay');
    if(o){ o.classList.add('hidden'); o.classList.remove('show'); o.style.display='none'; o.style.visibility='hidden'; }
    activeIntroKey='';
    hideOldWheels();
  }

  function carouselHTML(colorKey, roundId){
    const target = 72;
    const tile = 76, gap = 8;
    const vw = Math.max(320, Math.min(900, window.innerWidth || 390));
    const shift = Math.round((vw/2) - (target*(tile+gap) + tile/2));
    const keys=[];
    for(let i=0;i<95;i++) keys.push(KEYS[(i*7 + 2) % KEYS.length]);
    keys[target] = colorKey;
    // Zorg dat vlakjes rondom ook gemixt blijven.
    keys[target-1] = KEYS[(KEYS.indexOf(colorKey)+2)%KEYS.length];
    keys[target+1] = KEYS[(KEYS.indexOf(colorKey)+3)%KEYS.length];
    return `<div class="bbIntroV129 bbIntroCarouselV129" data-round="${E(roundId||'')}">
      <div class="bbIntroPointerV129"></div>
      <div class="bbCarouselMaskV129">
        <div class="bbCarouselTrackV129" style="--bb-shift:${shift}px">
          ${keys.map(k=>`<span class="bbCarouselTileV129" style="--c:${HEX[k]}"></span>`).join('')}
        </div>
      </div>
      <div class="bbIntroTextV129">Categorie wordt gekozen...</div>
    </div>`;
  }
  function lightRingHTML(colorKey, roundId){
    const pos = {yellow:0, pink:72, purple:144, blue:216, green:288};
    const final = pos[colorKey] ?? 0;
    return `<div class="bbIntroV129 bbIntroLightRingV129" data-round="${E(roundId||'')}">
      <div class="bbLightPointerV129"></div>
      <div class="bbLightRingV129">
        ${KEYS.map((k,i)=>`<span class="bbLightSegmentV129 seg${i}" style="--c:${HEX[k]}"></span>`).join('')}
        <span class="bbLightDotV129" style="--bb-final:${final}deg;--bb-dot:${HEX[colorKey]}"></span>
        <span class="bbLightCenterV129"></span>
      </div>
      <div class="bbIntroTextV129">Categorie wordt gekozen...</div>
    </div>`;
  }
  function resultHTML(color, cat){
    return `<div class="bbIntroResultV129" style="--bb-result:${color.hex}">
      <div class="bbIntroResultDotV129"></div>
      <div class="bbIntroResultNameV129">${E(color.name)}</div>
      <div class="bbIntroResultCatV129">${E(cat)}</div>
    </div>`;
  }
  function showIntro(type, colorKey, roundId){
    const o=ensureOverlay();
    const key = `${roundId||''}:${type}:${colorKey}`;
    hideOldWheels();
    if(activeIntroKey !== key || !o.classList.contains('show')){
      activeIntroKey = key;
      o.innerHTML = type === 'lightRing' ? lightRingHTML(colorKey, roundId) : carouselHTML(colorKey, roundId);
    }
    o.classList.remove('hidden'); o.classList.add('show');
    o.style.display='flex'; o.style.visibility='visible'; o.style.zIndex='2147483647';
  }
  ...
  // Oude wheel APIs neutraal maken zodat oude listeners niets visueels meer kunnen tonen.
  window.__bbWheelV92 = {showWheelFull:()=>{}, showWheelResult:()=>{}, hideWheelFull:()=>{}};
  window.__bbWheelV125 = {showWheel:()=>{}, showResult:()=>{}, hideWheel:()=>{}, wheelHTML:()=>''};
  window.__bbIntroV129 = {showIntro, showResult, hideIntro};
  if(typeof pickerHTML === 'function') pickerHTML = () => '<div class="bbIntroHostStatusV129">Categorie wordt gekozen...</div>';
  ...
  if(typeof startRoundVisual === 'function'){
    startRoundVisual = function(room){
      ...
      if(q('playBtn')){ q('playBtn').disabled=true; q('playBtn').textContent='🎵 Nummer start zo...'; }
      if(q('showAnswerBtn')) q('showAnswerBtn').disabled=true;
      if(q('hostPickerArea')) q('hostPickerArea').innerHTML='<div class="bbIntroHostStatusV129">Categorie wordt gekozen...</div>';

      const colorKey = pickKey();
      const introType = pickIntroType();
      const color = colorObj(colorKey);
      const cat = categoryFor(color);
      currentRoundId = 'r_' + Date.now();
      ...
      db.ref('rooms/'+room+'/currentRound').set({
        ...
        pickerStartedAt:firebase.database.ServerValue.TIMESTAMP,
        seconds:Number(q('duration')?.value)||20
      }).then(()=>showIntro(introType,color.key,currentRoundId));

      setTimeout(()=>{
        try{ if(typeof flash==='function') flash(); }catch(_e){}
        showResult(color,cat);
        if(q('hostPickerArea')) q('hostPickerArea').innerHTML=`<div class="bbIntroHostResultV129">${E(color.emoji)} ${E(color.name)}<br><span>${E(cat)}</span></div>`;
        const ends = Date.now()+3000;
        db.ref('rooms/'+room+'/currentRound').set({
          id:currentRoundId,
          status:'precount',
  ...
  renderCompactDashboard = function(room,r){
    if(r && r.status === 'picking'){
      ...
      if(root){
        document.body.classList.add('bbStageMode');
        root.className='compactDashboard bbStageDashboard stageIntroV129';
        root.innerHTML='<section class="bbIntroPlayerStatusV129">Categorie wordt gekozen...</section>';
      }
      showIntro(r.introType || 'carousel', r.pendingColorKey || r.colorKey || 'yellow', r.id || '');
      return;
    }
  ...
  function detachOld(room){
    if(!db || !room) return;
    ['bbWheelV125','bbWheelV123Only','bbWheelV121','bbV109Wheel','bbV110Wheel','bbV119Wheel'].forEach(ns=>{
      try{ db.ref('rooms/'+room+'/currentRound').off('value.'+ns); }catch(_e){}
    });
  }
  function attach(){
    try{
      ...
      if(!room) return;
      detachOld(room);
      if(room===listenRoom) return;
      if(listenRoom) db.ref('rooms/'+listenRoom+'/currentRound').off('value.bbIntroV129');
      listenRoom=room;
      db.ref('rooms/'+room+'/currentRound').on('value.bbIntroV129',s=>{
        const r=s.val()||{};
        detachOld(room);
        if(r.status==='picking'){
          showIntro(r.introType || 'carousel', r.pendingColorKey || r.colorKey || 'yellow', r.id || '');
  ...
  document.addEventListener('DOMContentLoaded',()=>setTimeout(attach,250));
  setInterval(()=>{ attach(); hideOldWheels(); },250);
})();
