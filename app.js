...
(function(){
  ...
  const SCORE_MS = 3600;
  let v111ScoreRefreshTimer = null;
  ...
  function compactAnswer(r){
    const a=r?.correctAnswer||{};
    if(!a.track && !a.artist) return '';
    return `<div class="bbV111CorrectMini"><div class="bbV111Label">Juiste antwoord</div><strong>${E(a.track||'-')}</strong><span>${E(a.artist||'-')}${a.year?' • '+E(a.year):''}</span></div>`;
  }
  function bigAnswerHTML(r){
    const a=r?.correctAnswer||{};
    return `<section class="bbV111BigAnswer"><img src="bb_logo.png" alt="Bingo Beats"><div>Juiste antwoord</div><h1>${E(a.track||'-')}</h1><p>${E(a.artist||'-')}${a.year?' • '+E(a.year):''}</p></section>`;
  }
  ...
  function scheduleV111ScoreExit(r){
    try{
      clearTimeout(v111ScoreRefreshTimer);
      const until = Number(r?.scoreUntilMs || 0);
      const wait = until ? Math.max(80, until - Date.now() + 120) : 3800;
      v111ScoreRefreshTimer = setTimeout(()=>{
        try{
          if(!currentRoomCode || !db) return;
          db.ref('rooms/'+currentRoomCode).once('value').then(s=>{
            const latest = s.val() || {};
            const lr = latest.currentRound || {};
            if(lr.id === r.id && lr.status === 'judged' && typeof renderCompactDashboard === 'function'){
              renderCompactDashboard(latest, lr);
            }
          });
        }catch(_e){}
      }, wait);
  ...
  renderHostScore = function(room){
    ...
    const ps=room.players||{}, ans=room.answers?.[r.id]||{}, cor=room.correct?.[r.id]||{};
    let html = (r.correctAnswer ? `<div class="bbV111HostAnswer">⭐ <strong>${E(r.correctAnswer.track||'-')}</strong><br><span>${E(r.correctAnswer.artist||'-')}${r.correctAnswer.year?' • '+E(r.correctAnswer.year):''}</span></div>` : '');
