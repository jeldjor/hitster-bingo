...
function pickerHTML(mode){
  return `<div class="bbSimplePicker">
    <div class="bbSimpleIcon">🎮</div>
    <div class="pickerTitle">Kleur wordt gekozen...</div>
  </div>`;
}
...
/* removed category storage: hardcoded categories in V51 */



/* ==================================================
   V51 HARDCODED CATEGORIEËN
   Vaste categorieën. Geen opslaan/localStorage/Firebase meer.
   ================================================== */
(function(){
  const FIXED_CATS = {
    "cat-yellow": "Voor of na 2001",
    "cat-pink": "Naam van artiest",
    "cat-purple": "Decennium",
    "cat-blue": "Jaartal +/- 2",
    "cat-green": "Titel van track"
  };

  function applyFixedCategories(){
    Object.entries(FIXED_CATS).forEach(([id,value])=>{
      const el=document.getElementById(id);
      if(!el) return;
      el.value=value;
      ...
      el.title="Vaste Bingo Beats categorie";
    });
  }

  document.addEventListener("DOMContentLoaded",applyFixedCategories);
  window.addEventListener("pageshow",()=>setTimeout(applyFixedCategories,50));
  window.addEventListener("focus",()=>setTimeout(applyFixedCategories,50));
  setTimeout(applyFixedCategories,50);
  setTimeout(applyFixedCategories,300);
  setTimeout(applyFixedCategories,1000);
  setInterval(applyFixedCategories,1500);
})();
...
/* ==================================================
   ...
   ================================================== */
(function(){
  window.bbPlayerJoinInProgress = false;
  ...
  function persistPlayerState(){
    try{
      ...
      if(room) localStorage.setItem("roomCode", room);
  ...
  document.addEventListener("click", function(e){
    ...
    if(txt.includes("meedoen")){
      window.bbPlayerJoinInProgress = true;
      persistPlayerState();
  ...
  const observer = new MutationObserver(()=>{
    if(!window.bbPlayerJoinInProgress) return;

    const joinScreen =
      document.getElementById("joinScreen") ||
      document.querySelector(".joinScreen") ||
      document.querySelector("[data-screen='join']");

    const lobbyScreen =
      document.getElementById("lobbyScreen") ||
      document.querySelector(".lobbyScreen") ||
      document.querySelector("[data-screen='lobby']");

    if(joinScreen && lobbyScreen){
      try{
...
/* ==================================================
   ...
   ================================================== */
(function(){
  const MASCOT_SRC = "bb_mascot_dj.png";
  let badShownKey = "";
  let goodOpenKey = "";
...
/* ==================================================
   ...
   ================================================== */
(function(){
  const BB_MASCOT = "bb_mascot_dj.png";
  const BB_LOGO = "bb_logo_lime.png";
...
function copyRoomLink(){
  ...
  const text = input?.value || (currentRoomCode ? location.origin + location.pathname + "?room=" + currentRoomCode : "");
  if(!text) return;
  const done = () => {
    if(btn){
...
(function(){
  ...
  function isPlayer(){ return !!new URLSearchParams(location.search).get("room"); }

  function setModeAndVisibilityV73(){
    const mt = q("modeText");
  ...
  if(typeof hbRenderRoomBoxV7B === "function"){
    hbRenderRoomBoxV7B = function(code){
      ...
      box.classList.remove("hidden");
      box.innerHTML = `
        <div class="hostRoomGridV73">
          <div class="hostRoomCodeV73">
            <div class="hostRoomLabelV73">SPELCODE</div>
            <div class="hostRoomCodeTextV73">${esc(code)}</div>
          </div>
          <div class="hostRoomLinkV73">
            <div class="hostRoomLabelV73">DEEL DE LINK</div>
            <input id="joinLink" readonly value="${esc(link)}">
            <button id="copyRoomLinkBtn" type="button" class="copyRoomLinkBtn">📋 Kopieer link</button>
          </div>
          <img class="hostQrV73" alt="QR code" src="${hbQrV7B(link)}">
        </div>`;
    };
  }

  if(typeof showRoom === "function" && !window.__showRoomV73Wrapped){
    window.__showRoomV73Wrapped = true;
    const oldShowRoom = showRoom;
    showRoom = function(){
      const out = oldShowRoom.apply(this, arguments);
      const code = (typeof currentRoomCode !== "undefined" && currentRoomCode) ? currentRoomCode : localStorage.getItem("hb_host_room");
      if(code && typeof hbRenderRoomBoxV7B === "function") setTimeout(() => hbRenderRoomBoxV7B(code), 30);
      return out;
    };
  }

  window.bbPickerHTMLV73 = function(){
    return `<div class="bbPickerV73">
      <img src="bb_logo_purple.png" class="bbPickerLogoV73" alt="Bingo Beats">
      <div class="bbPickerTitleV73">🐵 BB-aap kiest een kleur...</div>
      <div class="bbPickerDotsV73">
      ...
      <div class="bbPickerSmallV73">Nog even spannend...</div>
    </div>`;
  };

  if(typeof pickerHTML === "function") pickerHTML = function(){ return window.bbPickerHTMLV73(); };
  if(typeof sharedPickerHTML === "function") sharedPickerHTML = function(){ return window.bbPickerHTMLV73(); };

  if(typeof renderPlayerPicker === "function"){
    renderPlayerPicker = function(round){
      const area = q("playerPickerArea") || q("dashPickerArea");
      if(area) area.innerHTML = window.bbPickerHTMLV73();
    };
  }
  if(typeof renderPlayerPickerFixed === "function"){
    renderPlayerPickerFixed = function(round){
      const area = q("playerPickerArea") || q("dashPickerArea");
      if(area) area.innerHTML = window.bbPickerHTMLV73();
    };
  ...
  if(typeof renderCompactPicker === "function"){
    renderCompactPicker = function(room, r){
      ...
      if(r && r.status === "picking"){
        area.innerHTML = window.bbPickerHTMLV73();
        return;
      }
      if(r && (r.colorName || r.category)){
        const map = {yellow:"#FFCC33",pink:"#00D4C7",purple:"#FF8A1F",blue:"#7ED957",green:"#FF5A5F"};
        area.innerHTML = `<div class="bbPickedV73">
          <div class="bbPickedBallV73" style="background:${map[r.colorKey] || "#FFCC33"}"></div>
          <div class="bbPickedNameV73">${esc(r.colorName || "")}</div>
          <div class="bbPickedCatV73">${esc(r.category || "")}</div>
        </div>`;
        return;
      }
  ...
  document.addEventListener("DOMContentLoaded", setModeAndVisibilityV73);
  setTimeout(setModeAndVisibilityV73, 250);
...
/* =========================
   V75 iPHONE RESPONSIVE
   - Speler badge linksboven
   - Host badge blijft onder logo
   - Speler-grid compact
   ========================= */
(function(){
  function q(id){ return document.getElementById(id); }
  ...
  function ensurePlayerCornerBadgeV75(){
    let badge = q("playerCornerBadgeV75");
    if(isPlayer()){
      if(!badge){
        badge = document.createElement("div");
        badge.id = "playerCornerBadgeV75";
        badge.className = "playerCornerBadgeV75";
        badge.textContent = "🎮 Speler";
  ...
  document.addEventListener("DOMContentLoaded", setModeV75);
  setTimeout(setModeV75, 200);
  setTimeout(setModeV75, 1000);
  setInterval(setModeV75, 2500);
})();


/* =========================
   V77 BB AAP OVERLAYS
   Hulpfuncties voor goed/fout feedback.
   ========================= */
window.bbShowCorrectOverlay = function(){
  let o=document.getElementById("bbMonkeyOverlay");
  if(!o){
    o=document.createElement("div");
    o.id="bbMonkeyOverlay";
    o.className="bbMonkeyOverlay";
    document.body.appendChild(o);
  }
  o.innerHTML='<div class="bbMonkeyCard"><img src="bb_mascot_dj.png" class="bbMonkeyImg"><h2>😎 GOED ANTWOORD!</h2><p>Die had je goed!</p><div class="bbMonkeySub">Kies een vakje op je bingokaart</div></div>';
  o.classList.add("show");
};

window.bbShowWrongOverlay = function(answer){
  let o=document.getElementById("bbMonkeyOverlay");
  if(!o){
    o=document.createElement("div");
    o.id="bbMonkeyOverlay";
  ...
  o.innerHTML='<div class="bbMonkeyCard"><img src="bb_mascot_dj.png" class="bbMonkeyImg"><h2>🙈 HELAAS!</h2><p>'+(answer||'Volgende ronde beter!')+'</p></div>';
  o.classList.add("show");
...
/* =========================
   V81 - FORCE BB AAP OVERLAY
   Als de kleine vakjes al oplichten, forceert deze laag alsnog de fullscreen BB-aap + grote kaart.
   ========================= */
(function(){
  const seen = {};
  ...
  function colorHex(key){
    return {yellow:"#FFCC33",pink:"#00D4C7",purple:"#FF8A1F",blue:"#7ED957",green:"#FF5A5F",free:"#FFCC33"}[key] || "#ffffff";
  }

  function icon(c, marked, i){
    if(marked || i === 12) return "🐵";
    return {yellow:"🟡",pink:"🩵",purple:"🟠",blue:"🟢",green:"🔴",free:"🐵"}[c] || "";
  }

  function overlay(){
    let o = q("bbForceOverlayV81");
    if(!o){
      o = document.createElement("div");
      o.id = "bbForceOverlayV81";
      o.className = "bbForceOverlayV81";
      document.body.appendChild(o);
    }
    return o;
  }
  ...
  function showGood(room, r){
    ...
    const cells = card.map((c,i)=>{
      ...
      const pickable = c === r.colorKey && c !== "free" && !marked[i];
      return `<button type="button"
        class="bbForceCellV81 ${isMarked ? "marked" : ""} ${pickable ? "pickable" : "blocked"}"
        data-i="${i}"
        style="background:${colorHex(c)}"
        ${pickable ? "" : "disabled"}>${icon(c,isMarked,i)}</button>`;
    }).join("");

    const o = overlay();
    o.innerHTML = `
      <div class="bbForceCardV81 good">
        <div class="bbForceMonkeyWrapV81">
          <img src="bb_logo_lime.png" class="bbForceMonkeyV81" alt="Bingo Beats" onerror="this.style.display='none';this.nextElementSibling.style.display='block'">
          <div class="bbForceMonkeyFallbackV81">🐵😎</div>
        </div>
        <div class="bbForceTitleV81">GOED ANTWOORD!</div>
        <div class="bbForceTextV81">Kies één ${r.colorEmoji || ""} ${r.colorName || ""} vakje</div>
        <div class="bbForceBingoV81">${cells}</div>
      </div>`;
    o.classList.add("show");

    o.querySelectorAll(".bbForceCellV81.pickable").forEach(btn=>{
      btn.addEventListener("click", ()=>{
        const i = Number(btn.dataset.i);
        btn.textContent = "🐵";
        btn.classList.add("chosen");
  ...
  function showBad(room, r){
    ...
    const o = overlay();
    o.innerHTML = `
      <div class="bbForceCardV81 bad">
        <div class="bbForceMonkeyWrapV81">
          <img src="bb_logo_red.png" class="bbForceMonkeyV81" alt="Bingo Beats" onerror="this.style.display='none';this.nextElementSibling.style.display='block'">
          <div class="bbForceMonkeyFallbackV81">🐵🙈</div>
        </div>
        <div class="bbForceTitleV81">HELAAS!</div>
        <div class="bbForceTextV81">Deze was lastig...</div>
        <div class="bbForceAnswerV81">${answer}</div>
        <button type="button" class="bbForceContinueV81">Verder</button>
      </div>`;
    o.classList.add("show");
    o.querySelector(".bbForceContinueV81")?.addEventListener("click", close);
  ...
  function check(){
    try{
      ...
      console.error("V81 overlay check fout", e);
    }
  }

  // Extra trigger: zodra kleine vakjes oplichten, check direct.
  const obs = new MutationObserver(()=>{
    if(document.querySelector(".compactCell.pickable, .pickableCell, #dashOwnCard .pickable")){
      check();
    }
  });
