...
(function(){
  ...
  function renderHostWheel(index, roundId=''){
    const el = q('hostPickerArea');
    if(!el) return;
    const key = `${roundId}:${normIndex(index)}`;
    if(el.dataset.bbWheelKey !== key || !el.querySelector('.bbWheelV123')){
      el.innerHTML = wheelHTML(index, roundId);
  ...
  renderCompactDashboard = function(room, r){
    if(r && r.status === 'picking'){
      ...
      if(root){
        ...
        root.innerHTML = `<section class="bbWheelInlineV123">${wheelHTML(idx, r.id||'')}</section>`;
      }
      showWheel(idx, r.id||'');
      return;
    }
