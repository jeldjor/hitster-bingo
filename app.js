...
/* V7 startpopup + kamerlock */
(function(){
 const ROOM_KEY="hb_host_room", LAST_KEY="hb_last_stable_room";
 const isPlayer=()=>!!new URLSearchParams(location.search).get("room");
 const savedRoom=()=>localStorage.getItem(ROOM_KEY)||localStorage.getItem(LAST_KEY)||"";
 const saveRoom=c=>{if(c){localStorage.setItem(ROOM_KEY,c);localStorage.setItem(LAST_KEY,c);}};
 const qr=t=>"https://api.qrserver.com/v1/create-qr-code/?size=220x220&data="+encodeURIComponent(t||"");
 function renderRoomBox(code){
   const box=document.getElementById("roomBox"); if(!box||!code)return;
   const link=location.origin+location.pathname+"?room="+code;
   box.classList.remove("hidden"); box.classList.add("hbRoomRestored");
   if(box.dataset.hbQrCode===code)return; box.dataset.hbQrCode=code;
   box.innerHTML=`<div style="display:grid;grid-template-columns:1fr 126px;gap:14px;align-items:center">
    <div><div style="font-size:12px;opacity:.75;font-weight:900;margin-bottom:5px">SPELCODE</div>
    <div style="font-size:42px;font-weight:900;color:#73ff73;letter-spacing:3px;line-height:1">${esc(code)}</div>
    <div style="width:100%;margin-top:10px;padding:8px 9px;border-radius:10px;background:rgba(0,0,0,.25);border:1px solid rgba(255,255,255,.14);font-size:11px;word-break:break-all">${esc(link)}</div></div>
    <img alt="QR code" src="${qr(link)}" style="width:126px;height:126px;background:white;border-radius:12px;padding:6px;box-shadow:0 0 18px rgba(255,255,255,.25)"></div>`;
 }
 function resumeRoom(code){
   if(!code||!db)return; currentRoomCode=code; saveRoom(code); renderRoomBox(code);
   try{listenHost(code)}catch(e){} try{listenBingo(code)}catch(e){}
   const st=document.getElementById("hostStatus"); if(st)st.textContent="Kamer hervat. Klaar voor nieuwe ronde.";
 }
 function removeModal(){document.getElementById("hbStartOverlay")?.remove();}
 function showModal(){
   if(isPlayer()||document.getElementById("hbStartOverlay"))return;
   const saved=savedRoom();
   const overlay=document.createElement("div"); overlay.id="hbStartOverlay"; overlay.className="hbStartOverlay";
   overlay.innerHTML=`<div class="hbStartModal"><h2>🎵 Voor je begint</h2>
    <div class="small">Controleer dit even voordat je de ronde start.</div>
    ${saved?`<p style="margin:12px 0 0">Laatste kamer: <span class="hbRoomCode">${esc(saved)}</span></p>`:`<p style="margin:12px 0 0">Geen vorige kamer gevonden.</p>`}
    <div class="hbStartChecks">
      <div class="hbStartCheck">✅ Zorg dat je Spotify-app geopend is</div>
      <div class="hbStartCheck">✅ Zorg dat Spotify niet actief is op telefoon, TV of ander apparaat</div>
      <div class="hbStartCheck">✅ Upload/controleer je CSV voordat je echt begint</div>
    </div>
    <div class="hbStartActions">
      <button id="hbResumeRoomBtn" class="hbResumeBtn" ${saved?"":"disabled"}>🔄 Hervat ${saved?esc(saved):"kamer"}</button>
      <button id="hbNewRoomBtn" class="hbNewBtn">🎫 Nieuwe kamer</button>
    </div>
    <div class="hbSmallNote">Spotify, CSV uploaden en apparaat activeren veranderen de kamer niet. Alleen “Nieuwe kamer” maakt bewust een nieuwe spelcode.</div></div>`;
   document.body.appendChild(overlay);
   document.getElementById("hbResumeRoomBtn")?.addEventListener("click",()=>{if(saved)resumeRoom(saved);removeModal();});
   document.getElementById("hbNewRoomBtn")?.addEventListener("click",()=>{localStorage.removeItem(ROOM_KEY);localStorage.removeItem(LAST_KEY);currentRoomCode="";removeModal(); if(typeof createRoom==="function")createRoom();});
 }
 if(typeof createRoom==="function"&&!window.__hbCreateRoomV7){window.__hbCreateRoomV7=1;const old=createRoom;createRoom=function(){const r=old.apply(this,arguments);setTimeout(()=>{const c=currentRoomCode||localStorage.getItem(ROOM_KEY);if(c){saveRoom(c);renderRoomBox(c);}},900);return r;};}
 if(typeof showRoom==="function"&&!window.__hbShowRoomV7){window.__hbShowRoomV7=1;const old=showRoom;showRoom=function(){const r=old.apply(this,arguments);const c=currentRoomCode||localStorage.getItem(ROOM_KEY);if(c){saveRoom(c);setTimeout(()=>renderRoomBox(c),50);}return r;};}
 function protect(fn){return function(){const room=currentRoomCode||savedRoom();const out=fn.apply(this,arguments);setTimeout(()=>{if(room&&!isPlayer()){if(!currentRoomCode)currentRoomCode=room;saveRoom(room);renderRoomBox(room);}},900);return out;};}
 if(typeof login==="function"&&!window.__hbLoginLockV7){window.__hbLoginLockV7=1;login=protect(login);}
 if(typeof activatePlayer==="function"&&!window.__hbActLockV7){window.__hbActLockV7=1;activatePlayer=protect(activatePlayer);}
 if(typeof handleCsv==="function"&&!window.__hbCsvLockV7){window.__hbCsvLockV7=1;handleCsv=protect(handleCsv);}
 if(typeof loadCsv==="function"&&!window.__hbLoadCsvLockV7){window.__hbLoadCsvLockV7=1;loadCsv=protect(loadCsv);}
 function playerCodeTop(){if(!isPlayer())return;const room=(new URLSearchParams(location.search).get("room")||currentRoomCode||localStorage.getItem("hb_player_room")||"").toUpperCase();const rn=document.getElementById("dashRoundNo");if(rn&&room)rn.textContent=room;}
 setTimeout(()=>{playerCodeTop();if(!isPlayer()){showModal();const s=savedRoom();if(s)renderRoomBox(s);}},900);
 setInterval(playerCodeTop,1500);
 window.hbHostResumeRoom=resumeRoom;
})();
