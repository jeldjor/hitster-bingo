...
/* V11 Host speelt mee - aparte speleromgeving met hostpaneel */
let hbHostSelfIdV11=localStorage.getItem("hb_host_self_id")||"";
let hbHostSelfNameV11=localStorage.getItem("hb_host_self_name")||"";

function hbRoomV11(){return currentRoomCode||localStorage.getItem("hb_host_room")||localStorage.getItem("hb_last_stable_room")||"";}

function hbEnsureHostLauncherV11(){
 if(new URLSearchParams(location.search).get("room"))return;
 const roomPanel=Array.from(document.querySelectorAll("#hostApp > .panel")).find(p=>p.textContent.includes("Kamer"));
 if(roomPanel&&!document.getElementById("hostModeLauncherV11")){
  const box=document.createElement("div");
  box.id="hostModeLauncherV11";box.className="hostModeLauncher";
  box.innerHTML='<h3>Host modus</h3><button id="onlyHostBtnV11" class="secondary">🎮 Alleen host</button><button id="hostPlayBtnV11">👤 Host speelt mee</button><p class="small">Doe eerst Spotify, CSV, categorieën en kamer. Kies daarna je modus.</p>';
  roomPanel.appendChild(box);
  document.getElementById("onlyHostBtnV11").addEventListener("click",()=>alert("Alleen host actief."));
  document.getElementById("hostPlayBtnV11").addEventListener("click",hbStartHostSelfEnvV11);
 }
}

function hbEnsureHostFloatingV11(){
 if(!document.getElementById("hostFloatBtnV11")){
  const btn=document.createElement("button");
  btn.id="hostFloatBtnV11";btn.className="hostFloatBtn hidden";btn.textContent="⚙️ Host";
  btn.addEventListener("click",hbOpenHostControlV11);document.body.appendChild(btn);
 }
 if(!document.getElementById("hostControlCloseV11")){
  const close=document.createElement("button");
  close.id="hostControlCloseV11";close.className="hostControlClose hidden";close.textContent="← Terug";
  close.addEventListener("click",hbCloseHostControlV11);document.body.appendChild(close);
 }
 if(!document.getElementById("hostControlModalV11")){
  const modal=document.createElement("div");
  modal.id="hostControlModalV11";modal.className="hostControlModal";
  modal.innerHTML='<div class="hostControlCard"><div id="hostSelfWarningV11" class="hostSelfWarning hidden"></div></div>';
  document.body.appendChild(modal);
 }
}

function hbStartHostSelfEnvV11(){
 const room=hbRoomV11();
 if(!room){alert("Maak of hervat eerst een kamer.");return;}
 let name=hbHostSelfNameV11||localStorage.getItem("hb_player_name")||"";
 name=(prompt("Naam waarmee jij meespeelt:",name||"Host")||"").trim();
 if(!name)return;
 if(!hbHostSelfIdV11){
  hbHostSelfIdV11="host_"+Math.random().toString(36).slice(2,10)+Date.now().toString(36).slice(-4);
  localStorage.setItem("hb_host_self_id",hbHostSelfIdV11);
 }
 hbHostSelfNameV11=name;localStorage.setItem("hb_host_self_name",name);
 db.ref("rooms/"+room+"/players/"+hbHostSelfIdV11).once("value").then(s=>{
  const ex=s.val()||{};
  return db.ref("rooms/"+room+"/players/"+hbHostSelfIdV11).update({
   name,online:true,ready:true,isHostPlayer:true,
   joinedAt:ex.joinedAt||firebase.database.ServerValue.TIMESTAMP,
   lastSeen:firebase.database.ServerValue.TIMESTAMP,
   card:ex.card||genCard(),marked:ex.marked||{}
  });
 }).then(()=>{
  currentRoomCode=room;currentPlayerId=hbHostSelfIdV11;currentPlayerName=name;
  localStorage.setItem("hb_player_room",room);localStorage.setItem("hb_player_id",hbHostSelfIdV11);localStorage.setItem("hb_player_name",name);
  hbEnterHostSelfEnvV11();
 });
}

function hbEnterHostSelfEnvV11(){
 hbEnsureHostFloatingV11();
 document.body.classList.add("playerMode","hostSelfEnv");
 document.body.classList.remove("hostControlOpen");
 const mode=document.getElementById("modeText");if(mode)mode.textContent="Host speelt mee";
 document.getElementById("playerApp")?.classList.remove("hidden");
 document.getElementById("hostFloatBtnV11")?.classList.remove("hidden");
 const roomCode=document.getElementById("playerRoomCode");if(roomCode)roomCode.textContent=currentRoomCode;
 document.getElementById("screenJoin")?.classList.add("hidden");
 document.getElementById("screenDashboard")?.classList.remove("hidden");
 if(typeof listenPlayer==="function")listenPlayer();
 if(typeof hbEnsurePlayerViewToggleV9==="function")hbEnsurePlayerViewToggleV9();
 hbWatchHostSelfSafetyV11();
}

function hbOpenHostControlV11(){
 hbEnsureHostFloatingV11();
 const card=document.querySelector("#hostControlModalV11 .hostControlCard");
 const hostApp=document.getElementById("hostApp");
 if(card&&hostApp&&!card.contains(hostApp))card.appendChild(hostApp);
 document.body.classList.add("hostControlOpen");
 document.getElementById("hostControlCloseV11")?.classList.remove("hidden");
 hbUpdateHostSafetyV11();
}

function hbCloseHostControlV11(){
 document.body.classList.remove("hostControlOpen");
 document.getElementById("hostControlCloseV11")?.classList.add("hidden");
}

function hbHostHasAnsweredV11(room){
 if(!room||!room.currentRound||!room.currentRound.id)return true;
 const r=room.currentRound;
 if(r.status!=="answering")return true;
 return !!(room.answers&&room.answers[r.id]&&room.answers[r.id][hbHostSelfIdV11]);
}

function hbWatchHostSelfSafetyV11(){
 const room=hbRoomV11();
 if(!room||window.__hbHostSelfSafetyRoomV11===room)return;
 window.__hbHostSelfSafetyRoomV11=room;
 db.ref("rooms/"+room).on("value",snap=>hbUpdateHostSafetyV11(snap.val()||{}));
}

function hbUpdateHostSafetyV11(data){
 if(!document.body.classList.contains("hostSelfEnv"))return;
 if(!data)return;
 const answered=hbHostHasAnsweredV11(data);
 document.body.classList.toggle("hostNotAnswered",!answered);
 const warn=document.getElementById("hostSelfWarningV11");
 if(warn){
  if(!answered){warn.classList.remove("hidden");warn.textContent="Je speelt mee: vul eerst jouw eigen antwoord in. Tot die tijd zijn antwoorden/uitslag verborgen en Toon antwoord geblokkeerd.";}
  else{warn.classList.add("hidden");warn.textContent="";}
 }
}

setTimeout(hbEnsureHostLauncherV11,600);
setTimeout(hbEnsureHostLauncherV11,1600);
setInterval(hbEnsureHostLauncherV11,3000);
